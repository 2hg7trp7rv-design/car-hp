...
const SOURCE_LABELS: Array<[RegExp, Omit<SourceDisplay, "url">]> = [
  ...
  [/hks-power\.co\.jp\/en\/product\/intake\/spf\/index\.html$/u, { title: "HKS｜Super Power Flow", meta: "メーカー公式 / オープンタイプエアクリーナー" }],
...
function sourceDisplay(value: ArticleSource): SourceDisplay {
  if (typeof value !== "string") {
    return {
      url: value.url,
      title: value.title,
      meta: value.publisher?.trim() || "参考資料",
      claim: value.claim?.trim() || null,
      accessedAt: value.accessedAt?.trim() || null,
    };
  }
  try {
        ...
        return (
          <li key={`${display.url}-${index}`}>
            ...
            <a href={display.url} target="_blank" rel="noreferrer">
              ...
              {display.claim ? <small className={styles.sourceClaim}>支える主張：{display.claim}</small> : null}
              {display.accessedAt ? <small className={styles.sourceAccessedAt}>確認日：{formatDateDot(display.accessedAt)}</small> : null}
            </a>
          </li>
        );
      })}</ol> : null}
