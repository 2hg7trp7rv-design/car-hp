  ...
  return (
    <main className="relative min-h-screen text-white">
      ...
      <div className="fixed inset-x-0 top-0 z-40">
        <div className="mx-auto h-[3px] max-w-6xl rounded-full bg-black/80/5 shadow-glass-inner backdrop-blur">
          <div
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-10 px-4 pb-20 pt-24 sm:px-6 lg:flex-row lg:gap-16 lg:px-8">
        ...
        <article className="w-full lg:w-[68%]">
          <GlassCard
            padding="lg"
            className="porcelain relative overflow-hidden border border-white/80 bg-white/95 px-5 py-6 text-text-main shadow-soft lg:px-8 lg:py-8"
          >
            {/* 背景の光 */}
            ...
            <div className="relative z-10">
              {/* ラベル行 + メタ */}
              <Reveal>
                <header className="mb-8 border-b border-[#222222]/10 pb-6">
                  <div className="mb-4 flex flex-wrap items-center gap-3 text-[10px] font-semibold tracking-[0.32em] text-tiffany-700">
                    ...
                    <span className="h-[1px] w-6 bg-[#222222]/12" />
                    <span className="text-[#222222]/55">
                      {mapCategoryLabel(item.category)}
                    </span>
                  </div>

                  <h1 className="serif-heading text-2xl font-semibold leading-relaxed tracking-tight text-[#222222] sm:text-3xl lg:text-[2.2rem]">
                    {item.title}
                  </h1>

                  <div className="mt-4 flex flex-wrap items-center gap-2 text-[10px] text-[#222222]/55">
                    {item.publishedAt && (
                      <span className="rounded-full bg-[#222222]/4 px-3 py-1">
                      ...
                      <>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {item.tags.slice(0, 4).map((tag) => (
                            <span
                              key={tag}
                              className="rounded-full bg-[#222222]/4 px-2 py-0.5 text-[9px]"
                            >
                              #{tag}
                            </span>
                          ))}
                          {item.tags.length > 4 && (
                            <span className="rounded-full bg-[#222222]/4 px-2 py-0.5 text-[9px] text-[#222222]/45">
                              +{item.tags.length - 4}
                            </span>
                          )}
                        </div>
              ...
              {heroImage && (
                <Reveal>
                  <div className="mb-8 overflow-hidden rounded-[1.75rem] border border-[#222222]/10">
                    <img
                      src={heroImage}
                      alt={item.title}
              ...
              <div className="mt-4">
                {blocks.map((block, index) => {
                  if (block.type === "heading") {
                    ...
                    const styleClass =
                      heading.level === 2
                        ? "mt-14 mb-6 font-serif text-xl font-medium text-[#222222] sm:text-[1.5rem]"
                        : "mt-9 mb-4 text-sm font-semibold tracking-[0.06em] text-[#222222]/85";
                  ...
                  if (block.type === "list") {
                    return (
                      <Reveal key={`list-${index}`} delay={80}>
                        <ul className="mt-4 space-y-1.5 text-sm leading-relaxed text-[#222222]/85 sm:text-[15px] sm:leading-[1.9rem]">
                  ...
                  if (!firstParagraphRendered && block.text.trim().length) {
                    ...
                    return (
                      <Reveal key={`p-${index}`} delay={160}>
                        <p className="first-letter-float mt-2 text-[13px] leading-8 text-[#222222]/85 sm:text-[15px] sm:leading-[2rem]">
                  ...
                  return (
                    <Reveal key={`p-${index}`} delay={60}>
                      <p className="mt-6 text-[13px] leading-8 text-[#222222]/85 sm:text-[15px] sm:leading-[2rem]">
              ...
              {/* 戻る導線（PCメイン） */}
              <div className="mt-16 border-t border-[#222222]/10 pt-8">
                <Link
                  href="/column"
                  className="group inline-flex items-center gap-2 text-xs font-medium tracking-[0.2em] text-[#222222]/55 hover:text-tiffany-600"
                >
                  <span className="flex h-8 w-8 items-center justify-center rounded-full border border-[#222222]/12 transition group-hover:border-tiffany-400 group-hover:bg-tiffany-50/60">
                    ←
                  </span>
                  BACK TO COLUMNS
                </Link>
        ...
        <aside className="hidden w-[28%] lg:block">
          <div className="sticky top-28 rounded-3xl border border-white/70 bg-white/80 p-6 text-[11px] text-[#222222]/65 shadow-soft backdrop-blur">
            <p className="mb-4 text-[10px] font-bold tracking-[0.25em] text-[#222222]/45">
              CONTENTS
            </p>
            ...
            {/* 小さなヒント */}
            <p className="mt-6 border-t border-[#222222]/10 pt-4 text-[10px] leading-relaxed text-[#222222]/45">
