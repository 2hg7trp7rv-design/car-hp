...
export async function GuideCardList({ slugs, fallbackCount = 4 }: Props) {
  ...
  if (items.length === 0) {
    return <p className="text-[11px] text-[var(--text-tertiary)]">関連するガイドは準備中です。</p>;
    }
      ...
      {items.map((guide, index) => (
        <Reveal key={guide.id ?? `${guide.slug}-${index}`} delay={80 + index * 30}>
          <Link href={`/guide/${encodeURIComponent(guide.slug)}`}>
            <GlassCard className="group h-full border border-[rgba(14,12,10,0.12)] bg-[var(--surface-1)] p-4 text-xs shadow-soft transition hover:-translate-y-[1px] hover:border-[rgba(27,63,229,0.32)] hover:shadow-soft-card sm:p-5">
              ...
              <p className="mt-3 text-[10px] font-semibold tracking-[0.18em] text-[var(--text-tertiary)]">
                READ →
              </p>
            </GlassCard>
          </Link>
