...
function buildBlocksForCar(car: CarItem): {
  ...
  const pushDetails = (summary: string, items: string[]) => {
    const it = items.map((s) => s.trim()).filter(Boolean);
    if (it.length === 0) return;
    blocks.push({ type: "details", summary, items: it });
  };
  ...
  if (weaknesses.length > 0) {
    ...
    const dup = weaknesses.filter((w) => checkPointSet.has(normalizeBullet(w)));
    if (unique.length > 0) pushUl(unique);
    if (dup.length > 0) {
      pushDetails("購入前チェックで触れた弱み（再掲）", dup);
    }
  }
  if (troubles.length > 0) {
    pushH3("トラブル傾向");
    const unique = troubles.filter((t) => !checkPointSet.has(normalizeBullet(t)));
    const dup = troubles.filter((t) => checkPointSet.has(normalizeBullet(t)));
    if (unique.length > 0) pushUl(unique);
    if (dup.length > 0) {
      pushDetails("購入前チェックで触れたトラブル傾向（再掲）", dup);
    }
  }
...
export default async function CarDetailPage({ params }: Props) {
  ...
  return (
    <main className="relative text-white">
      ...
      <div className="page-shell pb-24 pt-24">
        ...
        {/* 結論ファーストボックス — 向く人 / 向かない人 (B11) */}
        {(safeList(car.bestFor).length > 0 || safeList(car.notFor).length > 0) ? (
          <section className="mt-10">
            <div className="overflow-hidden rounded-3xl border border-[#0ABAB5]/30 bg-[#0ABAB5]/[0.07] backdrop-blur">
              <div className="px-6 py-5 sm:px-8 sm:py-6">
                <p className="text-[10px] font-semibold tracking-[0.3em] text-[#0ABAB5]/80">CONCLUSION</p>
                <h2 className="mt-1 text-[15px] font-semibold tracking-[0.04em] text-white">
                  この車が向く人 / 向かない人
                </h2>
              </div>
              <div className="mx-6 h-px bg-[#0ABAB5]/20" />
              <div className="grid gap-0 sm:grid-cols-2">
                {safeList(car.bestFor).length > 0 && (
                  <div className="px-6 py-5 sm:px-8 sm:py-6">
                    <p className="flex items-center gap-2 text-[10px] font-semibold tracking-[0.22em] text-[#0ABAB5]">
                      <span aria-hidden className="inline-block h-1.5 w-1.5 rounded-full bg-[#0ABAB5]" />
                      向く人
                    </p>
                    <ul className="mt-3 space-y-2">
                      {safeList(car.bestFor).map((b) => (
                        <li key={b} className="flex items-start gap-2 text-[13px] leading-relaxed text-white/80">
                          <span aria-hidden className="mt-[0.6em] h-1 w-1 shrink-0 rounded-full bg-[#0ABAB5]/60" />
                          {b}
                        </li>
                      ))}
                    </ul>
                ...
                {safeList(car.notFor).length > 0 && (
                  <div className="border-t border-[#0ABAB5]/15 px-6 py-5 sm:border-l sm:border-t-0 sm:px-8 sm:py-6">
                    <p className="flex items-center gap-2 text-[10px] font-semibold tracking-[0.22em] text-white/50">
                      <span aria-hidden className="inline-block h-1.5 w-1.5 rounded-full bg-white/30" />
                      向かない人
                    </p>
                    <ul className="mt-3 space-y-2">
                      {safeList(car.notFor).map((n) => (
                        <li key={n} className="flex items-start gap-2 text-[13px] leading-relaxed text-white/60">
                          <span aria-hidden className="mt-[0.6em] h-1 w-1 shrink-0 rounded-full bg-white/25" />
                          {n}
                        </li>
                      ))}
                    </ul>
                    ...
                    {sec.blocks.map((b, i) => {
                      ...
                      if (b.type === "details") {
                        return (
                          <details
                            key={i}
                            className="group cb-prose-block overflow-hidden rounded-2xl border border-white/15 bg-black/20 backdrop-blur"
                          >
                            <summary
                              className="cb-tap flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 text-[12px] text-white/85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0ABAB5]/50 [&::-webkit-details-marker]:hidden"
                            >
                              <span className="min-w-0 truncate">{b.summary}</span>
                              <span className="shrink-0 text-[10px] tracking-[0.22em] text-[#0ABAB5]">
  ...
  <div className="absolute right-3 top-3 z-20">
    <CompareAddButton slug={cc.slug} source="car_compare_section" mode="icon" className="h-9 w-9" />
        ...
        {/* PILLAR — 本文後ろ (B04: タイトル直下から移動) */}
        <section className="mt-12 border-t border-white/10 pt-10">
          <p className="text-[11px] tracking-[0.22em] text-white/45">関連ナビ</p>
          <div className="mt-4 flex flex-wrap gap-3">
            <Link
              href={parentPillarHref}
              className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-black/20 px-4 py-2 text-[11px] tracking-[0.22em] text-white/75 backdrop-blur hover:bg-white/10"
            >
              {parentPillarLabel}
        ...
        {/* 末尾CTA — 主CTA1本 + 副CTAに統一 (B04) */}
        <section className="mt-14 border-t border-white/10 pt-10">
          <p className="text-[11px] tracking-[0.22em] text-white/45">NEXT</p>
          <p className="mt-2 text-[13px] text-white/70">
            候補が絞れたら、比較で並べると判断が早くなります。
          </p>
          <div className="mt-5 flex flex-wrap gap-3">
            <Link
              href="/compare"
              className="inline-flex items-center gap-2 rounded-full bg-[#0ABAB5] px-6 py-3 text-[12px] font-semibold tracking-[0.18em] text-[#042624] transition hover:opacity-90"
            >
              COMPARE で比較する <span aria-hidden>→</span>
            </Link>
            <Link
              href={backToCarsHref}
              className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-black/20 px-5 py-3 text-[12px] tracking-[0.18em] text-white/75 backdrop-blur hover:bg-white/10"
            >
              一覧に戻る
            </Link>
          </div>
