...
const hubLinks = [
  ...
  },
  {
    ...
    subHubs: [
      { href: "/guide/hub-sell-price",   title: "売却相場HUB" },
      { href: "/guide/hub-sell-prepare", title: "査定準備HUB" },
      { href: "/guide/hub-sell-compare", title: "比較の使い方HUB" },
      { href: "/guide/hub-sell-loan",    title: "残債・名義HUB" },
    ],
  },
  {
    ...
    <main className="relative min-h-screen text-white">
      ...
      <div className="page-shell pb-24 pt-24">
        ...
        <div className="flex items-center justify-between">
          <Breadcrumb tone="light" items={[{ label: "HOME", href: "/" }, { label: "GUIDE" }]} />
        ...
        <ArchiveEntrance
          ...
          lead="選ぶための知識ではなく、選び方の順番を残す。"
          active="guide"
        />


        <div className="porcelain porcelain-panel mx-auto mt-10 max-w-3xl overflow-hidden rounded-3xl border border-[#222222]/10 bg-white/92 text-[#222222] shadow-soft-card backdrop-blur">
          <div className="p-6 sm:p-8">

        {/* pillar hubs — 最上段（B05: HUBを先に置く） */}
            <section aria-label="目的別HUB">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {hubLinks.map((hub) => (
              <div key={hub.href} className="flex flex-col gap-1.5">
                <Link
                  href={hub.href}
                  className="cb-tap rounded-2xl border border-[#222222]/12 bg-white/60 p-4 shadow-sm transition hover:bg-white hover:border-[#0ABAB5]/35"
                >
                  <p className="text-[13px] font-semibold tracking-wide text-[#222222]">
                    {hub.title}
                  </p>
                  <p className="mt-1 text-[11px] leading-relaxed text-[#222222]/65">
                    {hub.desc}
                  </p>
                </Link>
                {(hub as any).subHubs && (
                  <div className="flex flex-wrap gap-1.5 pl-2">
                    {((hub as any).subHubs as { href: string; title: string }[]).map((sub) => (
                      <Link
                        key={sub.href}
                        href={sub.href}
                        className="cb-tap inline-flex items-center gap-1 rounded-full border border-[#222222]/10 bg-white/60 px-3 py-1 text-[10px] font-medium tracking-[0.14em] text-[#222222]/65 transition hover:border-[#0ABAB5]/35 hover:text-[#0ABAB5]"
                      >
                        <span aria-hidden className="text-[#0ABAB5]/50">↳</span>
                        {sub.title}
                      </Link>
                    ))}
                  </div>
        ...
        {/* search & chips */}
            <section className="mt-10" aria-label="すべてのガイドを見る">
              ...
              {categoryChips.map((c) => {
                ...
                return (
                  <Link
                    ...
                    className={[
                      "cb-tap inline-flex items-center rounded-full px-3 py-2 text-[10px] font-semibold tracking-[0.18em] transition",
                      selected
              ...
              {Array.from({ length: Math.min(totalPages, 7) }).map((_, idx) => {
                ...
                return (
                  <Link
                    ...
                    className={[
                      "cb-tap inline-flex h-8 w-8 items-center justify-center rounded-full transition",
                      active
        ...
        {/* list */}
            <section className="mt-10" aria-label="ガイド一覧">
            ...
            <div>
              {currentPage > 1 ? (
                <Link
                  ...
                  className="cb-tap rounded-full border border-[#222222]/10 bg-white px-4 py-2 text-[10px] font-semibold tracking-[0.18em] text-[#222222] shadow-soft hover:border-[#0ABAB5]/35"
                >
                  PREV
                </Link>
              ) : null}
          ...
          <div className="mt-8 flex justify-center">
            <Link
              href="/guide"
              className="cb-tap inline-flex w-full max-w-md items-center justify-center rounded-full bg-gradient-to-r from-[#0ABAB5] to-[#2F7F86] px-7 py-4 text-[11px] font-semibold tracking-[0.2em] text-white shadow-soft-card transition hover:opacity-95"
            >
              すべてのガイドを回る
            </Link>
          </div>
          ...
          <div className="relative overflow-hidden rounded-3xl border border-[#222222]/10 bg-[#0B1220] shadow-soft-card">
            ...
            <div className="relative p-6 sm:p-8">
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                <Link
                  href="/cars"
                  className="cb-tap inline-flex items-center rounded-full bg-white px-5 py-3 text-[10px] font-semibold tracking-[0.18em] text-[#222222] shadow-soft"
                >
                  CAR DATABASE
                </Link>
                <Link
                  href="/column"
                  className="cb-tap inline-flex items-center rounded-full bg-[#0ABAB5] px-5 py-3 text-[10px] font-semibold tracking-[0.18em] text-white shadow-soft"
                >
                  READ COLUMN
                </Link>
              </div>
