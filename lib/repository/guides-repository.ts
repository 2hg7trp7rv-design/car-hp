...
function coerceGuideDetailBlocks(value: unknown): GuideDetailBlock[] {
  ...
  for (const item of value) {
    ...
    if (type === "paragraph") {
      ...
      if (!text) continue;
      out.push({ type: "paragraph", text, flow, highlights: highlights.length > 0 ? highlights : null });
      continue;
    }
    ...
    if (type === "image") {
      ...
      out.push({
        ...
        fit: coerceString(rec.fit) === "contain" ? "contain" : "cover",
    ...
    if (type === "subheading") {
      ...
      if (!title) continue;
      out.push({ type: "subheading", title, level });
      continue;
    }
    ...
    if (type === "quote") {
      const text = coerceString(rec.text);
      if (!text) continue;
      out.push({ type: "quote", text, caption: coerceString(rec.caption) });
      continue;
    }
    ...
    if (type === "comparisonTable") {
      ...
      out.push({
        ...
        rows,
        note: coerceString(rec.note),
        display: coerceString(rec.display) === "table" ? "table" : coerceString(rec.display) === "cards" ? "cards" : null,
      });
      continue;
    }
