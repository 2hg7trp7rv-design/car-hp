...
function JunaBubble({ data }: { data: JunaBubbleData }) { return <div className={styles.junaBubble}><img src={CBJ_CHARACTERS.shuna.question} alt="シュナ" className={styles.junaAvatar} loading="lazy" decoding="async" onError={(event) => { event.currentTarget.src = CBJ_CHARACTERS.shuna.understood; }} /><div className={styles.junaBubbleContent}><div className={styles.junaBubbleTriangle} /><div className={styles.junaBubbleCard}>{data.badge ? <span className={styles.junaBadge}>{data.badge}</span> : null}{data.name ? <div className={styles.junaName}>{data.name}</div> : null}<p className={styles.junaText}><RichText value={data.text} /></p></div></div></div>; }
