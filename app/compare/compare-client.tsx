...
export function CompareClient({ cars }: { cars: CompareCarIndexItem[] }) {
  ...
  const selectedCars = useMemo(
    () => slugs.map((s) => carBySlug.get(s)).filter(Boolean) as CompareCarIndexItem[],
    [slugs, carBySlug],
  );
    ...
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="text-[11px] font-semibold tracking-[0.18em] text-slate-700">
            SELECTED
        ...
        {selectedCars.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-200 bg-white/70 p-5 text-[12px] text-slate-600">
            まだ車種が選ばれていません。
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {selectedCars.map((car) => {
              const hero = car.heroImage && car.heroImage.trim().length > 0 ? car.heroImage : "/images/cars/placeholder.jpg";
              return (
                <div
                  key={car.slug}
                  className="relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white/80 shadow-soft"
                >
            ...
            {suggestGroups.map((g) => (
              <ShelfImpression key={g.id} shelfId={`compare_suggest_${g.id}`}>
                <div className="rounded-2xl border border-slate-200/70 bg-white/70 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="text-[11px] font-semibold tracking-[0.16em] text-slate-700">
                      {g.label}
                    </div>
                    <div className="text-[10px] text-slate-500">クリックで比較に追加</div>
                  </div>
      ...
      <section className="space-y-3">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="text-[11px] font-semibold tracking-[0.18em] text-slate-700">ADD</div>
            <p className="mt-1 text-[11px] text-slate-600">
          ...
          <input
            ...
            className="w-full rounded-full border border-slate-200 bg-white/90 px-4 py-2 text-[12px] text-slate-800 shadow-soft focus:border-tiffany-300 focus:outline-none sm:max-w-sm"
          />
        </div>
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {candidates.map((car) => (
            <div
              key={car.slug}
              className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200/70 bg-white/70 px-4 py-3"
            >
              <div>
                <div className="text-[10px] font-semibold tracking-[0.22em] text-tiffany-700">
                  {car.maker}
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <div className="text-[11px] font-semibold tracking-[0.18em] text-slate-700">TABLE</div>
          {selectedCars.length < 2 && (
            <div className="text-[11px] text-slate-500">2台以上を選ぶと差分が見えます</div>
          )}
        </div>
        ...
        <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white/80">
          <table className="min-w-[720px] w-full text-[11px]">
            <thead className="bg-slate-50/70 text-[10px] font-semibold tracking-[0.16em] text-slate-500">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100">
              {rows.map((row) => (
                <tr key={row.key} className="align-top">
                  <td className="bg-white/50 px-4 py-3 font-semibold text-slate-700">
                    {row.label}
                  </td>
                  {selectedCars.map((car) => (
                    <td key={car.slug} className="px-4 py-3 text-slate-700">
