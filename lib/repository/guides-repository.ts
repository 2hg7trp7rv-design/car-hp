...
type RawGuideRecord = {
  ...
  /**
   * 任意: JSON-LD / カード表示などの説明文。
   * 未入力の場合は seoDescription / summary などから補完する。
   */
...
function normalizeGuide(raw: RawGuideRecord, index: number): GuideItem {
  ...
  // JSON-LD 等で参照する“汎用 description”。データに無ければ seoDescription で埋める。
  // （seoDescription が無い期間でも最低限の説明が出るよう、タイトルからフォールバックを作る）
  const description =
    coerceString(raw.description) ??
    seoDescription ??
    summary ??
    lead ??
    (title ? `${title}の結論・手順・注意点を、迷わない順番で整理します。` : null);
