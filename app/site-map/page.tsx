...
export default async function SiteMapPage() {
  ...
  const entryLinks: LinkItem[] = [
    { href: "/", label: "HOME", description: "全体の入口" },
    { href: "/start", label: "START", description: "目的別の入口" },
  ];

  const archiveLinks: LinkItem[] = [
    { href: "/cars", label: "CARS", description: "車種から比較候補を絞る" },
    { href: "/guide", label: "GUIDE", description: "手順を進める" },
    { href: "/column", label: "COLUMN", description: "判断材料を短く読む" },
    { href: "/heritage", label: "HERITAGE", description: "歴史の文脈から読む" },
  ];

  const toolLinks: LinkItem[] = [
    { href: "/news", label: "NEWS", description: "公式発表・一次情報を追う" },
    { href: "/compare", label: "COMPARE", description: "候補を並べて違いを確認する" },
    { href: "/search", label: "SEARCH", description: "サイト内を横断して探す" },
  ];
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          <Reveal delay={40}>
            <GlassCard className="border border-[#222222]/10 bg-white/80 p-6" padding="none">
              <h2 className="font-serif text-[15px] font-semibold text-[#222222]">入口</h2>
              <ul className="mt-4 space-y-3 text-[12px] text-[#222222]/80">
                {entryLinks.map((link) => (
                  <li key={link.href} className="flex flex-col gap-1">
                    ...
                    {link.description ? <span className="text-[11px] text-[#222222]/55">{link.description}</span> : null}
                  </li>
                ))}
              </ul>
            </GlassCard>
          ...
          <Reveal delay={120}>
            <GlassCard id="tools" className="border border-[#222222]/10 bg-white/80 p-6" padding="none">
              <h2 className="font-serif text-[15px] font-semibold text-[#222222]">TOOLS</h2>
              <p className="mt-2 text-[12px] leading-relaxed text-[#222222]/65">
                主要カテゴリから少し離れて、目的が固まった時に使う道具です。
              </p>
              <ul className="mt-4 space-y-3 text-[12px] text-[#222222]/80">
                {toolLinks.map((link) => (
                  <li key={link.href} className="flex flex-col gap-1">
