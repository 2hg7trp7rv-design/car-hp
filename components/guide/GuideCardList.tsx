      ...
      {items.map((guide, index) => (
        <Reveal key={guide.id ?? `${guide.slug}-${index}`} delay={80 + index * 30}>
          <Link href={`/guide/${encodeURIComponent(guide.slug)}`}>
            <GlassCard className="group h-full border border-[rgba(31,28,25,0.12)] bg-[var(--surface-1)] p-4 text-xs shadow-soft transition hover:-translate-y-[1px] hover:border-accent-100 hover:shadow-soft-card sm:p-5">
