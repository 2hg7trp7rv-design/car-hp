        ...
        {takeawayItems.length > 0 ? (
          <section className="mt-8" aria-label="おすすめポイント">
            <div className="overflow-hidden rounded-3xl border border-white/15 bg-black/20 p-6 backdrop-blur ">
              ...
              <ul className="mt-4 space-y-3 text-[15px] leading-[1.95] tracking-[0.08em] text-white/85">
                {takeawayItems.map((t, idx) => (
                  <li key={idx} className="flex gap-3">
                    <span className="mt-[6px] inline-flex h-5 w-5 items-center justify-center rounded-full bg-white text-[#0ABAB5] ">
                      <span className="text-[14px] font-bold">+</span>
                    </span>
                    <span className="flex-1">{renderRichText(t, guideTitleBySlug)}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            {blocks.map((b, idx) => {
              if (b.type === "heading") {
                if (b.heading.level === 2) {
                  ...
                  return (
                    <div
                      ...
                      id={b.heading.id}
                      className={`scroll-mt-28 pt-2 ${step === 1 ? "" : "mt-16"}`}
                    >
                      <div className="flex items-baseline gap-3">
                        <span className="text-[20px] font-semibold text-[#0ABAB5]">
                          {step}.
                        </span>
                        <h2 className="serif-heading text-[20px] leading-relaxed text-white sm:text-[22px]">
                          {title}
                        </h2>
                      </div>
                      ...
                      {checkItems.length > 0 && step === renderCheckPointAfterStep ? (
                        <div className="mt-6 overflow-hidden rounded-3xl border border-white/15 bg-black/20 p-6 backdrop-blur ">
                          <p className="text-[13px] font-semibold tracking-[0.16em] text-white/85">
                            — 事前に CHECK POINT
                          </p>
                          <ul className="mt-4 space-y-2 text-[15px] leading-[1.95] tracking-[0.08em] text-white/85">
                            {checkItems.map((t, i) => (
                ...
                return (
                  <h3
                    key={`${b.heading.id}-${idx}`}
                    id={b.heading.id}
                    className="serif-heading scroll-mt-28 pt-2 text-[17px] font-semibold leading-relaxed text-white"
                  >
                    {b.heading.text}
                  </h3>
                );
              ...
              if (b.type === "paragraph") {
                return (
                  <p
                    key={idx}
                    className="text-[15px] leading-[1.95] tracking-[0.08em] text-white/85"
                  >
                    {renderRichText(b.text, guideTitleBySlug)}
                  </p>
                );
              }
                ...
                return (
                  <div key={idx} className="my-16 space-y-6">
                    {lines.map((ln, j) => (
                      <p
                        key={`${idx}-${j}`}
                        className="serif-heading text-[24px] font-semibold leading-[1.55] tracking-[0.08em] text-[#0ABAB5]"
                        style={{ textShadow: "0 0 22px rgba(10,186,181,0.22)" }}
                      >
                        {renderRichText(ln, guideTitleBySlug)}
                      </p>
                    ))}
                  </div>
                );
              ...
              if (b.type === "list") {
                return (
                  <ul key={idx} className="space-y-2 text-[15px] leading-[1.95] tracking-[0.08em] text-white/85">
                    {b.items.map((t, i) => (
                      <li key={i} className="flex gap-3">
                        <span className="mt-[7px] h-1.5 w-1.5 rounded-full bg-white/35" />
                        <span className="flex-1">{renderRichText(t, guideTitleBySlug)}</span>
