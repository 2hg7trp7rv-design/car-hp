...
function fitFeatureTitleStyle(text?: string | null, scale = 1): CSSProperties {
  const count = Math.max(4, Array.from(String(text ?? "").replace(/\s+/g, "").trim()).length);
  const vw = Math.min(6.8, Math.max(2.05, (72 * scale) / count));
    ...
    return (
      <section key={`callout-${index}`} className="detail-card-muted p-5 sm:p-6">
        ...
        <h3 className="mt-2 text-[19px] font-semibold tracking-[-0.03em] text-[var(--text-primary)]">
          {stripLeadingDisplayNumber(block.title)}
        </h3>
        <div className="mt-3">
          <TextWithInternalLinkCards
            text={block.body}
        ...
        {block.items.map((card, cardIndex) => (
          ...
          >
            <h3 className="text-[16px] font-semibold leading-6 tracking-[-0.02em] text-[var(--text-primary)]">
              {stripLeadingDisplayNumber(card.title)}
            </h3>
            {card.body ? (
              <div className="mt-3">
                <TextWithInternalLinkCards
          ...
          >
            ...
            <div>
              <h3 className="text-[16px] font-semibold tracking-[-0.02em] text-[var(--text-primary)]">
                {stripLeadingDisplayNumber(step.title)}
              </h3>
              <div className="mt-2">
                <TextWithInternalLinkCards
                  text={step.body}
      ...
      <div className="detail-shell pb-24 pt-24 sm:pt-28">
        ...
        <section className="mt-6 grid gap-8 lg:grid-cols-[minmax(0,1.08fr)_minmax(320px,0.92fr)] lg:items-end">
          ...
          <div className="order-2">
            ...
            <h1 className="page-title mt-4 cbj-fit-title" style={fitFeatureTitleStyle(title)}>{title}</h1>
            ...
            >
              <div className="cb-stage-chapterTop">
                <div className="flex items-baseline gap-3 px-6 py-5 sm:px-8">
                  ...
                  <h2 className="cb-stage-chapterTitle cbj-fit-title" style={fitFeatureTitleStyle(section.title, 0.78)}>{section.title}</h2>
                </div>
