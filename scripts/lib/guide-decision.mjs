export const DECISION_GUIDE_MIN = {
  bodyLen: 2500,
  headings: 4,
  keyPoints: 3,
  checkpoints: 3,
...
export function countMarkdownHeadings(body) {
...
export function isDecisionGuide(guide) {
  const variant = asLower(guide?.layoutVariant);
  return variant === "decision-v1" || variant === "guide-board-v2";
}

export function isDecisionColumn(column) {
  const variant = asLower(column?.layoutVariant);
  return variant === "decision-v1" || variant === "column-decision-v2";
}

export function isStructuredGuide(guide) {
  const variant = asLower(guide?.layoutVariant);
  return variant === "decision-v1" || variant === "guide-board-v2" || variant === "cbj-world-v1";
}

export function isStructuredColumn(column) {
  const variant = asLower(column?.layoutVariant);
  return variant === "decision-v1" || variant === "column-decision-v2" || variant === "cbj-world-v1";
}
...
export function getDecisionGuideStats(guide) {
  return getDecisionStats(guide);
}

export function getDecisionColumnStats(column) {
...
function blockText(block) {
  ...
  switch (block.type) {
    ...
    case "subheading":
      return safeString(block.title) ? [safeString(block.title)] : [];
    case "comparisonTable": {
      ...
      const rows = Array.isArray(block.rows)
        ? block.rows
            ...
            .map((row) => row.map((cell) => safeString(cell)).filter(Boolean))
    ...
    case "editorialBoard": {
      const lines = [];
      const title = safeString(block.title);
      const eyebrow = safeString(block.eyebrow);
      const lead = safeString(block.lead);
      const note = safeString(block.note);
      const variant = safeString(block.variant);
      const accent = safeString(block.accent);
      ...
      const items = Array.isArray(block.items) ? block.items : [];
      items.forEach((item) => {
        const number = safeString(item?.number);
        const badge = safeString(item?.badge);
...
function buildDecisionBody(item) {
  const chunks = [];
...
export function buildDecisionGuideBody(guide) {
  return buildDecisionBody(guide);
}

export function buildDecisionColumnBody(column) {
  return buildDecisionBody(column);
}

export function getDecisionGuideAuditBody(guide) {
  const rawBody = safeString(guide?.body);
  if (rawBody) return rawBody;
  return buildDecisionGuideBody(guide);
}

export function getDecisionColumnAuditBody(column) {
  const rawBody = safeString(column?.body);
  if (rawBody) return rawBody;
  return buildDecisionColumnBody(column);
}

export function getStructuredGuideAuditBody(guide) {
  const rawBody = safeString(guide?.body);
  if (rawBody) return rawBody;
  return buildDecisionGuideBody(guide);
}

export function getStructuredColumnAuditBody(column) {
  const rawBody = safeString(column?.body);
  if (rawBody) return rawBody;
  return buildDecisionColumnBody(column);
}
