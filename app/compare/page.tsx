        ...
        <div className="porcelain porcelain-panel rounded-3xl border border-[#222222]/10 bg-white/92 text-[#222222] shadow-soft-card backdrop-blur p-6 sm:p-8">
          ...
          <Reveal delay={80}>
            <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-3">
              {[
                { step: "01", title: "車種を選ぶ", desc: "CARS一覧や下のリストから比較したい車種を追加する（最大3台）" },
                { step: "02", title: "比較表を確認", desc: "価格帯・維持費・ボディタイプなどを横並びで見比べる" },
                { step: "03", title: "次の一手へ", desc: "詳細ページへ入るか、GUIDEで購入手順を確認する" },
              ].map((s) => (
                <div key={s.step} className="rounded-2xl border border-[#222222]/10 bg-white/60 px-5 py-4">
                  <p className="text-[10px] font-medium tracking-[0.3em] text-[#0ABAB5]">STEP {s.step}</p>
                  <p className="mt-1.5 text-[13px] font-semibold text-[#222222]">{s.title}</p>
                  <p className="mt-1 text-[12px] leading-relaxed text-[#222222]/60">{s.desc}</p>
                </div>
              ))}
            </div>
          </Reveal>
                ...
                <noscript>
                  <div className="mt-4 rounded-2xl border border-dashed border-[#222222]/12 bg-white/70 p-5 text-[12px] text-[#222222]/65">
                    ...
                    <div className="mt-2 text-[11px] text-[#222222]/55">
                      迷ったら <a className="underline" href="/start">START — 目的別の入口</a> から候補を整理してください。
                    </div>
                  </div>
                </noscript>
              </GlassCard>
