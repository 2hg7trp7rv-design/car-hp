...
export function renderInlineMarkdown(text: string, opts?: { tone?: Tone }): ReactNode {
  const tone: Tone = opts?.tone ?? "dark";
    ...
    <>
      {tokens.map((t, idx) => {
        ...
        if (t.type === "bold") {
          ...
          return (
            <strong key={idx} className={tone === "light" ? "font-semibold text-[#111]" : "font-semibold text-white"}>
              {renderInlineMarkdown(t.value, { tone })}
            </strong>
          );
        }
        ...
        if (t.type === "tooltip") {
          return (
            ...
            >
              {renderInlineMarkdown(t.term, { tone })}
            </span>
          );
        }
        ...
        if (href.startsWith("/")) {
          // In light tone, internal links should look like links (ideal template).
          if (tone === "light") {
            return (
              <a key={idx} href={href} className="text-[#0066cc] hover:underline">
                {label}
              </a>
            );
          }

          // In dark tone, internal links are rendered as plain text here,
          // because internal navigation is rendered as cards below.
          ...
          <a
            ...
            className={
              tone === "light"
                ...
                : "underline decoration-white/35 underline-offset-4 hover:decoration-[#0ABAB5]"
            }
          >
            {label}
          </a>
