...
type Props = {
  ...
  type?: "insurance" | "buy" | "sell" | "parts"; // アイコンや色味の切り替え用
};

export function AffiliateCtaBlock({ title, description, buttonLabel, href, type = "buy" }: Props) {
  // タイプ別のアクセントカラー定義
  const accents = {
    ...
    parts: "from-slate-50 to-gray-100 border-slate-200 text-slate-900",
  };

  const btnVariants = {
    insurance: "bg-blue-600 hover:bg-blue-500",
    buy: "bg-emerald-600 hover:bg-emerald-500",
    sell: "bg-amber-600 hover:bg-amber-500",
    parts: "bg-slate-800 hover:bg-slate-700",
  };

  return (
    <div className="my-10 not-prose">
      <GlassCard className={`relative overflow-hidden p-6 md:p-8 bg-gradient-to-br ${accents[type]} shadow-soft-card`}>
        {/* 背景の装飾（キラキラ） */}
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-6">
          ...
          <div className="flex-shrink-0">
            <Button 
              asChild 
              magnetic 
              className={`rounded-full px-8 py-6 text-sm font-bold tracking-wider shadow-lg text-white transition-transform hover:-translate-y-1 ${btnVariants[type]}`}
            >
