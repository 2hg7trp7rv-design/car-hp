...
export default async function ColumnDetailPage({ params }: Props) {
  ...
  const related = pickRelatedColumns(item, all, 3);
  const linkIndex = await getInternalLinkIndex();

  const relatedMain = related.slice(0, 2);
  const relatedMini = related.slice(2, 3);
  ...
  const columnUrl = `${getSiteUrl()}/column/${encodeURIComponent(item.slug)}`;
  const columnImageUrl = safeHeroImage
  ...
  const articleJsonLd = {
    ...
    "@type": "Article",
    headline: title,
    description: leadText || item.summary || item.subtitle || "",
    url: columnUrl,
    mainEntityOfPage: { "@type": "WebPage", "@id": columnUrl },
    image: columnImageUrl ? [columnImageUrl] : undefined,
  ...
  return (
    <main className="detail-page">
      <DetailFixedBackground seed={slug} imageSrc={(item.heroImage ?? (item as any).ogImageUrl ?? null) as string | null} />
        ...
        <section className="mt-6 grid gap-8 lg:grid-cols-[minmax(0,1.08fr)_minmax(320px,0.92fr)] lg:items-end">
          ...
          <div className="order-2">
            <div className="flex flex-wrap items-center gap-2">
              {badge ? <span className="detail-chip detail-chip-clay">{badge}</span> : null}
              ...
              <ul className="mt-5 space-y-4">
                {takeaways.map((t) => (
                  <li key={t} className="flex items-start gap-3">
                    <span className="mt-[0.72em] h-[6px] w-[6px] shrink-0 rounded-full bg-[var(--accent-clay)]" />
            ...
            >
              ...
              <div className="px-6 py-6 sm:px-8 sm:py-7">
                {checkPoints.length > 0 && secIndex === checkPointSectionIndex ? (
                  <div className="detail-card-slate mb-8 p-6">
                    ...
                    <ul className="mt-4 space-y-3">
                      {checkPoints.map((it) => (
                        <li key={it} className="flex items-start gap-3">
                          <span className="mt-[0.75em] h-[6px] w-[6px] shrink-0 rounded-full bg-[var(--accent-slate)]" />
                          <TextWithInternalLinkCards
                            text={it}
                            linkIndex={linkIndex}
                            as="span"
                ...
                <div className="space-y-8 cb-prose">
                  {section.blocks.map((b, idx) => {
                    ...
                    if (b.type === "ul") {
                      return (
                        <ul key={idx} className="cb-prose-block space-y-4">
                          {b.items.map((it) => (
                            <li key={it} className="flex items-start gap-3">
                              <span className="mt-[0.75em] h-[6px] w-[6px] shrink-0 rounded-full bg-[var(--accent-clay)]" />
                              <TextWithInternalLinkCards
                                text={it}
                                linkIndex={linkIndex}
                                as="span"
                    ...
                    if (b.type === "hr") {
                      return <div key={idx} className="h-px w-full bg-[rgba(31,28,25,0.08)]" />;
                    }
                        ...
                        <li key={i}>
                          {isUrl ? (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="detail-link underline decoration-[rgba(122,135,108,0.35)] underline-offset-4"
                            >
                              {trimmed}
                            </a>
                          ) : (
