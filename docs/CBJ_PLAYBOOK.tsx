 ...
 * - コンテンツ量産の前に「勝てる型」と「index/noindexの基準」を固定する
 */
...
## 1. 大原則（これを破ると伸びない）
1) **公開済み記事は原則 index**（noindex は内部・検索結果・下書き・redirect元に限定）
...
1) **/sitemap.xml が 4xx にならないこと**
   ...
   - 完了条件: Search ConsoleでURL検査→検索結果/絞り込みURLのみインデックス対象外になる

3) 公開済み記事の index 固定
   - 変更: /cars/[slug] / /guide/[slug] / /column/[slug] / /heritage/[slug] は、draft/redirect/明示noindex以外を index
   - 完了条件: sitemap掲載記事が noindex を返さない
...
## 5. 運用ルール（迷ったらここに戻る）
- 量産より “勝てる型”
- 公開済み記事は index。noindex は内部ページ・検索結果・下書き・redirect元だけに限定する
- 新規記事: 週5より、週2でもS+を守る
...
export const CBJ_FIX_BACKLOG: FixItem[] = [
  ...
  {
    priority: "P0",
    target: "/search, 絞り込みURL",
    change: "noindex,follow（重複URL対策）",
    done: "URL検査で検索結果/絞り込みURLだけ noindex が反映",
  },
  {
    priority: "P0",
    target: "/cars/[slug]",
    change: "公開済み車種ページは原則 index。完成基準未満は公開前に本文を補強",
    done: "sitemap掲載車種が noindex を返さない",
  },
  {
    priority: "P1",
    target: "/guide",
