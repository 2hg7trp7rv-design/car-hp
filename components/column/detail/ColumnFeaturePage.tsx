    ...
    <div className="mt-5 grid gap-3 sm:grid-cols-2">
      {items.map((entry) => (
        <Link
          key={`${entry.href}-${entry.title}`}
          href={entry.href}
          className="rounded-[20px] border border-[rgba(14,12,10,0.08)] bg-[rgba(255,255,255,0.78)] p-4 transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_16px_40px_rgba(20,18,16,0.08)]"
        >
      ...
      <div key={`cards-${index}`} className="grid gap-3 sm:grid-cols-2">
        {block.items.map((card, cardIndex) => (
          <section
            ...
            className="rounded-[20px] border border-[rgba(14,12,10,0.08)] bg-[rgba(255,255,255,0.72)] p-5"
          >
      ...
      <div key={`steps-${index}`} className="space-y-4">
        {block.items.map((step, stepIndex) => (
          <div
            ...
            className="grid grid-cols-[40px_minmax(0,1fr)] gap-4 rounded-[20px] border border-[rgba(14,12,10,0.08)] bg-[rgba(255,255,255,0.74)] p-4"
          >
  ...
  return (
    <main className="detail-page">
      ...
      <div className="detail-shell pb-24 pt-24 sm:pt-28">
          ...
          <div className="order-1">
            <div className="detail-photo-frame relative aspect-[16/10] w-full">
              {safeHeroImage ? (
                ...
                <div className="flex h-full items-center justify-center bg-[rgba(14,12,10,0.06)] text-[13px] text-[var(--text-secondary)]">
                  視点
                </div>
              )}
            </div>
        ...
        {feature.actionBox ? (
          <section className="mt-10 detail-card-fog p-6 sm:p-8">
                  ...
                  {sourceList.map((entry, index) => (
                    <li key={`source-${index}`} className="text-[14px] leading-7 text-[var(--text-secondary)] break-words">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="underline decoration-[rgba(14,12,10,0.22)] underline-offset-4 hover:text-[var(--text-primary)]"
                      >
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {curatedRelated.map((entry) => (
                  <Link
                    key={`${entry.href}-${entry.title}`}
                    href={entry.href}
                    className="rounded-[20px] border border-[rgba(14,12,10,0.08)] bg-[rgba(255,255,255,0.78)] p-5 transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_16px_40px_rgba(20,18,16,0.08)]"
                  >
                    <p className="text-[11px] font-semibold tracking-[0.18em] text-[var(--text-tertiary)]">
