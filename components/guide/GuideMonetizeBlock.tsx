...
export function GuideMonetizeBlock(props: GuideMonetizeBlockProps) {
  ...
  // 既定は pageContext から推定（hub/guide に対応）
  const ctx = usePageContext();
  const page_type = (pageType ?? (ctx.page_type === "hub" ? "hub" : "guide")) as Exclude<
    PageType,
    "top"
  >;
  const content_id = (contentId ?? ctx.content_id) as string;

  // unknown / index / top を送らない（データ汚染回避）
  const canTrack =
    (page_type === "guide" || page_type === "hub") &&
    !!content_id &&
    content_id !== "unknown" &&
    content_id !== "index" &&
    content_id !== "top";
