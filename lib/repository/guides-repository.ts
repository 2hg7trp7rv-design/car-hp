...
function coerceBlockPresentation(value: unknown): GuideDetailBlock["presentation"] {
  if (!value || typeof value !== "object") return null;
  const rec = value as Record<string, unknown>;
  const variant = coerceString(rec.variant);
  const widthRaw = coerceString(rec.width);
  const width = widthRaw === "normal" || widthRaw === "wide" || widthRaw === "bleed" ? widthRaw : null;
  ...
  const motion =
    ...
    motionRaw === "scale-in"
      ? motionRaw
      : null;
  if (!variant && !width && !motion) return null;
  return { variant, width, motion };
}
...
function coerceGuideEditorialBoardItems(value: unknown): GuideEditorialBoardItem[] {
  if (!Array.isArray(value)) return [];
  const out: GuideEditorialBoardItem[] = [];
  for (const item of value) {
    if (!item || typeof item !== "object") continue;
    const rec = item as Record<string, unknown>;
    const title = coerceString(rec.title);
    ...
    out.push({
      title,
      badge: coerceString(rec.badge),
      number: coerceString(rec.number),
      body: coerceString(rec.body),
      items: coerceStringArray(rec.items),
    });
  }
...
function coerceGuideDetailBlocks(value: unknown): GuideDetailBlock[] {
  ...
  for (const item of value) {
    ...
    if (type === "paragraph") {
      ...
      const flow = flowRaw === "natural" || flowRaw === "sentence" ? flowRaw : null;
      const highlights = coerceStringArray(rec.highlights);
      if (!text) continue;
      pushBlock({ type: "paragraph", text, flow, highlights: highlights.length > 0 ? highlights : null });
      continue;
    }

    if (type === "image") {
      const src = coerceString(rec.src);
      if (!src) continue;
      const fitRaw = coerceString(rec.fit);
      const fit =
        fitRaw === "contain" || fitRaw === "bleed" || fitRaw === "articleWide" || fitRaw === "cover"
          ? fitRaw
          : "cover";
      pushBlock({
        type: "image",
        ...
        fit,
        aspectRatio: coerceString(rec.aspectRatio),
        width: coercePositiveInteger(rec.width),
        height: coercePositiveInteger(rec.height),
      });
      continue;
    }
    ...
    if (type === "subheading") {
      const title = coerceString(rec.title);
      const levelRaw = rec.level;
      const level = levelRaw === 4 ? 4 : 3;
      if (!title) continue;
      pushBlock({ type: "subheading", title, level });
      continue;
    }
    ...
    if (type === "quote") {
      const text = coerceString(rec.text);
      if (!text) continue;
      pushBlock({ type: "quote", text, caption: coerceString(rec.caption) });
      continue;
    }
    ...
    if (type === "comparisonTable") {
      ...
      pushBlock({
        ...
        note: coerceString(rec.note),
        display: (() => {
          const display = coerceString(rec.display);
          return display === "table" || display === "cards" || display === "contrast" || display === "checklist"
            ? display
            : null;
        })(),
      });
    ...
    if (type === "editorialBoard") {
      const items = coerceGuideEditorialBoardItems(rec.items);
      if (items.length === 0) continue;
      const variantRaw = coerceString(rec.variant);
      const variant = variantRaw === "white" || variantRaw === "dark" || variantRaw === "smoke" ? variantRaw : null;
      pushBlock({
        type: "editorialBoard",
        title: coerceString(rec.title),
        eyebrow: coerceString(rec.eyebrow),
        lead: coerceString(rec.lead),
        note: coerceString(rec.note),
        variant,
        accent: coerceString(rec.accent),
        items,
      });
      continue;
    }
...
function coerceGuideDetailSections(value: unknown): GuideDetailSection[] | null {
  ...
  for (const item of value) {
    ...
    out.push({
      ...
      title,
      displayTitle: coerceString(rec.displayTitle),
      chapterLabel: coerceString(rec.chapterLabel),
      deck: coerceString(rec.deck),
...
function blockText(block: GuideDetailBlock): string[] {
  switch (block.type) {
    case "paragraph":
      return [block.text];
    case "image":
      return [block.label, block.alt].filter((value): value is string => Boolean(value));
    case "list":
      return block.items.map((item) => `- ${item}`);
    case "subheading":
      return [block.title];
    case "quote":
      return [block.text, block.caption].filter((value): value is string => Boolean(value));
    case "divider":
      return [];
    ...
    case "editorialBoard": {
      const lines = [block.eyebrow, block.title, block.lead, block.note].filter((value): value is string => Boolean(value));
      lines.push(...block.items.flatMap((item) => {
        const itemLines = [item.number, item.badge, item.title, item.body].filter((value): value is string => Boolean(value));
        if (item.items?.length) itemLines.push(...item.items.map((entry) => `- ${entry}`));
        return itemLines;
      }));
      return lines;
    }
...
function normalizeGuide(raw: RawGuideRecord, index: number): GuideItem {
  ...
  const articleDesign = coerceArticleDesign(raw.articleDesign);
