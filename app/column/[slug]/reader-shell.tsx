    ...
    if (token.startsWith("**") && token.endsWith("**")) {
      ...
      result.push(
        <a
          ...
          className="underline decoration-[rgba(27,63,229,0.45)] underline-offset-2"
        >
          {token}
        </a>,
      );
  ...
  return (
    <main className="relative min-h-screen bg-[var(--bg-stage)] text-[var(--text-primary)]">
      ...
      <div className="fixed inset-x-0 top-0 z-40">
        <div className="mx-auto h-[3px] max-w-6xl rounded-full bg-[rgba(14,12,10,0.12)]">
          <div
            className="h-full rounded-full bg-gradient-to-r from-[rgba(173,188,195,0.9)] via-[rgba(27,63,229,0.85)] to-[rgba(27,63,229,0.8)] transition-[width] duration-150 ease-out"
            ...
            <div className="relative z-10">
              ...
              <Reveal>
                <header className="mb-8 border-b border-[var(--border-default)] pb-6">
                  ...
                  <h1 className="cb-sans-heading text-2xl font-semibold leading-relaxed tracking-tight text-[var(--text-primary)] sm:text-3xl lg:text-[2.2rem]">
                    {item.title}
                  </h1>
              ...
              <div className="mt-4">
                {blocks.map((block, index) => {
                  if (block.type === "heading") {
                    ...
                    const styleClass =
                      heading.level === 2
                        ? "mt-14 mb-6 font-sans text-xl font-medium text-[var(--text-primary)] sm:text-[1.5rem]"
                        : "mt-9 mb-4 text-sm font-semibold tracking-[0.06em] text-[rgba(14,12,10,0.86)]";
                  ...
                  if (block.type === "list") {
                    return (
                      <Reveal key={`list-${index}`} delay={80}>
                        <ul className="mt-4 space-y-1.5 text-sm leading-relaxed text-[rgba(14,12,10,0.86)] sm:text-[15px] sm:leading-[1.9rem]">
                          {block.items.map((itemText, idx) => (
                            <li key={`${itemText}-${idx}`} className="flex gap-2">
                              <span className="mt-[7px] h-[3px] w-5 rounded-full bg-[rgba(27,63,229,0.7)]" />
                  ...
                  if (!firstParagraphRendered && block.text.trim().length) {
                    ...
                    return (
                      <Reveal key={`p-${index}`} delay={160}>
                        <p className="first-letter-float mt-2 text-[13px] leading-8 text-[rgba(14,12,10,0.86)] sm:text-[15px] sm:leading-[2rem]">
                  ...
                  return (
                    <Reveal key={`p-${index}`} delay={60}>
                      <p className="mt-6 text-[13px] leading-8 text-[rgba(14,12,10,0.86)] sm:text-[15px] sm:leading-[2rem]">
              ...
              <div className="mt-16 border-t border-[var(--border-default)] pt-8">
                <Link
                  href="/column"
                  className="group inline-flex items-center gap-2 text-xs font-medium tracking-[0.2em] text-[rgba(107,101,93,0.88)] hover:text-[var(--accent-base)]"
                >
                  ...
                  return (
                    <li key={h.id}>
                      <a
                        href={`#${h.id}`}
                        className={[
                          "block leading-relaxed transition-colors hover:text-[var(--accent-base)]",
                          h.level === 2
                            ? "font-medium text-[rgba(14,12,10,0.8)]"
