...
const isIndexableColumn = (column) => {
  ...
  const body = isStructuredColumn(column) ? getStructuredColumnAuditBody(column) : safeString(column.body);
...
const isIndexableGuide = (guide) => {
  ...
  const body = isStructuredGuide(guide) ? getStructuredGuideAuditBody(guide) : safeString(guide.body);
