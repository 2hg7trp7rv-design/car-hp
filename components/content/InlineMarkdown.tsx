...
function renderTextWithHardBreaks(value: string, keyPrefix: string): ReactNode {
        ...
        if (/^\n+$/u.test(part)) {
          return (
            <Fragment key={`${keyPrefix}-br-${index}`}>
              {Array.from(part).map((_, brIndex) => <br key={brIndex} />)}
            </Fragment>
          );
        }
...
export function renderInlineMarkdown(text: string): ReactNode {
  ...
  return (
    <>
      {tokens.map((t, idx) => {
        if (t.type === "text") {
          return <Fragment key={idx}>{renderTextWithHardBreaks(t.value, `text-${idx}`)}</Fragment>;
        }
