...
// ──────────────────────────────────────────────
const readMarkdownBodies = async (dirRel, { extensions = [".md"] } = {}) => {
  const dirAbs = path.join(ROOT, dirRel);
  const allowed = new Set(
    (Array.isArray(extensions) ? extensions : [".md"]).map((e) => {
...
const isIndexableCar = (car) => {
  ...
  const yearly = car?.maintenanceSimulation && typeof car.maintenanceSimulation === "object"
    ...
    : "";
  const hasMaintenanceCost = Boolean(yearly) || Boolean(safeString(car.costImpression)) || (typeof car.maintenanceCostYenPerYear === "number" && Number.isFinite(car.maintenanceCostYenPerYear));

  const ok = Boolean(slug) && meetsLength && hasBullets && hasBudget && hasMaintenanceCost;
  if (ok) return true;
...
const isIndexableNews = (n) => {
  ...
  const slug = safeString(n.slug || n.id);
  const override = getOverride("NEWS", slug);
  if (override === "forceNoindex") return false;

  // default: noindex（重複/瞬間性が高い）
  if (override === "forceIndex") return true;
  return false;
};
...
const main = async () => {
  ...
  // ---- Dynamic data slugs ----
  const carBodies = await readMarkdownBodies("content/cars");
  const guideBodies = await readMarkdownBodies("content/guides");
  const columnBodies = await readMarkdownBodies("content/columns");
  const heritageBodies = await readMarkdownBodies("content/heritage");
  const cars = uniqBySlug(await readJsonGlob("data", /^cars\d*\.json$/)).map((car) => {
    const slug = safeString(car?.slug);
    const bodyFromJson = safeString(car?.body);
    const bodyFromFile = slug ? String(carBodies[slug] ?? "").trim() : "";
    const body = bodyFromFile || bodyFromJson;
    return body ? { ...car, body } : car;
  });
  const guides = uniqBySlug(await readJsonGlob("data", /^guides\d*\.json$/)).map((g) => {
    const slug = safeString(g?.slug);
    const bodyFromJson = safeString(g?.body);
    const bodyFromFile = slug ? String(guideBodies[slug] ?? "").trim() : "";
    const body = bodyFromFile || bodyFromJson;
    return body ? { ...g, body } : g;
  });
  const columns = uniqBySlug(await readJsonGlob("data", /^columns\d*\.json$/)).map((c) => {
    const slug = safeString(c?.slug);
    const bodyFromJson = safeString(c?.body);
    const bodyFromFile = slug ? String(columnBodies[slug] ?? "").trim() : "";
    const body = bodyFromFile || bodyFromJson;
    return body ? { ...c, body } : c;
  });
  const heritage = uniqBySlug(await readJsonGlob("data", /^heritage\d*\.json$/)).map((h) => {
    const slug = safeString(h?.slug);
    const bodyFromJson = safeString(h?.body);
    const bodyFromFile = slug ? String(heritageBodies[slug] ?? "").trim() : "";
    const body = bodyFromFile || bodyFromJson;
