...
// generate-robots.mjs / verify-robots.mjs と同じ判定。
// Vercel Preview / Development は robots.txt を全拒否にしてよい。
// ただし、Production相当またはZIP納品/ローカル生成物では全拒否を止める。
...
const shouldAssertCrawlableRobots = !isExplicitPreviewOrDev;
...
function articleHasContent(kind, item) {
  ...
  if (["decision-v1", "guide-board-v2", "column-decision-v2", "cbj-world-v1"].includes(safeString(item?.layoutVariant).toLowerCase())) {
...
// 1) robots.txt が全拒否になっていないこと（ZIP/本番相当のみ）
// Preview/Development は検索流入させないため Disallow: / を許可する。
// ここを無条件に落とすと、Vercel Preview の確認デプロイが毎回失敗する。
const robots = readText("public/robots.txt");
if (shouldAssertCrawlableRobots && robots.split(/\r?\n/).some((line) => /^\s*Disallow:\s*\/\s*$/.test(line))) {
