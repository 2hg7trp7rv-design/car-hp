...
type LayoutConfig = {
  slug: string;
  designSystem?: string;
  hero?: { score?: string; imageSrc?: string; imageAlt?: string; character?: string };
...
function sectionTitle(section: ArticleSection, index: number) {
...
function mainTitle(title: string) {
...
function RichText({ text, linkIndex, as = "p", className }: { text?: string | null; linkIndex: Record<string, InternalLinkMeta>; as?: "p" | "span"; className?: string }) {
  if (!text) return null;
...
function AccentTitle({ title, hero = false }: { title: string; hero?: boolean }) {
  ...
  if (!clean.includes("後悔しやすい理由")) return <>{clean}</>;
  return <>{clean.replace(/後悔しやすい理由/u, "")}<span className={hero ? styles.heroTitleAccent : styles.chapterTitleAccent}>後悔しやすい理由</span></>;
}

function importantLine(article: KintoArticle) {
  for (const section of ((article.sections ?? []) as ArticleSection[])) {
    for (const block of section.blocks ?? []) {
      const highlights = Array.isArray(block.highlights) ? block.highlights : [];
      const hit = highlights.find((item) => typeof item === "string" && /戻せる|説明|安全|物理的|使い続け|後悔/.test(item));
...
function Figure({ block, layout }: { block: Block; layout: LayoutConfig | null }) {
  const rawSrc = asText(block.src);
  const src = rawSrc ? layout?.imageOverrides?.[rawSrc] ?? rawSrc : "";
  const caption = asText(block.label) || asText(block.alt);
...
function ListBlock({ items = [], linkIndex }: { items?: string[]; linkIndex: Record<string, InternalLinkMeta> }) {
  return <ol className={styles.stepList}>{items.map((item, index) => <li className={styles.stepItem} key={index}><span className={styles.stepNum}>{String(index + 1).padStart(2, "0")}</span><RichText text={item} linkIndex={linkIndex} className={styles.stepDesc} /></li>)}</ol>;
}
...
function FlowBlock({ block }: { block: Block }) {
  const steps = (block.steps ?? block.items ?? []) as Block[];
  return <ol className={styles.stepList}>{steps.map((step, index) => <li className={styles.stepItem} key={index}><span className={styles.stepNum}>{asText(step.label) || String(index + 1).padStart(2, "0")}</span><div>{step.title ? <h3 className={styles.stepTitle}>{renderInlineMarkdown(stripNumber(asText(step.title)))}</h3> : null}{step.body ? <p className={styles.stepDesc}>{renderInlineMarkdown(asText(step.body))}</p> : null}</div></li>)}</ol>;
}

function CardsBlock({ block, linkIndex }: { block: Block; linkIndex: Record<string, InternalLinkMeta> }) {
  const items = (block.cards ?? block.items ?? block.cases ?? []) as Block[];
  return <div className={styles.systemsGrid}>{items.map((item, index) => <article className={styles.borderCardNeutral} key={index}><span className={styles.borderCardLabelNeutral}>{asText(item.badge) || asText(item.number) || String(index + 1).padStart(2, "0")}</span>{item.title ? <h3 className={styles.cardTitle}>{renderInlineMarkdown(stripNumber(asText(item.title)))}</h3> : null}{item.body || item.intro ? <RichText text={asText(item.body) || asText(item.intro)} linkIndex={linkIndex} className={styles.cardText} /> : null}{Array.isArray(item.items) ? <ListBlock items={item.items} linkIndex={linkIndex} /> : null}</article>)}</div>;
}

function Callout({ block, linkIndex }: { block: Block; linkIndex: Record<string, InternalLinkMeta> }) {
  const warn = block.tone === "warn";
  return <aside className={warn ? styles.borderCardWarning : styles.solutionBox}>{block.title ? <span className={warn ? styles.borderCardLabelWarning : styles.solutionBoxLabel}>{renderInlineMarkdown(stripNumber(asText(block.title)))}</span> : null}{block.body ? <RichText text={asText(block.body)} linkIndex={linkIndex} className={styles.cardText} /> : null}{Array.isArray(block.items) ? <ListBlock items={block.items} linkIndex={linkIndex} /> : null}</aside>;
}
...
function RenderBlock({ block, layout, linkIndex, index }: { block: Block; layout: LayoutConfig | null; linkIndex: Record<string, InternalLinkMeta>; index: number }) {
  switch (block.type) {
    case "paragraph": return <RichText text={asText(block.text)} linkIndex={linkIndex} className={index === 0 ? styles.leadBody : styles.bodyText} />;
    case "image": return <Figure block={block} layout={layout} />;
    case "list": return <ListBlock items={Array.isArray(block.items) ? block.items : []} linkIndex={linkIndex} />;
    case "subheading": return <h3 className={styles.cardTitle}>{renderInlineMarkdown(stripNumber(asText(block.title)))}</h3>;
    case "quote": return <blockquote className={styles.mindsetBox}><p className={styles.mindsetAccent}>{renderInlineMarkdown(asText(block.text))}</p></blockquote>;
...
function CharacterSummary({ spec, text }: { spec?: V29Section; text?: string | null }) {
  ...
  return <aside className={cx(styles.charSummary, variant)}><Image className={styles.charSummaryImg} src={spec.character} alt="" width={120} height={180} /><div><span className={styles.charSummaryLabel}>{spec.type === "dark-risk" ? "CHECK" : "GUIDE"}</span><p className={styles.charSummaryText}>{text}</p></div></aside>;
}
