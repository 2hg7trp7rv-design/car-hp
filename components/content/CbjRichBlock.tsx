...
function toneClasses(tone: Tone) {
  switch (tone) {
    case "warn":
      return {
        border: "border-[rgba(27,63,229,0.28)]",
    ...
    case "note":
      return {
        border: "border-[rgba(14,12,10,0.10)]",
    ...
    case "accent":
      return {
        border: "border-[rgba(27,63,229,0.24)]",
...
function renderFallback(config: UnknownConfig) {
  ...
  return (
    <div className="mt-6 overflow-hidden rounded-[24px] border border-[var(--border-default)] bg-[rgba(251,248,243,0.92)] shadow-soft-card">
      ...
      <pre className="max-h-[520px] overflow-auto border-t border-[rgba(14,12,10,0.08)] bg-[rgba(251,248,243,0.96)] p-5 text-[11px] leading-relaxed text-[var(--text-secondary)]">
        {pretty}
      </pre>
    </div>
  );
...
export function CbjRichBlock({ config, className }: Props) {
        ...
        <ol className="mt-3 space-y-3">
          {steps.map((s, i) => (
            <li key={i} className="flex gap-3">
              <span className="mt-[0.15em] inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[rgba(27,63,229,0.12)] text-[var(--accent-strong)] text-[11px] font-bold">
                {i + 1}
              </span>
      ...
      <div className={cn("mt-6", className)}>
        ...
        <div className={cn("mt-3 grid gap-3", cards.length >= 2 ? "sm:grid-cols-2" : "")}>
          {cards.map((card, i) => {
            ...
            const inner = (
              <div className="overflow-hidden rounded-[24px] border border-[var(--border-default)] bg-[rgba(251,248,243,0.92)] p-5 shadow-soft-card transition hover:-translate-y-[1px] hover:border-[rgba(27,63,229,0.32)]">
                <div className="flex items-center gap-2">
                  {badge ? (
                    <span className="inline-flex items-center rounded-full border border-[rgba(27,63,229,0.22)] bg-[rgba(27,63,229,0.12)] px-3 py-1 text-[10px] font-semibold tracking-[0.18em] text-[var(--accent-strong)]">
                      {badge}
                    </span>
                  ) : null}
                  <p className="cb-sans-heading text-[16px] text-[var(--text-primary)]">{t}</p>
                </div>
                {b ? (
  ...
  if (type === "kv") {
    ...
    return (
      ...
      >
        {title ? (
          <div className="border-b border-[rgba(14,12,10,0.08)] px-5 py-4">
            <p className="text-[10px] font-semibold tracking-[0.22em] text-[var(--text-tertiary)]">{title}</p>
          </div>
        ) : null}
        <dl className="divide-y divide-[rgba(14,12,10,0.08)]">
          {rows.map((pair, i) => {
