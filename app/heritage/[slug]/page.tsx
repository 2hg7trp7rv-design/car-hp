...
{/* TOC (Highlights は仕様変更で撤去) */}
          ...
          <section
            ...
            <div className="page-shell">
              ...
              <div className="cb-stage-chapter">
                ...
                {c.summary ? (
                  <p className="cb-stage-chapterSummary">
                    {c.summary}
                  </p>
                ) : null}
              </div>
        ...
        {relatedCars.length > 0 ? (
          <div>
            <div className="flex items-end justify-between gap-4">
              ...
              <Link href="/cars" className="cb-tap inline-flex items-center text-[12px] text-[#0ABAB5] hover:underline">
                車種DBへ
              </Link>
            </div>
              ...
              {relatedCars.slice(0, 10).map((car) => {
                ...
                return (
                  <Link
                    key={car.slug}
                    href={`/cars/${car.slug}`}
                    className="cb-tap flex min-w-[220px] items-center gap-3 rounded-full border border-white/18 bg-white/85 px-4 py-2 shadow-soft backdrop-blur transition hover:-translate-y-0.5"
                  >
        ...
        {relatedGuides.length > 0 ? (
          <div className={cn("mt-14", relatedCars.length > 0 ? "" : "mt-0")}>
            <div className="flex items-end justify-between gap-4">
              ...
              <Link href="/guide" className="cb-tap inline-flex items-center text-[12px] text-[#0ABAB5] hover:underline">
                GUIDE一覧
              </Link>
            </div>
      ...
      {relatedHeritage.length > 0 ? (
        <section className="page-shell pb-4">
          <p className="text-[11px] tracking-[0.22em] text-white/40">NEXT STORY</p>
          <Link
            href={`/heritage/${relatedHeritage[0].slug}`}
            className="group mt-3 flex items-start gap-5 rounded-2xl border border-white/12 bg-white/[0.04] p-5 backdrop-blur transition hover:border-[#0ABAB5]/40 hover:bg-white/[0.08]"
          >
            <div className="flex-1 min-w-0">
              ...
              <p className="mt-1 text-[15px] font-medium leading-snug text-white line-clamp-2 group-hover:text-[#0ABAB5] transition-colors">
                {relatedHeritage[0].title}
              </p>
              {(getHeritagePreviewText(relatedHeritage[0]) || relatedHeritage[0].summary) ? (
      ...
      <section className="page-shell py-10">
        <div className="rounded-2xl border border-white/12 bg-black/20 p-5 backdrop-blur">
          ...
          <p className="mt-2 text-[12px] leading-relaxed text-white/65">
            この系統の背景・系譜は、HERITAGE一覧でまとめて辿れます。
          </p>
          <Link
            href="/heritage"
          ...
          >
            HERITAGE一覧へ <span aria-hidden>→</span>
          </Link>
        </div>
      </section>
