...
type PosterVariant = "guide" | "column" | "generic";
...
const TONE_STYLES: Record<Tone, { wash: string; border: string; text: string }> = {
  ...
  clay: {
    wash: "bg-[var(--surface-clay)]",
    border: "border-[rgba(192,124,89,0.18)]",
    text: "text-[var(--accent-clay)]",
  },
  slate: {
    wash: "bg-[var(--surface-slate)]",
    border: "border-[rgba(135,152,166,0.18)]",
    text: "text-[var(--accent-slate)]",
  },
  taupe: {
    wash: "bg-[var(--surface-2)]",
    border: "border-[rgba(141,126,114,0.18)]",
    text: "text-[var(--accent-taupe)]",
  },
};
        ...
        <div
          className={cn(
            ...
            resolvedImage.hasRealImage
              ? "bg-[linear-gradient(180deg,rgba(251,248,243,0.02),rgba(31,28,25,0.2))]"
              : "bg-[linear-gradient(180deg,rgba(251,248,243,0.28),rgba(251,248,243,0.12),rgba(31,28,25,0.12))]",
          )}
        />
        {!resolvedImage.hasRealImage ? (
          ...
          {stats.length > 0 ? (
            <div className="grid gap-3 sm:grid-cols-3">
              {stats.map((stat, index) => {
                const tone = TONE_STYLES[stat.tone ?? "moss"];
                return (
                  <div
