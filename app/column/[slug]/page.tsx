...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      type: "article",
      url,
      images: (item as any).heroImage ? [(item as any).heroImage] : [],
    },
    twitter: {
    ...
    if (columnWithMeta.category === "MARKET" || columnWithMeta.tags?.includes("相場") || columnWithMeta.tags?.includes("購入")) {
      return (
        <AffiliateCtaBlock
          ...
          href={AFFILIATE_URLS.SEARCH_INVENTORY} // ★定数を使用
          type="buy"
        />
      );
    }
    if (columnWithMeta.category === "MAINTENANCE" || columnWithMeta.tags?.includes("故障") || columnWithMeta.tags?.includes("車検")) {
      return (
        <AffiliateCtaBlock
          ...
          href={AFFILIATE_URLS.WARRANTY} // ★定数を使用
          type="insurance"
        />
      );
    }
    if (columnWithMeta.category === "OWNER_STORY" || columnWithMeta.tags?.includes("売却")) {
        return (
          <AffiliateCtaBlock
            ...
            href={AFFILIATE_URLS.SELL_ASSESSMENT} // ★定数を使用
            type="sell"
          />
        );
      }
