...
type LinkTone = "cobalt" | "fog" | "cyan" | "rose";
...
function getToneClasses(tone?: LinkTone): {
  ...
  switch (tone) {
    ...
    default:
      return {
        hoverBorder: "hover:border-[rgba(27,63,229,0.32)]",
    ...
    <section aria-label={title} className="mt-8">
      <Reveal>
        <div className="mb-3">
          ...
          <h2 className="cb-sans-heading mt-2 text-lg text-[var(--text-primary)]">{title}</h2>
        </div>
              ...
              <Reveal key={`${navId}_${index}`} delay={60 + index * 60}>
                ...
                >
                  <GlassCard
                    padding="lg"
                    magnetic
                    className={`group border border-[rgba(14,12,10,0.12)] bg-[var(--surface-1)] shadow-soft transition hover:-translate-y-[1px] ${tone.hoverBorder} hover:shadow-soft-card`}
                  >
                    <p className="text-[10px] font-semibold tracking-[0.22em] text-[var(--text-tertiary)]">
                      
                    </p>
                    <h3
                      className={`mt-2 font-sans text-[14px] font-semibold leading-snug text-[var(--text-primary)] ${tone.titleHover}`}
                    >
                      {item.label}
                    </h3>
