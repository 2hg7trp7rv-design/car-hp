...
export default async function NewsDetailPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": `https://car-hp.vercel.app/news/${item.id}`,
    },
  };
    ...
    <main className="min-h-screen bg-gradient-to-r from-[#D1F2EB] via-[#E8F8F5] to-white pb-20 font-sans">
      ...
      <nav className="bg-white/60 backdrop-blur-md border-b border-white/50 sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-6 py-4 flex justify-between items-center">
          <Link
            href="/news"
            className="text-xs text-slate-400 hover:text-[#0ABAB5] transition-colors"
          >
      ...
      <article className="max-w-3xl mx-auto px-6 pt-10">
        ...
        <div className="bg-white rounded-2xl p-8 md:p-10 shadow-[0_2px_20px_-5px_rgba(0,0,0,0.05)] border border-slate-100">
          ...
          <div className="mt-12 pt-8 border-t border-slate-100">
            <p className="text-center text-[10px] text-slate-400 uppercase tracking-widest mb-4">
              Share this story
            </p>
            <div className="flex justify-center gap-4">
              ...
              <a
                href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(
                  ...
                  `https://car-hp.vercel.app/news/${item.id}`,
                )}`}
                target="_blank"
        ...
        {relatedNews.length > 0 && (
          <section className="mt-16">
            <div className="flex items-center mb-6">
              ...
              <span className="px-4 text-xs text-slate-500 font-serif tracking-widest uppercase">
                Related Stories
              </span>
              <div className="h-px flex-1 bg-slate-300/50"></div>
            </div>
