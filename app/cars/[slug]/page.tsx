...
function normalizeBulletItem(value: string): string {
  return value
    ...
    // common list markers
    .replace(/^[-*・\u2022]+\s*/g, "")
    // trailing punctuation (JP/EN)
    .replace(/[。．\.]+$/g, "")
    .trim();
}

function filterRedundantConclusionBlocks(
  blocks: ContentBlock[],
  opts: {
    bestFor: string[];
    notFor: string[];
  ...
  const bestNorm = new Set(uniqKeepOrder(opts.bestFor).map(normalizeBulletItem));
  const notNorm = new Set(uniqKeepOrder(opts.notFor).map(normalizeBulletItem));
  const summary = (opts.summaryText ?? "").trim();

  // If structured data is empty, we keep everything from body.
  const hasStructured = bestNorm.size > 0 || notNorm.size > 0 || !!summary;
  if (!hasStructured) return blocks;

  const out: ContentBlock[] = [];
  for (const b of blocks) {
    if (b.type === "h") {
      const t = b.text.trim();
      if (/結論/.test(t)) continue;
      if (/向く人|向かない人/.test(t)) continue;
    }

    if (b.type === "p") {
      const t = b.text.trim();
      if (summary && t === summary) continue;
      if (/^結論[:：]/.test(t)) continue;
    }

    if (b.type === "ul") {
      const rawItems = (b.items ?? []).map((s) => s.trim()).filter(Boolean);
      if (rawItems.length > 0 && (bestNorm.size > 0 || notNorm.size > 0)) {
        const kept: string[] = [];
        for (const it of rawItems) {
          const key = normalizeBulletItem(it);
          if (!key) continue;
          if (bestNorm.has(key)) continue;
          if (notNorm.has(key)) continue;
          kept.push(it);
        }

        // If this UL is fully redundant, drop it.
        if (kept.length === 0) continue;

        // If partially redundant, keep only unique items.
        if (kept.length !== rawItems.length) {
          out.push({ type: "ul", items: kept });
          continue;
        }
      }
    }
...
function buildBlocksForCar(car: CarItem): {
  ...
  // car.body の「結論」セクションは、データ(bestFor/notFor/summary)と重複しがち。
  // ここでは“追加情報”だけ残して重複を削る（削除ではなく表示重複の解消）。
  {
    const list = bodySections.conclusion;
    if (list && list.length > 0) {
      const filtered = filterRedundantConclusionBlocks(list, {
        bestFor,
        notFor,
        summaryText: car.summaryLong ?? car.summary ?? null,
      });
      for (const b of filtered) blocks.push(b);
    }
  }
