...
export const metadata: Metadata = {
  title: "著作権・引用ポリシー",
  description:
    "CAR BOUTIQUE JOURNALにおける著作権の取り扱いと、引用・外部コンテンツ利用に関するポリシーです",
  alternates: { canonical: `${getSiteUrl()}/legal/copyright` },
};

const SUMMARY_ITEMS = [
  { label: "Text", title: "無断転載不可", body: "文章、構成、レイアウトの無断転載や再配布は禁止します" },
  { label: "Quote", title: "引用は出典明記", body: "引用する場合は出典と該当URLを明示してください" },
  { label: "Media", title: "画像の扱い", body: "画像や動画は権利関係を確認した範囲で使用します" },
] as const;
...
export default function CopyrightPolicyPage() {
  const breadcrumbData = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "著作権・引用ポリシー", item: `${getSiteUrl()}/legal/copyright` },
    ],
  };
  ...
  return (
    <>
      ...
      <LegalDocHeader
        eyebrow="著作権"
        title="文章・画像・引用の扱い"
        lead="CAR BOUTIQUE JOURNALに掲載される文章、画像、外部コンテンツをどのように扱うかについての方針です"
        meta={[]}
      />
      ...
      <div className="mt-8 space-y-8">
        ...
        <div className="space-y-10">
          ...
          <LegalDocSection index="02" title="当サイトコンテンツの引用について">
            ...
            <ul>
              ...
              <li>出典として当サイト名と該当ページのURLを明記すること</li>
              <li>著作権法その他関係法令に反しない範囲であること</li>
            </ul>
          </LegalDocSection>

          <LegalDocSection index="03" title="外部コンテンツの扱い">
            <p>
              当サイトでは、外部資料を参照しつつ、要約、独自のコメント、明確な出典リンクという形で紹介する方針です
            </p>
            <p>
              原文の本文を大量に転載することは行わず、見出しや要約、当サイトの編集による解説にとどめます
            </p>
          </LegalDocSection>

          <LegalDocSection index="04" title="画像・動画・生成AIコンテンツの扱い">
            <p>当サイトで使用している画像や動画等は、次のいずれかの方法で入手したものです</p>
            <ul>
          ...
          <LegalDocSection index="05" title="権利侵害の可能性がある場合">
            <p>
              当サイトのコンテンツが著作権その他の権利を侵害している可能性がある場合は、権利者ご本人または正当な代理人の方が必要な情報を添えてご連絡ください
            </p>
            <p>
        ...
        <LegalRelatedLinks
          items={[
            { href: "/legal/sources-factcheck", label: "出典・ファクトチェック", description: "一次情報の優先順位と確認範囲" },
            { href: "/legal/disclaimer", label: "免責事項", description: "掲載情報の範囲と利用時の注意" },
            { href: "/legal/privacy", label: "プライバシーポリシー", description: "取得する情報と利用目的" },
            { href: "/contact", label: "お問い合わせ", description: "権利に関する確認や削除依頼の窓口" },
          ]}
        />
      </div>
    </>
