...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const title = "GUIDE｜車のガイド";
  const description =
    "車選びや整備、カスタムで迷ったときに見る実用記事をまとめています。";
...
function mapCategoryLabel(category?: string | null): string {
  switch ((category ?? "").toUpperCase()) {
    ...
    case "LIFE":
      return "日常と車";
    default:
      return category ? String(category) : "ガイド";
  }
}
...
export default async function GuideArchivePage({ searchParams }: PageProps) {
  ...
  const category = toSingle(resolvedSearchParams.category);
  const displayTagParam = toSingle(resolvedSearchParams.displayTag);
  const legacyTag = toSingle(resolvedSearchParams.tag);

  const sortKey = safeSortKey(toSingle(resolvedSearchParams.sort));
  const viewMode: "card" | "list" = toSingle(resolvedSearchParams.view) === "list" ? "list" : "card";
  ...
  const topDisplayTags = GUIDE_DISPLAY_TAGS.filter((tag) => (displayTagCounts.get(tag) ?? 0) > 0).slice(0, 8);
  const topCategories = categoryInfos.slice(0, 8);
  ...
  return (
    <main className="min-h-screen bg-[var(--paper)]">
      ...
      <div className="page-shell pb-24 pt-14">
        <section className="cb-filter-section cb-panel p-5 sm:p-6">
          ...
          >
            <div>
              ...
              <div className="relative mt-2">
                <span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[rgba(31,28,25,0.55)]">
                  <svg
                    width="18"
                    height="18"
                    viewBox="0 0 24 24"
            ...
            <section className="rounded-2xl border border-[rgba(31,28,25,0.10)] bg-[rgba(251,248,243,0.60)] px-4 py-3">
              ...
              <div className="grid gap-5 pt-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  >
                    <option value="">すべて</option>
                    {GUIDE_DISPLAY_TAGS.filter((t) => (displayTagCounts.get(t) ?? 0) > 0).map((t) => (
                ...
                <div className="sm:col-span-2">
                  ...
                  <input
                    ...
                    placeholder="例: 足回り / TVキャンセラー / 社外パーツ"
                    className="cb-input"
                  />
                </div>
              </div>
            ...
            {/* chipsで操作するため、フォーム送信時に値が落ちないよう hidden で保持 */}
              ...
              <div className="mt-6">
              <p className="cb-field-label mb-3">関連する分類</p>
              ...
              <div className="mt-6">
              <p className="cb-field-label mb-3">テーマ</p>
              <div className="flex flex-wrap gap-2">
              {topCategories.map((c) => (
              <Link
              key={c.key}
        ...
        <section className="mt-16">
          <div className="flex items-end justify-between gap-6">
            ...
            <p className="cb-font-sans text-sm tracking-[0.18em] text-[var(--text-tertiary)]">
              {activeFilterLinks.length > 0 ? `該当${totalItems}本` : `全${totalItems}本`}
            </p>
          </div>

          {totalItems === 0 ? (
            <div className="mt-8 rounded-3xl border border-[rgba(31,28,25,0.10)] bg-white/55 p-8">
