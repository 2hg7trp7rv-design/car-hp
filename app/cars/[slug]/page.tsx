...
function resolveCarHeroImagePath(car: Pick<CarItem, "slug" | "heroImage" | "mainImage" | "ogImageUrl">): string | null {
  const editorialImage = getEditorialCarImageBySlug(car.slug);
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    robots: isIndexableCar(car) ? INDEX_ROBOTS : NOINDEX_ROBOTS,
  };
}
...
function fitCarTitleStyle(text?: string | null, scale = 1): CSSProperties {
  const count = Math.max(4, Array.from(String(text ?? "").replace(/\s+/g, "").trim()).length);
  const vw = Math.min(12.8, Math.max(2.05, (108 * scale) / count));
  return { "--cbj-title-chars": count, "--cbj-title-vw": `${vw.toFixed(3)}vw` } as CSSProperties;
}
...
function buildBlocksForCar(car: CarItem): {
  ...
  const isInternalPathListItem = (text: string): boolean => {
    ...
    return /^\/(cars|guide|column|heritage)\//.test(t) || /\]\(\/(cars|guide|column|heritage)\//.test(t);
  };
...
export default async function CarDetailPage({ params }: Props) {
  ...
  const relatedCars = pickRelatedSameGroup(car, indexCarsForNav, {
    limit: 3,
    getGroupKey: (item) => String(item.makerKey ?? item.maker ?? ""),
    getFallbackGroupKey: (item) => String(item.bodyType ?? item.segment ?? ""),
    getSecondaryScore: (base, item) => tagOverlapScore(base.tags, item.tags),
  });
  ...
  const carUrl = `${getSiteUrl()}/cars/${encodeURIComponent(car.slug)}`;
  const carImageUrl = safeHeroImage
    ? (safeHeroImage.startsWith("http") ? safeHeroImage : `${getSiteUrl()}${safeHeroImage}`)
    : undefined;
  const articleJsonLd = {
    "@context": "https://schema.org",
    "@type": "Article",
    headline: title,
    description: heroLead || takeaways[0] || car.summary || car.summaryLong || "",
    url: carUrl,
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": carUrl,
    },
    image: carImageUrl ? [carImageUrl] : undefined,
    datePublished: car.publishedAt || car.createdAt,
    dateModified: car.updatedAt || car.publishedAt || car.createdAt,
    inLanguage: "ja",
  ...
  return (
    <main className="car-article-page">
      ...
      <JsonLd id={`ld-car-${car.slug}`} data={articleJsonLd} />
      ...
      <section className="car-article-hero" aria-labelledby="car-article-title">
        <div className="car-article-hero-media" aria-hidden="true">
          {ENABLE_CAR_IMAGES && safeHeroImage ? <Image src={safeHeroImage} alt={`${heroDisplayTitle}のメインビジュアル`} fill sizes="(max-width: 493px) 100vw, 493px" className="car-article-hero-image" style={{ objectPosition: "50% 46%" }} priority /> : null}
          <div className="car-article-hero-shade" />
        </div>
        <div className="car-article-hero-inner">
          <nav className="car-article-crumb" aria-label="パンくずリスト"><Link href="/">ホーム</Link><span>・</span><Link href="/cars">車種</Link><span>・</span><span>{heroDisplayTitle}</span></nav>
          {heroMetaLabel ? <p className="car-article-hero-meta">{heroMetaLabel}</p> : null}
          <h1 id="car-article-title" className="car-article-hero-title" style={fitCarTitleStyle(heroDisplayTitle)}>{heroDisplayTitle}</h1>
      ...
      <div className="car-article-paper">
        <section className="car-article-overview" aria-labelledby="car-overview-title">
          ...
          <h2 id="car-overview-title" className="car-article-overview-title" style={fitCarTitleStyle(heroDisplayTitle, 0.88)}>{heroDisplayTitle}</h2>
          ...
          <div className="car-article-overview-links"><Link href={parentPillarHref}>{parentPillarLabel} <span aria-hidden="true">→</span></Link><Link href="/cars">車種一覧へ <span aria-hidden="true">→</span></Link></div>
        ...
        <article className="car-article-main">
          {sections.map((sec, idx) => <section key={sec.id} className="car-article-section" aria-labelledby={sec.id}><p className="car-article-section-no">{chapterNo(idx)}.</p><h2 id={sec.id} className="car-article-section-title" style={fitCarTitleStyle(sec.title, 0.82)}>{sec.title}</h2><div className="car-article-rule" /><div className="car-article-section-body">{renderSectionBlocks(sec)}</div></section>)}
        </article>

        {relatedCars.length > 0 ? (
          <section className="car-article-section" aria-label="関連記事3選">
            <p className="car-article-section-no">NEXT.</p>
            <h2 className="car-article-section-title" style={fitCarTitleStyle("関連記事3選", 0.82)}>関連記事3選</h2>
            <div className="car-article-rule" />
            <div className="car-article-related-list">
              {relatedCars.map((related) => (
                <Link key={related.slug} href={`/cars/${encodeURIComponent(related.slug)}`} className="car-article-related-card">
                  <div>
                    <p><span>{String(related.maker ?? "CAR").toUpperCase()}</span>{related.releaseYear ? <time>{related.releaseYear}年頃</time> : null}</p>
                    <strong>{stripMakerPrefix(related.titleJa ?? related.title, related)}</strong>
                    {related.summary ? <small>{related.summary}</small> : null}
                  </div>
        ...
        <nav className="car-article-bottom-nav" aria-label="車種ページ内ナビゲーション"><Link href={backToCarsHref}>← 車種一覧</Link><Link href="/guide">ガイド一覧 →</Link></nav>
      </div>
    </main>
  );
}
