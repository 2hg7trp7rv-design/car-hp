...
async function getSameCategoryGuideRelated(guide: GuideItem, limit = 3): Promise<GuideItem[]> {
  const all = await getAllGuides();
  return pickRelatedSameGroup(guide, all, {
    limit,
    getGroupKey: (item) => String(item.category ?? ""),
    getFallbackGroupKey: (item) => resolveGuideDisplayTag(item),
    getSecondaryScore: (base, item) =>
      tagOverlapScore(base.tags, item.tags) + tagOverlapScore(base.intentTags, item.intentTags),
  });
}
...
export default async function GuideDetailPage({ params }: PageProps) {
  ...
  if (guide.layoutVariant === "decision-v1") {
    const related = await getSameCategoryGuideRelated(guide, 3);
      ...
      <div className="detail-shell pb-24 pt-24 sm:pt-28">
        ...
        {takeawaysItems.length > 0 ? (
          <section className="mt-8">
            <section className="detail-card-moss p-6 sm:p-8" aria-label="おすすめポイント">
              ...
              <ul className="mt-5 space-y-5">
                {takeawaysItems.map((t, idx) => (
                  <li key={idx} className="flex gap-3">
                    <span className="mt-[0.55em] inline-flex h-5 w-5 items-center justify-center rounded-full bg-[rgba(122,135,108,0.12)] text-[var(--accent-strong)] shadow-soft">
              ...
              <div className="px-6 py-6 sm:px-8 sm:py-7">
                ...
                <div className="space-y-8 cb-prose">
                  {section.blocks.map((b, idx) => {
                    ...
                    if (b.type === "hr") {
                      return <hr key={idx} className="my-8 border-[rgba(31,28,25,0.08)]" />;
                    }
        ...
        {related.length > 0 ? (
          <section className="mt-14" aria-label="関連記事">
            <div className="flex items-baseline justify-between gap-4">
              <h2 className="text-[24px] font-semibold tracking-[-0.03em] text-[var(--text-primary)]">関連記事3選</h2>
