  ...
  return (
    <div className="min-h-screen bg-[var(--bg-stage)] text-[var(--text-primary)]">
      <section className="relative overflow-hidden border-b border-[var(--border-default)] bg-[linear-gradient(135deg,rgba(251,248,243,0.98)_0%,rgba(238,231,222,0.92)_55%,rgba(246,242,235,1)_100%)]">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(135,152,166,0.16),transparent_34%)]" />
        ...
        {leadStory ? (
          <section className="mb-14 grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
            <Link
              ...
              className="group relative overflow-hidden rounded-[22px] border border-[var(--border-default)] bg-[var(--surface-1)] p-8 transition-all duration-180 hover:border-[rgba(122,135,108,0.3)]"
            >
              ...
              <div className="relative flex min-h-[260px] flex-col justify-end">
                ...
                <h2
                  className="mt-4 text-[24px] font-semibold leading-[1.35] text-[var(--text-primary)] group-hover:text-[var(--accent-slate)] lg:text-[28px]"
                  style={{ fontFamily: "var(--font-editorial)" }}
                >
                  {leadStory.title}
                </h2>
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {ERA_CHIPS.filter((era) => activeGroups.some((group) => group.decade === era)).map((era) => (
                  <a
                    key={era}
                    href={`#${getAnchor(era)}`}
                    className="rounded-full border border-[var(--border-default)] bg-[var(--surface-2)] px-4 py-1.5 text-[11px] font-semibold tracking-[0.12em] text-[var(--text-secondary)] transition-colors duration-150 hover:border-[rgba(122,135,108,0.3)] hover:text-[var(--text-primary)]"
                  >
                    {era}
                  </a>
                ))}
                ...
                <div className="grid gap-4 lg:grid-cols-[1.35fr_0.85fr_0.85fr]">
                  {lead ? (
                    <Link
                      ...
                      className="group flex min-h-[220px] flex-col justify-end rounded-[20px] border border-[var(--border-default)] bg-[var(--surface-1)] p-6 transition-all duration-180 hover:border-[rgba(122,135,108,0.3)] lg:row-span-2 lg:min-h-[310px]"
                    >
                      <p className="text-[10px] font-semibold tracking-[0.18em] text-[var(--accent-base)]">注目記事</p>
                      <h3
                        className="mt-4 text-[18px] font-semibold leading-[1.4] text-[var(--text-primary)] group-hover:text-[var(--accent-slate)] lg:text-[22px]"
                        style={{ fontFamily: "var(--font-editorial)" }}
                      >
                        {lead.title}
                      </h3>
                  ...
                  {others.map((item) => (
                    <Link
                      ...
                      className="group flex flex-col justify-between rounded-[20px] border border-[var(--border-default)] bg-[var(--surface-1)] p-5 transition-all duration-180 hover:-translate-y-[1px] hover:border-[rgba(122,135,108,0.3)]"
                    >
                      <div>
                        <p className="text-[10px] font-semibold tracking-[0.16em] text-[var(--text-tertiary)]">関連する記事</p>
                        <h3 className="mt-3 text-[15px] font-semibold leading-[1.5] text-[var(--text-primary)] group-hover:text-[var(--accent-slate)] line-clamp-3">{item.title}</h3>
                      </div>
