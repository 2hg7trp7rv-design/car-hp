...
export default async function ColumnDetailPage({ params }: Props) {
  ...
  const publicUpdateReason = formatPublicUpdateReason(item.updateReason);
    ...
    <main className="relative text-white">
      ...
      <div className="page-shell pb-24 pt-24">
        ...
        <article className="mt-10 cb-prose">
          ...
          {(publicUpdateReason || (item.sources && item.sources.length > 0)) && (
            <div className="mt-12 space-y-6">
              {publicUpdateReason ? (
                <section className="rounded-3xl border border-white/10 bg-white/5 p-6">
                  <h2 className="serif-heading text-[18px] tracking-tight text-white">このコラムについて</h2>
        ...
        {/* 次の1本 — 関連COLUMNから先頭1件 (B11) */}
        {related.length > 0 ? (
          <section className="mt-14 border-t border-white/10 pt-10">
            <p className="text-[11px] tracking-[0.22em] text-white/40">NEXT</p>
            <Link
              href={`/column/${encodeURIComponent(related[0].slug)}`}
              className="group mt-3 flex items-start gap-4 rounded-2xl border border-white/12 bg-white/[0.04] p-5 backdrop-blur transition hover:border-[#0ABAB5]/40 hover:bg-white/[0.08]"
            >
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-medium tracking-[0.22em] text-white/40">次に読む</p>
                <p className="mt-1 text-[15px] font-medium leading-snug text-white line-clamp-2 group-hover:text-[#0ABAB5] transition-colors">
                  {related[0].titleJa ?? related[0].title}
                </p>
                {related[0].summary ? (
                  <p className="mt-1.5 text-[12px] leading-relaxed text-white/50 line-clamp-1">
                    {related[0].summary}
                  </p>
                ) : null}
              </div>
              <span className="shrink-0 text-[#0ABAB5] mt-1" aria-hidden>→</span>
            </Link>
          </section>
        ) : null}

        {/* PILLAR — 本文後ろ (B06: 冒頭から移動) */}
        <section className="mt-12 border-t border-white/10 pt-8">
          <div className="rounded-2xl border border-white/12 bg-black/20 p-5 backdrop-blur">
            <p className="text-[10px] font-medium tracking-[0.22em] text-white/45">関連ナビ</p>
            <p className="mt-2 text-[12px] leading-relaxed text-white/65">
              コラムは「短い結論」を積み上げるための棚です。関連テーマは一覧から辿れます。
            </p>
            <Link
              href="/column"
              className="cb-tap mt-3 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/8 px-4 py-2 text-[11px] tracking-[0.18em] text-white/70 hover:bg-white/12"
            >
        ...
        {/* 末尾CTA — 次の判断に向けた1本 (B06) */}
        <div className="mt-12 border-t border-white/10 pt-10">
          <p className="text-[11px] tracking-[0.22em] text-white/40">NEXT</p>
          <div className="mt-4 flex flex-wrap gap-3">
            <Link
              href="/column"
              className="cb-tap inline-flex items-center gap-2 rounded-full bg-[#0ABAB5] px-6 py-3 text-[12px] font-semibold tracking-[0.18em] text-[#042624] transition hover:opacity-90"
            >
              COLUMN一覧へ <span aria-hidden>→</span>
            </Link>
            <Link
              href="/guide"
              className="cb-tap inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/8 px-5 py-3 text-[12px] tracking-[0.18em] text-white/75 backdrop-blur transition hover:bg-white/12"
            >
              GUIDEで手順を確認
            </Link>
          </div>
