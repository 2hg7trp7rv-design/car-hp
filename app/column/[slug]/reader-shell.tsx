  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-slate-50/80 via-white to-white">
      ...
      <div className="fixed inset-x-0 top-0 z-50 h-[3px] bg-slate-100">
        <div
          className="h-full bg-gradient-to-r from-tiffany-300 to-tiffany-500 transition-[width] duration-100 ease-out"
          ...
          <div className="mt-8">
            {blocks.map((block, index) => {
              ...
              return (
                <p
                  key={index}
                  className="mt-6 text-sm leading-8 text-slate-600 sm:text-[16px] sm:leading-[2rem]"
                >
                  {block}
                </p>
              );
            ...
            <ul className="space-y-3">
              {headings.map((h) => (
                <li key={h.id}>
                  <a
                    href={`#${h.id}`}
                    className={`block text-[11px] transition-colors hover:text-tiffany-600 ${
                      h.level === 2
