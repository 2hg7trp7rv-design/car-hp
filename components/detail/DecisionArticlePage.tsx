...
function normalizeSections(sections?: GuideDetailSection[] | null): NormalizedSection[] {
  return (sections ?? []).map((section, index) => {
    const id = section.id?.trim() || slugifyId(section.title);
    const kind = resolveSectionKind(section.title, index);
    return {
      ...section,
      id,
      displayTitle: resolveSectionTitle(section.title, kind),
      deck: resolveSectionDeck(section.title, kind) ?? undefined,
      chapterLabel: resolveChapterLabel(section.title, index, kind),
      kind,
    };
  });
}

function resolveSectionKind(title: string, index: number): SectionKind {
...
function resolveChapterLabel(title: string, index: number, kind: SectionKind): string {
  switch (kind) {
    case "conclusion":
      return "CONCLUSION";
    case "process":
...
function normalizeDisplayTitleSource(text: string): string {
  return (text ?? "")
    .replace(/[「」『』]/g, "")
    .replace(/\s+/g, " ")
    .replace(/。+$/u, "")
...
function splitJapaneseDisplayTitle(text: string): string[] {
...
function resolveHeroCopy(article: DecisionArticleViewModel): HeroCopy {
  ...
  const titleLines = splitJapaneseDisplayTitle(primary);
  const sub = rest.join("。");
...
function resolveSectionTitle(title: string, kind: SectionKind): string {
  ...
  if (compact.length > 22) return stripped.replace(/[、,]/g, "、\n");
  return stripped;
}

function resolveSectionDeck(title: string, kind: SectionKind): string | null {
  const compact = compactText(title);
...
function renderTitleLines(lines: string[]) {
  return lines.map((line, index) => {
    ...
    const charCount = Array.from(safeLine).length;
    const keepLine = charCount <= 8;
    const compactLine = keepLine && charCount >= 7;
    ...
    return (
      <span
        ...
        className={[
          styles.titleLine,
          keepLine ? styles.titleLineTight : "",
          compactLine ? styles.titleLineCompact : "",
        ]
          .filter(Boolean)
          .join(" ")}
      >
...
function renderSourcesSummary(source: string): string {
  if (!isExternalHref(source)) return source;
  try {
        ...
        {block.steps.map((step, stepIndex) => (
          <li key={stepIndex} className={styles.sequenceItem} data-cbj-reveal data-cbj-delay={stepIndex * 80}>
            ...
            <div className={styles.sequenceBody}>
              <h3 className={styles.sequenceTitle}>{step.title}</h3>
        ...
        {block.items.map((item, itemIndex) => (
          <li key={itemIndex} className={styles.sequenceItem} data-cbj-reveal data-cbj-delay={itemIndex * 80}>
            ...
            <div className={styles.sequenceBody}>
              ...
              {item.title ? <h3 className={styles.sequenceTitle}>{item.title}</h3> : null}
      ...
      <div className={styles.cardGrid}>
        {block.cards.map((card, cardIndex) => (
          <article key={cardIndex} className={dark ? styles.darkCard : styles.whiteCard} data-cbj-reveal data-cbj-delay={cardIndex * 90}>
            ...
            <h3 className={dark ? styles.darkCardTitle : styles.cardTitle}>{card.title}</h3>
  ...
  return (
    <section key={section.id} id={section.id} className={sectionClass} data-cbj-reveal data-cbj-parallax>
      ...
      <div className={styles.sectionHead} data-cbj-reveal>
        ...
        <h2 className={isDark ? styles.darkSectionTitle : styles.sectionTitle}>{renderTitleLines(section.displayTitle.split("\n"))}</h2>
    ...
    <main className={styles.page} data-cbj-article-page>
        ...
        <div className={styles.heroInner}>
          ...
          <div className={styles.heroGrid}>
            <div className={styles.heroCopy}>
              ...
              <h1 id="decision-article-title" className={styles.title} data-cbj-reveal data-cbj-delay={160}>{renderTitleLines(heroCopy.titleLines)}</h1>
      ...
      <div className={styles.articleRail}>
        ...
        <section className={styles.endCard} data-cbj-reveal data-cbj-parallax>
          ...
          <h2>{renderTitleLines([heroCopy.closingTitle])}</h2>
