...
const quality = {
  ...
  GUIDE: (g) => {
    const body = isDecisionGuide(g) ? getDecisionGuideAuditBody(g) : asString(g.body);
  ...
  COLUMN: (c) => {
    const body = isDecisionColumn(c) ? getDecisionColumnAuditBody(c) : asString(c.body);
    const bodyLen = body.length;
    const headings = countDecisionMarkdownHeadings(body);
...
const main = async () => {
  ...
  const items = all.map((item) => {
    ...
    // Banned chars
    const body =
      type === "GUIDE" && isDecisionGuide(item)
        ? getDecisionGuideAuditBody(item)
        : type === "COLUMN" && isDecisionColumn(item)
          ? getDecisionColumnAuditBody(item)
          : item.body;
