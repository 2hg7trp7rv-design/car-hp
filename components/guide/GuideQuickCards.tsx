...
function renderBullet(text: string): ReactNode {
  ...
  if (m) {
    ...
    return (
      <Link
        href={href}
        className="underline decoration-[rgba(122,135,108,0.45)] underline-offset-2 hover:text-[var(--accent-strong)]"
      >
        {label}
      </Link>
    );
    ...
    >
      <div className="p-5 sm:p-6">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="hidden text-[10px] tracking-[0.18em] text-[var(--text-tertiary)] sm:inline">
            迷わないための要点だけ
          </span>
        </div>

        <div className="mt-4 overflow-hidden rounded-[20px] border border-[rgba(31,28,25,0.12)] bg-[rgba(251,248,243,0.6)]">
          <div className="grid sm:grid-cols-3">
            {resolved.map((card, idx) => (
              <div
                ...
                className={[
                  "p-4",
                  idx !== 0 ? "border-t border-[rgba(31,28,25,0.12)] sm:border-l sm:border-t-0" : "",
                ]
                  .filter(Boolean)
                  .join(" ")}
              >
                {(() => {
                  ...
                  const Title = (
                    <>
                      <span>{head}</span>
                      {tail ? (
                        <span className="mt-1 block text-[11px] font-normal tracking-normal text-[var(--text-tertiary)]">
                          {tail}
                        </span>
                      ) : null}
                    </>
                  ...
                  return (
                    <h3 className="text-[12px] font-semibold tracking-[0.12em] text-[var(--text-primary)]">
                      {card.id ? (
                        <a
                          href={`#${card.id}`}
                          className="underline decoration-transparent underline-offset-4 hover:decoration-[rgba(122,135,108,0.45)]"
                        >
                          {Title}
                        </a>
                      ) : (
                ...
                <ul className="mt-2 space-y-1.5 text-[12px] leading-relaxed text-[var(--text-secondary)]">
