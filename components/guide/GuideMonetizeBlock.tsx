...
export function GuideMonetizeBlock(props: GuideMonetizeBlockProps) {
  ...
  const normalizedMonetizeKey = isNonEmptyString(monetizeKey)
    ? monetizeKey
    : null;

  const mergedLinks = useMemo(() => {
    if (!normalizedMonetizeKey) {
      return {} as AffiliateLinksMap;
    }

    return (
      resolveAffiliateLinksForGuide({
        monetizeKey: normalizedMonetizeKey,
        affiliateLinks: (affiliateLinks ?? null) as any,
      }) ?? {}
    ) as AffiliateLinksMap;
  }, [normalizedMonetizeKey, affiliateLinks]);
  ...
  const config = useMemo(() => {
    ...
    return resolveMonetizeConfig(normalizedMonetizeKey, mergedLinks);
  }, [normalizedMonetizeKey, mergedLinks]);

  const hrefRaw = config?.primaryCta?.href;
  const href = isNonEmptyString(hrefRaw) ? ensureAmazonTag(hrefRaw) : "";
  const resolvedCtaId =
    isNonEmptyString(ctaId)
      ? ctaId
      : `${page_type}_${content_id}_${normalizedMonetizeKey ?? "monetize"}_${position}`;
  const internal = href.length > 0 && isInternalHref(href);
  const canRender =
    ENABLE_MONETIZATION && Boolean(config) && isNonEmptyString(hrefRaw);

  const rel = "nofollow sponsored noopener noreferrer";

  const impressedKeysRef = useRef<Set<string>>(new Set());
  useEffect(() => {
    if (!canRender || !canTrack || !normalizedMonetizeKey) return;
    ...
    trackCtaImpression({
      page_type,
      content_id,
      monetize_key: normalizedMonetizeKey,
            ...
            <div className="shrink-0 md:text-right">
              ...
              >
                <Link
                  ...
                  onClick={() => {
                    if (!canTrack || !normalizedMonetizeKey) return;
