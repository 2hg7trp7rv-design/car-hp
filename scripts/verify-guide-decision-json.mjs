...
import { DECISION_GUIDE_MIN, countMarkdownHeadings, getDecisionGuideAuditBody, isStructuredGuide } from "./lib/guide-decision.mjs";
...
const PUBLIC_INTERNAL_PHRASES = /(child guide|editorial structure|common philosophy|decision UI|duplicate point|rebuild|cleanup|ChatGPT|生成AI|ZIP)/iu;
const CHILD_GUIDE_REQUIRED_MAIN_UI = new Map([
  ["aftermarket-air-cleaner-risk-guide", "付けてもいい / 慎重 / 避ける の判断"],
  ["car-suspension-hard-soft-merit-demerit", "試乗で確認する場面とNG寄りの症状"],
  ["electronic-damper-coilover-risk-guide", "残るもの / 失う可能性があるもの / 残すべき記録"],
  ["adas-lowered-car-aiming-risk-guide", "施工前 / 施工時 / 施工後 の確認フロー"],
  ["tv-canceller-can-risk-guide", "目的別に先に考えること"],
  ["used-custom-car-check-guide", "買っていい / 保留 / 避ける の判断"],
]);
...
function collectPublicText(value) {
  if (value == null) return [];
  if (typeof value === "string") return [value];
  if (Array.isArray(value)) return value.flatMap(collectPublicText);
  if (typeof value === "object") return Object.values(value).flatMap(collectPublicText);
  return [];
}
...
function includesRequiredTableTitle(guide, expectedTitle) {
  return asArray(guide.detailSections)
    .flatMap((section) => asArray(section?.blocks))
    .some((block) => block?.type === "comparisonTable" && String(block?.title ?? "").trim() === expectedTitle);
}

function validateCbjWorldGuideStructure(file, guide, errors) {
  const variant = String(guide.layoutVariant ?? "").trim();
  if (variant !== "cbj-world-v1") return;

  if (!guide.articleDesign || typeof guide.articleDesign !== "object") {
    pushError(errors, file, "cbj-world-v1 guide は articleDesign が必要です");
    return;
  }

  const heroLead = guide.articleDesign.heroLead;
  const heroPromise = guide.articleDesign.heroPromise;
  if (!isNonEmptyString(heroLead)) {
    pushError(errors, file, "articleDesign.heroLead が必要です。ArticleHero側でleadへフォールバックしない前提です");
  }
  if (!isNonEmptyString(heroPromise)) {
    pushError(errors, file, "articleDesign.heroPromise が必要です");
  }
  if (normalizeForCompare(heroLead) === normalizeForCompare(guide.lead)) {
    pushError(errors, file, "articleDesign.heroLead と lead が同義です。ヒーローと本文導入を分けてください");
  }

  const keyPointCount = asArray(guide.keyPoints).filter(isNonEmptyString).length;
  if (keyPointCount > 4) {
    pushError(errors, file, "子GUIDEのkeyPointsは3〜4件に抑えてください。ヒーローの判断文と近い導入密度を避けます");
  }

  const expectedMainUi = CHILD_GUIDE_REQUIRED_MAIN_UI.get(String(guide.slug ?? ""));
  if (expectedMainUi && !includesRequiredTableTitle(guide, expectedMainUi)) {
    pushError(errors, file, `記事別の主役UI「${expectedMainUi}」が必要です`);
  }

  const firstSectionBlocks = asArray(guide.detailSections?.[0]?.blocks);
  if (firstSectionBlocks.some((block) => block?.type === "decisionCards")) {
    pushError(errors, file, "子GUIDE冒頭に固定decisionCardsを置かないでください。記事別の主役UIへ寄せてください");
  }
  const anyDecisionCards = asArray(guide.detailSections)
    .flatMap((section) => asArray(section?.blocks))
    .filter((block) => block?.type === "decisionCards");
  if (anyDecisionCards.length > 0) {
    pushError(errors, file, "子GUIDEでは汎用decisionCardsを使わず、記事別のcomparisonTable/flowで判断UIを作ってください");
  }

  const detailCallouts = asArray(guide.detailSections)
    .flatMap((section) => asArray(section?.blocks))
    .filter((block) => block?.type === "callout");
  if (detailCallouts.some((block) => String(block?.tone ?? "info") === "info")) {
    pushError(errors, file, "本文内のtone=info calloutはPOINT表示になるため使用しないでください。NOTE/CHECK/CAUTIONへ分けてください");
  }
  if (detailCallouts.some((block) => String(block?.title ?? "").trim().includes("結論"))) {
    pushError(errors, file, "本文内に結論calloutを置かず、articleDesign.closingBlocksの最終結論へ統合してください");
  }

  const finalConclusionBlocks = asArray(guide.articleDesign.closingBlocks)
    .filter((block) => block?.type === "callout" && String(block?.title ?? "").trim() === "最終結論");
  if (finalConclusionBlocks.length !== 1) {
    pushError(errors, file, "articleDesign.closingBlocks に『最終結論』calloutを1つだけ置いてください");
  } else if (String(finalConclusionBlocks[0]?.tone ?? "info") !== "info") {
    pushError(errors, file, "最終結論calloutは tone=info として、POINT表示を1回だけ使ってください");
  }

  const publicText = collectPublicText({
    updateReason: guide.updateReason,
    lead: guide.lead,
    keyPoints: guide.keyPoints,
    checkpoints: guide.checkpoints,
    detailSections: guide.detailSections,
    actionBox: guide.actionBox,
    faq: guide.faq,
  }).join("\n");

  if (/SYSTEM\s*0[123]/iu.test(publicText)) {
    pushError(errors, file, "子GUIDE本文に固定SYSTEM 01/02/03を残さないでください");
  }
  if (/(最初に確認すること|取り付け前チェック)/u.test(publicText)) {
    pushError(errors, file, "子GUIDEの確認ブロック見出しを汎用名で固定せず、記事別の役割名にしてください");
  }
  if (PUBLIC_INTERNAL_PHRASES.test(publicText)) {
    pushError(errors, file, "公開本文・更新履歴に内部作業語が残っています");
  }
}
...
function validateBlock(file, sectionIndex, blockIndex, block, errors) {
  ...
  if (block.type === "dialogue") {
    if (block.character !== "juna" && block.character !== "rina") {
      pushError(errors, file, `${base}.character は juna / rina のいずれかです`);
    }
  ...
  if (block.type === "image") {
    if (!isNonEmptyString(block.src)) {
      pushError(errors, file, `${base}.src が必要です`);
    } else {
      validateAssetPath(file, `${base}.src`, block.src, errors);
    }
    if (!isNonEmptyString(block.alt)) {
      pushError(errors, file, `${base}.alt が必要です`);
    }
    if (block.fit != null && block.fit !== "cover" && block.fit !== "contain" && block.fit !== "bleed" && block.fit !== "articleWide") {
      pushError(errors, file, `${base}.fit は cover/contain/bleed/articleWide のみ対応です`);
    }
    return;
  }
  ...
  if (block.type === "subheading") {
    if (!isNonEmptyString(block.title)) pushError(errors, file, `${base}.title が必要です`);
    return;
  }
  ...
  if (block.type === "editorialBoard") {
    if (block.variant != null && !["smoke", "white", "dark"].includes(String(block.variant))) {
      pushError(errors, file, `${base}.variant は smoke / white / dark のいずれかです`);
    }
    if (block.accent != null && !isNonEmptyString(block.accent)) {
      pushError(errors, file, `${base}.accent は空文字にできません`);
    }
    const items = asArray(block.items);
    if (items.length < 2) {
      pushError(errors, file, `${base}.items は 2 件以上必要です`);
    ...
    items.forEach((item, itemIndex) => {
      ...
      if (!isNonEmptyString(item.title)) {
        pushError(errors, file, `${base}.items[${itemIndex}].title が必要です`);
      }
      const bullets = asArray(item.items).filter(isNonEmptyString);
      if (!isNonEmptyString(item.body) && bullets.length === 0) {
        pushError(errors, file, `${base}.items[${itemIndex}] は body か items のどちらかが必要です`);
      }
    });
    return;
  }
...
async function main() {
  ...
  for (const { filePath, guide } of guides) {
    ...
    if (!isStructuredGuide(guide)) continue;
    ...
    validateCbjWorldGuideStructure(file, guide, errors);
    ...
    if (!isNonEmptyString(guide.articleDesign?.heroPromise)) {
      pushError(errors, file, "articleDesign.heroPromise が必要です");
    }
    const inlineConclusionCount = asArray(guide.detailSections)
      .flatMap((section) => asArray(section?.blocks))
      .filter((block) => block?.type === "callout" && String(block?.title ?? "").trim() === "結論").length;
    if (inlineConclusionCount > 0) {
      pushError(errors, file, "detailSections内の『結論』calloutは最終結論へ統合してください");
    }
    const inlinePointCalloutCount = asArray(guide.detailSections)
      .flatMap((section) => asArray(section?.blocks))
      .filter((block) => block?.type === "callout" && String(block?.tone ?? "info") === "info").length;
    if (inlinePointCalloutCount > 0) {
      pushError(errors, file, "本文内のtone=info calloutはPOINT表示になるため使用しないでください。NOTE/CHECK/CAUTIONへ分けてください");
    }
    const finalConclusionCount = asArray(guide.articleDesign?.closingBlocks)
      .filter((block) => block?.type === "callout" && String(block?.title ?? "").includes("結論")).length;
    if (finalConclusionCount !== 1) {
      pushError(errors, file, "articleDesign.closingBlocks に最終結論calloutを1つだけ置いてください");
    }
    ...
    for (const [actionIndex, action] of asArray(guide.actionBox?.actions).entries()) {
      if (isNonEmptyString(action?.label) && !String(action.label).includes("｜")) {
        pushError(errors, file, `actionBox.actions[${actionIndex}].label には「読む理由」を｜で含めてください`);
      }
    }
  ...
  if (errors.length > 0) {
    console.error("\n[verify-guide-decision-json] decision-v1 / guide-board-v2 JSON に不備があります:\n");
