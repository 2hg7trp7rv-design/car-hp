...
function categoryChip(category: string) {
  if (category === "MONEY" || category === "MAINTENANCE_COST") {
    return "border-[rgba(122,135,108,0.22)] bg-[var(--surface-moss)] text-[var(--accent-strong)]";
  }
  if (category === "TROUBLE" || category === "MAINTENANCE") {
    return "border-[rgba(192,124,89,0.22)] bg-[var(--surface-clay)] text-[var(--accent-clay)]";
  }
  return "border-[rgba(135,152,166,0.22)] bg-[var(--surface-slate)] text-[var(--accent-slate)]";
}
    ...
    <section className="mb-10 mt-16">
      <Reveal>
        <div className="mb-6 flex items-baseline justify-between gap-4">
          <div>
            <p className="text-[10px] font-semibold tracking-[0.22em] text-[var(--accent-clay)]">
              OWNERSHIP REALITY
            </p>
            <h2 className="serif-heading mt-2 text-[26px] font-semibold leading-[1.25] text-[var(--text-primary)] md:text-[30px]">
              「{carName}」と付き合う現実
            </h2>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
            ...
            return (
              <Reveal key={guide.slug} delay={idx * 100}>
                ...
                >
                  <GlassCard
                    interactive
                    className="flex h-full flex-col justify-between border border-[var(--border-default)] bg-[var(--surface-1)] p-5 transition-all duration-200 hover:border-[rgba(122,135,108,0.30)]"
                  >
                    <div>
                      <div className="mb-4 flex items-center gap-2">
                        {isMoney && <IconWallet className="h-5 w-5 text-[var(--accent-strong)]" />}
                        {isTrouble && (
                          <IconShieldCheck className="h-5 w-5 text-[var(--accent-clay)]" />
                        )}
                        {!isMoney && !isTrouble && (
                          <IconCar className="h-5 w-5 text-[var(--accent-slate)]" />
                        )}
        ...
        <Reveal delay={200}>
          <GlassCard className="relative flex h-full flex-col justify-center overflow-hidden border border-[rgba(192,124,89,0.18)] bg-[linear-gradient(135deg,rgba(241,226,216,0.96),rgba(251,248,243,0.98))] p-6 shadow-soft-card">
            <div className="pointer-events-none absolute right-0 top-0 h-32 w-32 rounded-full bg-[rgba(192,124,89,0.12)] blur-3xl" />

            <div className="relative z-10">
              <p className="mb-3 text-[10px] font-semibold tracking-[0.22em] text-[var(--accent-clay)]">
                FIND YOURS
              </p>
              <h3 className="serif-heading text-[28px] leading-[1.18] text-[var(--text-primary)]">
                運命の1台を探す
              </h3>
