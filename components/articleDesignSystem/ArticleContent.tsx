...
import { getArticlePartVariant, isMockArticlePartVariant } from "@/components/articleDesignSystem/article-toybox";
...
const MOBILE_FIGURE_SOURCES: Record<string, string> = {
  ...
  "/images/cbj/article-system/custom-air-cleaner-risk-v1.svg": "/images/cbj/article-system/custom-air-cleaner-risk-mobile-v1.svg",
...
function presentationVariantClass(variant: string | null): string | undefined {
  switch (variant) {
    case "soft":
      return styles.blockVariantSoft;
    case "outline":
      return styles.blockVariantOutline;
    case "emphasis":
      return styles.blockVariantEmphasis;
    case "compact":
      return styles.blockVariantCompact;
    case "lead":
      return styles.blockVariantLead;
    case "section":
      return styles.blockVariantSection;
    default:
      return undefined;
  }
}
...
function SubheadingBlock({
  ...
  const variant = getArticlePartVariant(block.presentation);
  ...
  return (
    <aside
      className={cx(
        ...
        variant === "mock-info" && styles.mockInfoBox,
      )}
    >
      <header>
...
function TableBlock({
  ...
  return (
    <section
      className={cx(
        styles.tableBlock,
        variant === "mock-table" && styles.mockTableBlock,
      )}
    >
      {block.title ? <h3>{renderInlineMarkdown(block.title)}</h3> : null}
      <div
...
function StepList({
  ...
  const resolvedVariant = getArticlePartVariant({ variant });
  const className = cx(
    styles.stepBlock,
    resolvedVariant === "mock-numbered" && styles.mockNumberedList,
    resolvedVariant === "mock-step-cards" && styles.mockStepCards,
  );
  return (
        ...
        {steps.map((step, index) => {
          ...
          return (
            <li key={`${index}-${stepTitle || label}`}>
              <span>
                {resolvedVariant === "mock-step-cards"
...
function DecisionCards({
  ...
  const resolvedVariant = getArticlePartVariant({ variant });
  const isMockSystem = resolvedVariant === "mock-system";
...
function presentationClasses(block: GuideDetailBlock): string {
  ...
  const variant = getArticlePartVariant(presentation);
  const variantClass = presentationVariantClass(variant);
  const widthClass =
      ...
      : presentation.motion === "fade-left"
        ...
        : presentation.motion === "fade-right"
          ...
          : presentation.motion === "scale-in"
            ? styles.motionScaleIn
            : undefined;
  const isMockVariant = isMockArticlePartVariant(variant);
  return cx(
    ...
    case "paragraph":
      content = (
        <RichParagraph
          ...
          className={
            getArticlePartVariant(block.presentation) === "lead"
              ? styles.leadText
              : styles.bodyText
          }
        />
