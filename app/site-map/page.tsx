...
export default async function SiteMapPage() {
  ...
  const exhibitionRoutes = getExhibitionRoutes();
          ...
          <Reveal delay={110}>
            <GlassCard className="border border-[#222222]/10 bg-white/80 p-6" padding="none">
              <div className="flex items-end justify-between gap-4">
                <div>
                  <h2 className="font-serif text-[15px] font-semibold text-[#222222]">EXHIBITION ROUTES</h2>
                  <p className="mt-2 text-[11px] text-[#222222]/55">読む順番（目的別チケット）</p>
                </div>
                <Button asChild variant="outline" size="sm">
                  <Link href="/exhibition">一覧へ</Link>
                </Button>
              </div>

              <ul className="mt-4 grid gap-2 text-[12px] text-[#222222]/80 sm:grid-cols-2">
                {exhibitionRoutes.map((r) => (
                  <li key={r.id} className="flex flex-col gap-1">
                    <Link href={`/exhibition/route/${encodeURIComponent(r.id)}`} className="font-semibold hover:underline">
                      {r.title}
                    </Link>
                    <span className="text-[10px] text-[#222222]/55">{r.duration}</span>
                  </li>
                ))}
              </ul>
            </GlassCard>
