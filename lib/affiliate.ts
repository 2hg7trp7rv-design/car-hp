...
function pickAffiliateJson(): AffiliateJsonShape {
  const env = (process.env.NEXT_PUBLIC_AFFILIATE_ENV ?? "prod").toLowerCase();
...
/**
 * monetizeKey から「外部遷移URL（1本）」を解決する（Column / Guide 共通で使える）。
 * - Amazon は tag を強制付与
  ...
  switch (normalizedKey) {
    ...
    case "car_search_price": {
      ...
      if (!name) return base;
          ...
          // 在庫検索サービス: /usedcar/freeword/<keyword>/index.html を優先
      try {
