...
async function resolveSearchParams(searchParams?: Promise<SearchParams>): Promise<SearchParams> {
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    robots: hasMeaningfulSearchParams(resolvedSearchParams) ? NOINDEX_ROBOTS : INDEX_ROBOTS,
  };
}
...
export default async function CarsPage({ searchParams }: PageProps) {
  ...
  const includeNoindex = toSingle(resolvedSearchParams.includeNoindex).trim() === "1";
  const baseAll: ExtendedCarItem[] = includeNoindex
    ? all
    : all.filter((car) => car.publicState === "index");

  const rawQ = toSingle(resolvedSearchParams.q);
  const q = normalize(rawQ);
  const makerParamRaw = toSingle(resolvedSearchParams.maker).trim();
  const makerKeyFilter = makerParamRaw ? normalizeMakerParamToKey(makerParamRaw) : "";
  const bodyTypeParamRaw = toSingle(resolvedSearchParams.bodyType).trim();
  const bodyTypeFilter = bodyTypeParamRaw ? normalizeBodyTypeParam(bodyTypeParamRaw) : "";
  const segmentParamRaw = toSingle(resolvedSearchParams.segment).trim();
  const segmentFilter = segmentParamRaw ? normalizeSegmentLabel(segmentParamRaw) : "";
  const sortKey = toSingle(resolvedSearchParams.sort).trim();

  const rawMinYear = toSingle(resolvedSearchParams.minYear).trim();
  const rawMaxYear = toSingle(resolvedSearchParams.maxYear).trim();
  const rawMinPrice = toSingle(resolvedSearchParams.minPrice).trim();
  const rawMaxPrice = toSingle(resolvedSearchParams.maxPrice).trim();
  const priceBandFilter = toSingle(resolvedSearchParams.priceBand).trim();

  const viewRaw = toSingle(resolvedSearchParams.view).trim();
  const viewMode: "card" | "list" = viewRaw === "list" ? "list" : "card";

  const perPageRaw = toSingle(resolvedSearchParams.perPage).trim();
  ...
  return (
    <main className="min-h-screen bg-[var(--bg-stage)] text-[var(--text-primary)]">
          ...
          <section className="relative isolate min-h-[calc(100svh-64px)] overflow-hidden bg-[#0d0b0a] lg:min-h-[calc(100svh-72px)]">
            ...
            <div className="page-shell relative flex min-h-[calc(100svh-64px)] flex-col items-center pb-10 pt-24 text-center sm:pb-12 sm:pt-28 lg:min-h-[calc(100svh-72px)] lg:pb-16 lg:pt-32">
              ...
              <div className="mt-auto w-full max-w-[44rem] text-white">
                ...
                <h1 className="cb-category-hero-title">
                  SPEC
                </h1>
      ...
      <div className="page-shell pb-24 pt-14">
        <section className="cb-filter-section mt-12">
            ...
            <form id="cars-filter-form" method="get" action="/cars" className="mt-6 space-y-6">
              ...
              <div>
                ...
                <div className="relative">
                  <span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[rgba(31,28,25,0.44)]">
                  ...
                  <input
                    ...
                    className="w-full rounded-full border border-[var(--border-default)] bg-[rgba(251,248,243,0.92)] py-3 pl-11 pr-24 text-[14px] leading-[1.6] text-[var(--text-primary)] outline-none transition focus:border-[rgba(122,135,108,0.55)] focus:bg-[rgba(251,248,243,1)] focus:shadow-[0_0_0_4px_rgba(122,135,108,0.08)]"
                  />
              ...
              <details className="cb-filter-extra rounded-2xl border border-[rgba(31,28,25,0.10)] bg-[rgba(251,248,243,0.62)] px-4 py-3">
                <summary className="cb-filter-advanced-summary">
                  <span>詳細条件を開く</span>
                  <span className="cb-filter-advanced-summary__state" aria-hidden="true">＋</span>
                </summary>
              ...
              <section className="rounded-2xl border border-[rgba(31,28,25,0.10)] bg-[rgba(251,248,243,0.60)] px-4 py-3">
                <p className="cb-field-label mb-3">詳細条件</p>
              ...
              <div className="cb-filter-followup">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-2">
            {activeFilterLinks.map((item) => (
              <Link
                ...
                href={item.href}
                rel="nofollow"
                className="rounded-full border border-[rgba(31,28,25,0.08)] bg-[rgba(251,248,243,0.9)] px-3 py-1 text-[11px] text-[var(--text-secondary)] transition hover:border-[rgba(122,135,108,0.3)] hover:text-[var(--accent-strong)]"
              >
                {item.label} ×
              </Link>
            ))}
