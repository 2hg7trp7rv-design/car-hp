...
type ShortcutKind = "news" | "cars" | "column" | "guide" | "heritage" | "database";
    ...
    case "news":
      return (
        <svg viewBox="0 0 24 24" fill="none" className={cls} aria-hidden="true">
          <path
            d="M6 7.5h8M6 11h12M6 14.5h10M4.5 4.5h13A2.5 2.5 0 0 1 20 7v11.5A2.5 2.5 0 0 1 17.5 21h-11A2.5 2.5 0 0 1 4 18.5V6A1.5 1.5 0 0 1 5.5 4.5Z"
            stroke="currentColor"
            strokeWidth="1.6"
            strokeLinecap="round"
          />
...
export function HeroSection({ overviewShortcuts }: HeroSectionProps) {
  return (
    <section aria-label="CAR BOUTIQUE トップコンテンツ概要" className="relative overflow-hidden bg-slate-950 text-white">
      {/* 背景画像 */}
      <div className="absolute inset-0">
        <Image
          src="/images/hero-sedan.jpg"
          alt="CAR BOUTIQUE hero"
          fill
          priority
          quality={72}
          sizes="100vw"
        ...
        />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,rgba(10,186,181,0.38),transparent_55%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_20%,rgba(15,23,42,0.45),transparent_60%)]" />
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 pb-14 pt-14 sm:px-6 sm:pb-16 sm:pt-16">
        ...
        <Reveal delay={60}>
          <div className="flex items-center gap-2 text-[10px] tracking-[0.28em] text-slate-100/80">
            <span className="rounded-full border border-white/25 bg-white/10 px-3 py-1">CAR BOUTIQUE</span>
            <span className="hidden h-[1px] w-6 bg-slate-300/70 sm:inline-block" />
            <span className="hidden text-slate-200/75 sm:inline">NEWS·COLUMNS·CAR DATABASE·GUIDE·HERITAGE</span>
          </div>
        </Reveal>
