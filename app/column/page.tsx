    ...
    <main className="relative min-h-screen text-white">
      ...
      <div className="page-shell pb-24 pt-24">
        ...
        <div className="flex items-center justify-between">
          <Breadcrumb tone="light" items={[{ label: "HOME", href: "/" }, { label: "COLUMN" }]} />
        </div>
        ...
        {/* search & chips */}
            <section className="mt-10" aria-label="すべてのコラムを見る">
        ...
        {/* list */}
            <section className="mt-10" aria-label="コラム一覧">
          ...
          <div className="mt-4 space-y-2">
            {paginated.map((c) => (
              <Link
                key={c.slug}
                href={`/column/${encodeURIComponent(c.slug)}`}
                className="group flex items-start gap-4 rounded-2xl border border-[#222222]/10 bg-white p-4 shadow-soft transition hover:-translate-y-[1px] hover:border-[#0ABAB5]/35 hover:shadow-soft-card"
              >
                <div className="flex-1 min-w-0">
                  {/* 結論1行 */}
                  <p className="text-[13px] font-semibold leading-snug tracking-tight text-[#222222] line-clamp-1 group-hover:text-[#0ABAB5] transition-colors">
                    {c.title}
                  </p>
                  {/* 理由1行 */}
                  {c.summary ? (
                    <p className="mt-1 text-[12px] leading-relaxed text-[#222222]/60 line-clamp-1">
                      {c.summary}
                    </p>
                  ) : null}
                </div>
                <div className="shrink-0 flex flex-col items-end gap-1.5 pt-0.5">
                  <span className="inline-flex items-center rounded-full border border-[#0ABAB5]/20 bg-[#0ABAB5]/10 px-2.5 py-0.5 text-[10px] font-semibold tracking-[0.14em] text-[#0ABAB5]">
                    {(c.tags ?? [])[0] ?? mapCategoryLabel(c.category)}
                  </span>
                  {c.publishedAt ?? c.updatedAt ? (
                    <span className="text-[10px] text-[#222222]/40">
                      {formatDate(c.publishedAt ?? c.updatedAt ?? null)}
                    </span>
                  ) : null}
                </div>
              </Link>
