...
function normalizeHeritage(raw: HeritageRecord, index: number): HeritageItem {
  ...
  // BaseContentMeta（必須）
  const status = normalizeStatus(safeString(raw.status));
  const kind = normalizeKind(safeString(raw.kind));

  const noindex =
    typeof (raw as any).noindex === "boolean" ? ((raw as any).noindex as boolean) : undefined;

  const publicState = (() => {
    const s = safeString((raw as any).publicState)?.toLowerCase();
    ...
    return "noindex";
  })() as HeritageItem["publicState"];

  const parentPillarId = safeString((raw as any).parentPillarId) ?? "/heritage";
  const relatedClusterIds = toStringArray((raw as any).relatedClusterIds) ?? [];
  const primaryQuery = safeString((raw as any).primaryQuery) ?? title;
  const updateReason = safeString((raw as any).updateReason) ?? "initial-import";
  const sources = toStringArray((raw as any).sources) ?? [];
  ...
  if (Array.isArray(rawSections)) {
    sections = (rawSections as unknown[])
      .map((sec: unknown): HeritageSection | null => {
  ...
  return {
    ...
    cinematicHeritage: (raw as any).cinematicHeritage ?? undefined,
...
function buildHeritageIndex(): HeritageIndex {
  const rawAll = findAllHeritage().filter((h) => !isRedirectSourcePath("/heritage/" + h.slug));
