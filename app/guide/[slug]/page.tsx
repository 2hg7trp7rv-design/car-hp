...
// Special-case: this guide must match the ideal (white) layout exactly.
// We intentionally reuse the dedicated ideal page implementation here as a fallback.
import RoadServiceChoiceGuidePage, {
  generateMetadata as generateRoadServiceChoiceGuideMetadata,
} from "../road-service-choice-guide/page";
...
import { formatPublicUpdateReason } from "@/lib/update-reason";
...
function renderRichText(text: string, guideTitleBySlug: Map<string, string>): ReactNode {
  ...
  while (i < text.length) {
    ...
    out.push(
      <strong key={`b-${k++}`} className="text-[#111111]">
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  // Ideal layout fallback (metadata) for the road-service guide.
  // This ensures consistent SEO metadata even if the route is served via the
  // generic [slug] handler (or an old slug is visited).
  if (
    params.slug === "road-service-choice-guide" ||
    params.slug === "road-service-comparison"
  ) {
    // Delegate to the dedicated ideal page metadata builder.
    // (We intentionally call the named export from the dedicated page instead of
    // importing a static metadata object, because the dedicated implementation
    // uses async guide data.)
    return await generateRoadServiceChoiceGuideMetadata();
  }
...
export default async function GuideDetailPage({ params }: PageProps) {
  // Ideal layout fallback for the road-service guide.
  // If the dedicated route is not being used (or an old slug is visited),
  // force the ideal page so the visual layout matches exactly.
  if (
    params.slug === "road-service-choice-guide" ||
    params.slug === "road-service-comparison"
  ) {
    return <RoadServiceChoiceGuidePage />;
  }
  ...
  const publicUpdateReason = formatPublicUpdateReason(guide.updateReason);
  ...
  const hasH2 = blocks.some((b) => b.type === "heading" && b.heading.level === 2);
  ...
  const contentBlocks: ContentBlock[] = hasH2
    ...
    : [
        {
          type: "heading",
          heading: { id: "content", text: "本文", level: 2 },
        },
        ...blocks,
      ];
  ...
  const toc = contentBlocks
    ...
    .map((b) =>
      b.type === "heading"
        ? ({ ...b.heading, text: stripLeadingStepNumber(b.heading.text) } as HeadingBlock)
        : null,
    )
  ...
  const updatedLabel = guide.updatedAt ? formatDateLabel(guide.updatedAt) : "";
  const readMinutesLabel = guide.readMinutes ? `${guide.readMinutes}分` : "";
  ...
  return (
    <main className="min-h-screen bg-[#fafafa] text-[#222222]">
      ...
      <div className="mx-auto max-w-[820px] px-4 pb-24 pt-24 sm:px-6">
        <div className="bg-white p-6 text-[#222222] sm:p-8">
        {/* top */}
        <Breadcrumb
          items={[
            { label: "HOME", href: "/" },
        ...
        {/* title */}
        <header>
          <div className="flex flex-wrap items-center gap-3 text-[12px] text-[#222222]/65">
            {badge ? (
              <span className="inline-flex items-center rounded-md bg-[#0B66C3] px-3 py-2 text-[12px] font-semibold tracking-[0.04em] text-white">
                {badge}
              </span>
            ) : null}
            {dateLabel ? <span>📅 公開: {formatDateJa(guide.publishedAt ?? guide.updatedAt ?? null)}</span> : null}
            {updatedLabel ? <span>🔄 更新: {formatDateJa(guide.updatedAt)}</span> : null}
            {readMinutesLabel ? <span>⏱️ 読了時間: {readMinutesLabel}</span> : null}
          </div>

          <h1 className="serif-heading mt-8 text-[26px] leading-[1.35] tracking-tight text-[#111111] sm:text-[34px]">
            {guide.title}
          </h1>

          {guide.summary ? (
            <p className="mt-6 rounded-2xl border-l-4 border-[#0B66C3] bg-[#eef3f7] px-5 py-4 text-[15px] leading-[1.95] text-[#374151]">
              {guide.summary}
            </p>
          ) : null}
        </header>
        ...
        {/* hero */}
        {safeHeroImage ? (
          <div className="mt-8 overflow-hidden rounded-3xl border border-[#222222]/10 bg-white shadow-soft-card">
        ...
        {takeawayItems.length > 0 ? (
          <section className="mt-8" aria-label="おすすめポイント">
            <div className="overflow-hidden rounded-3xl border border-[#d7e6f3] bg-[#eef6fc] p-6 shadow-soft">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-[14px] font-semibold tracking-[0.16em] text-[#222222]">
                  おすすめポイント
                </h2>
              </div>
              ...
              <ul className="mt-5 space-y-5">
                {takeawayItems.map((t, idx) => (
                  <li key={idx} className="flex gap-3">
                    <span className="mt-[0.55em] inline-flex h-5 w-5 items-center justify-center rounded-full bg-white text-[#0ABAB5] shadow-soft">
                    ...
                    as="span"
                    className="flex-1"
                    textClassName="cb-stage-body cb-stage-body-strong text-[#24323d]"
                  />
                  </li>
                ))}
              </ul>
...
{/* toc */}
        ...
        <article className="mt-10 cb-prose">
          <div className="space-y-14">
            {contentBlocks.map((b, idx) => {
              if (b.type === "heading") {
                if (b.heading.level === 2) {
                  ...
                  return (
                    ...
                    >
                      <div className="rounded-[1.75rem] border border-[#e5e7eb] bg-[#f8fafb] px-7 py-7 shadow-soft sm:px-9 sm:py-8">
                        <p className="text-[12px] tracking-[0.22em] text-[#6b7280]">
                          CHAPTER {chapter}
                        </p>
                        <h2 className="mt-3 font-serif text-[22px] leading-[1.4] tracking-[0.06em] text-[#111111] sm:text-[26px]">
                          {title}
                        </h2>
                      </div>
                      ...
                      {checkItems.length > 0 && step === renderCheckPointAfterStep ? (
                        <div className="mt-8 overflow-hidden rounded-3xl border border-[#eadf9a] bg-[#fff8d6] p-7 shadow-soft">
                          <p className="text-[12px] font-semibold tracking-[0.22em] text-[#946200]">
                            — 事前に CHECK POINT
                          </p>
                ...
                return (
                  <h3
                    key={`${b.heading.id}-${idx}`}
                    id={b.heading.id}
                    className="cb-prose-block scroll-mt-28 pt-2 font-serif text-[18px] font-semibold leading-[1.9] tracking-[0.08em] text-[#111111]"
                  >
                    {b.heading.text}
                  </h3>
                );
              ...
              if (b.type === "paragraph") {
                return (
                  <TextWithInternalLinkCards
                    ...
                    textClassName="cb-stage-body cb-stage-body-strong text-[#1f2937]"
                  />
                );
              }
                  ...
                  <div key={idx} className="my-16 space-y-6">
                    {lines.map((ln, j) => (
                      <TextWithInternalLinkCards
                        ...
                        as="p"
                        className="cb-prose-block"
                        textClassName="serif-heading text-[24px] font-semibold leading-[1.55] tracking-[0.08em] text-[#0B66C3]"
                      />
                    ))}
                  </div>
                );
              ...
              if (b.type === "image") {
                return (
                  <figure key={idx} className="relative left-1/2 w-screen -translate-x-1/2 px-2">
                    <div className="overflow-hidden rounded-3xl border border-[#222222]/10 bg-white shadow-soft-card">
                      <Image
                        src={b.src}
                        alt={b.alt || ""}
                        width={1600}
              ...
              if (b.type === "code") {
                return (
                  <pre
                    key={idx}
                    className="cb-prose-block overflow-x-auto rounded-2xl border border-[#222222]/10 bg-[#f5f7f9] p-4 text-[12px] leading-[1.7] text-[#1f2937]"
                  >
                    <code>{b.code}</code>
                  </pre>
                );
                  ...
                  <ul key={idx} className="cb-prose-block space-y-4">
                    {b.items.map((t, i) => (
                      <li key={i} className="flex gap-3">
                        <span className="mt-[0.75em] h-[6px] w-[6px] shrink-0 rounded-full bg-[#9ca3af]" />
                        <TextWithInternalLinkCards
                    text={t}
                    linkIndex={linkIndex}
                    as="span"
            ...
            {checkItems.length > 0 && renderedSteps < renderCheckPointAfterStep ? (
              <div className="overflow-hidden rounded-3xl border border-[#eadf9a] bg-[#fff8d6] p-6 shadow-soft">
                <p className="text-[13px] font-semibold tracking-[0.16em] text-[#946200]">
                  — 事前に CHECK POINT
                </p>
                <Checklist
                  items={checkItems}
            ...
            {/* PILLAR — 本文後ろ (B05: 冒頭から移動) */}
            <section className="mt-12 border-t border-[#222222]/10 pt-10">
              <div className="rounded-2xl border border-[#222222]/10 bg-[#f5f7f9] p-5">
                ...
                <Link
                  href={pillar.href}
                  className="cb-tap mt-3 inline-flex items-center gap-2 rounded-full border border-[#222222]/10 bg-white px-4 py-2 text-[11px] font-semibold tracking-[0.18em] text-[#222222] hover:border-[#0ABAB5]/35"
                >
                  {pillar.label}へ戻る
            ...
            {(publicUpdateReason || (guide.sources && guide.sources.length > 0)) ? (
              <div className="mt-12 space-y-6">
                {publicUpdateReason ? (
                  <section className="overflow-hidden rounded-3xl border border-[#222222]/10 bg-[#f5f7f9] p-6 shadow-soft">
                    <h2 className="serif-heading text-[18px] tracking-tight text-[#111111]">このガイドについて</h2>
                    <p className="mt-3 text-[13px] leading-relaxed text-[#4b5563]">
                      {guide.updatedAt ? `${formatDateLabel(guide.updatedAt)} 更新` : ""}
                      {publicUpdateReason ? `　${publicUpdateReason}` : ""}
                    </p>
                  </section>
                ) : null}
                ...
                {guide.sources && guide.sources.length > 0 ? (
                  <section className="overflow-hidden rounded-3xl border border-[#222222]/10 bg-[#f5f7f9] p-6 shadow-soft">
                    <h2 className="serif-heading text-[18px] tracking-tight text-[#111111]">出典・参考資料</h2>
                    <ul className="mt-4 list-decimal space-y-2 pl-5 text-[13px] leading-relaxed text-[#374151]">
                          ...
                          <li key={i}>
                            {isUrl ? (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="underline decoration-[#0ABAB5]/35 underline-offset-4 hover:decoration-[#0ABAB5]/70"
                              >
                                {trimmed}
                              </a>
                            ) : (
        ...
        {related.length > 0 ? (
          <section className="mt-12" aria-label="関連記事">
            <div className="flex items-baseline justify-between">
              <h2 className="serif-heading text-[18px] text-[#111111]">関連記事</h2>
              <Link
                href="/guide"
                className="text-[13px] tracking-[0.16em] text-[#4b5563] hover:text-[#0ABAB5]"
              >
                すべてのGUIDEへ
              </Link>
            </div>
        ...
        {/* 末尾CTA — 次の1本に絞る (B05) */}
        <div className="mt-14 border-t border-[#222222]/10 pt-10">
          <p className="text-[11px] tracking-[0.22em] text-[#222222]/45">NEXT</p>
          {related.length > 0 ? (
            <Link
              href={`/guide/${encodeURIComponent(related[0].slug)}`}
              className="mt-3 group flex items-center gap-4 rounded-2xl border border-[#222222]/10 bg-white p-5 shadow-soft transition hover:border-[#0ABAB5]/35 hover:shadow-soft-card"
            >
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-medium tracking-[0.22em] text-[#222222]/45">次に読む</p>
                <p className="mt-1 text-[14px] font-semibold text-[#222222] line-clamp-2 group-hover:text-[#0ABAB5] transition-colors">
                  {related[0].title}
                </p>
                {related[0].summary ? (
                  <p className="mt-1 text-[12px] text-[#222222]/55 line-clamp-1">{related[0].summary}</p>
                ) : null}
              </div>
              <span className="shrink-0 text-[#0ABAB5]" aria-hidden>→</span>
            </Link>
          ) : (
            <Link
              href="/guide"
              className="mt-3 inline-flex items-center gap-2 rounded-full border border-[#222222]/10 bg-white px-5 py-3 text-[12px] font-semibold tracking-[0.18em] text-[#222222] shadow-soft transition hover:border-[#0ABAB5]/35"
            >
              GUIDE一覧へ <span aria-hidden>→</span>
            </Link>
          )}
        </div>
        </div>
