 ...
 * - affiliateLinks は JSON 側に入っている「論理名 → URL」のマップをそのまま受け取る
 * - まだ本番アフィリエイト URL が決まっていない段階では demo.carboutique.jp へのデモリンクを想定
 */
export function GuideMonetizeBlock(props: GuideMonetizeBlockProps) {
  const { monetizeKey, affiliateLinks } = props;

  if (!monetizeKey || !affiliateLinks) {
    return null;
  }

  const config = resolveMonetizeConfig(monetizeKey, affiliateLinks);
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="space-y-2.5 md:max-w-[70%]">
              ...
              <p className="pt-1 text-[10px] leading-relaxed text-slate-400">
                ※ 現在はデモ用リンクです。実際に申し込み・購入される際は、各サービスの公式情報・注意事項を必ずご確認ください。
              </p>
            </div>
...
function resolveMonetizeConfig(
  ...
  switch (monetizeKey) {
    ...
    case "sell_basic_checklist":
      return {
        heading: "車を高く・スムーズに売るための「サービス選び」の型（デモ）",
        body: [
    ...
    case "sell_import_highclass":
      return {
        heading: "輸入車・高級車に強い買取サービスをどう選ぶか（デモ）",
        body: [
        ...
        primaryCta: {
          label: "輸入車・高級車に強い買取サービスをまとめてチェック（デモ）",
    ...
    case "sell_loan_remain":
      return {
        heading: "ローン残債ありでも相談しやすい買取サービス（デモ）",
        body: [
        ...
        primaryCta: {
          label: "ローン残債ありでも相談できる買取サービスを探す（デモ）",
    ...
    case "insurance_saving":
      return {
        heading: "補償を大きく削らずに「保険料だけ」を見直す第一歩（デモ）",
        body: [
    ...
    case "shaken_rakuten":
      return {
        heading:
          "楽天Car車検で「対応工場と概算費用」を先に押さえておく（デモ）",
        body: [
    ...
    case "insurance_corporate":
      return {
        ...
        primaryCta: {
          label:
            "法人・個人事業主向けの自動車保険を相談できる窓口を探す（デモ）",
    ...
    case "goods_drive_recorder":
      return {
        heading: "用途別に絞り込んだドラレコ候補をチェックする（デモ）",
        body: [
        ...
        primaryCta: {
          label:
            "Amazonで前後2カメラなど人気ドラレコの候補を見る（デモ）",
    ...
    case "goods_child_seat":
      return {
        ...
        primaryCta: {
          label:
            "Amazonで年齢別チャイルドシートの人気モデルをチェックする（デモ）",
    ...
    case "goods_car_wash_coating":
      return {
        heading: "手洗い派向け「スターターセット」をまとめて選ぶ（デモ）",
        body: [
        ...
        primaryCta: {
          label:
            "Amazonで洗車シャンプーと簡易コーティングのスターターセットを見る（デモ）",
    ...
    case "goods_interior_clean":
      return {
        ...
        primaryCta: {
          label:
            "Amazonで車内クリーナーと掃除グッズの定番セットを見る（デモ）",
    ...
    case "goods_jump_starter":
      return {
        heading:
          "いざというときのジャンプスターター候補を1台決めておく（デモ）",
        body: [
        ...
        primaryCta: {
          label:
            "Amazonで対応排気量別ジャンプスターターの候補を見る（デモ）",
