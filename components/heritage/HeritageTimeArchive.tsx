...
export function HeritageTimeArchive(props: { timeline: TimelineItem[] }) {
  ...
  const decadeNote = useMemo(() => {
    ...
    return (decade: string) => map[(decade ?? '').trim()] ?? 'Curating in progress.';
  }, []);
  ...
  return (
    <section className={styles.timelineWrap} aria-label="Heritage Time Archive" ref={rootRef as any}>
      ...
      {/* 3つの入口 — 年代から / テーマから / 車種から (B07) */}
      {!queryNorm && (
        <div className="px-4 pt-8 pb-4 sm:px-6">
          <div className="mx-auto max-w-4xl grid grid-cols-1 gap-3 sm:grid-cols-3">
            <button
              type="button"
              onClick={() => scrollToId('decade-' + encodeURIComponent(viewTimeline?.[0]?.decade ?? ''))}
              className="group flex flex-col gap-2 rounded-2xl border border-white/10 bg-white/[0.04] p-5 text-left backdrop-blur transition-all duration-300 hover:border-[#0ABAB5]/40 hover:bg-white/[0.08]"
            >
              <span className="text-[10px] font-medium tracking-[0.3em] text-[#0ABAB5]/70">01</span>
              <span className="text-[15px] font-medium tracking-[0.06em] text-white group-hover:text-[#0ABAB5] transition-colors">
                年代から遡る
              </span>
              <span className="text-[12px] leading-relaxed text-white/45">
                1960年代から現代まで、時代の壁を歩く
              </span>
            </button>

            <button
              type="button"
              onClick={() => setQuery('スポーツ')}
              className="group flex flex-col gap-2 rounded-2xl border border-white/10 bg-white/[0.04] p-5 text-left backdrop-blur transition-all duration-300 hover:border-[#0ABAB5]/40 hover:bg-white/[0.08]"
            >
              <span className="text-[10px] font-medium tracking-[0.3em] text-[#0ABAB5]/70">02</span>
              <span className="text-[15px] font-medium tracking-[0.06em] text-white group-hover:text-[#0ABAB5] transition-colors">
                テーマから探す
              </span>
              <span className="text-[12px] leading-relaxed text-white/45">
            ...
            <Link
              href="/cars"
              className="group flex flex-col gap-2 rounded-2xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur transition-all duration-300 hover:border-[#0ABAB5]/40 hover:bg-white/[0.08]"
            >
              <span className="text-[10px] font-medium tracking-[0.3em] text-[#0ABAB5]/70">03</span>
              <span className="text-[15px] font-medium tracking-[0.06em] text-white group-hover:text-[#0ABAB5] transition-colors">
                車種から遡る
              </span>
              <span className="text-[12px] leading-relaxed text-white/45">
            ...
            >
              ...
              <div className={styles.decadeInner}>
                ...
                <div className={styles.decadeGrid}>
                  ...
                  {t.total === 0 ? (
                    <div className={`${styles.decadeCardMuted} opacity-30`} aria-hidden="true">
                      <div className={styles.decadeCardMeta}>—</div>
                      <div className={styles.decadeCardTitle}>準備中</div>
                    </div>
