...
export function SearchClient(props: {
  ...
  const [results, setResults] = useState<SearchHit[]>(() => (Array.isArray(initialResults) ? initialResults : []));
  ...
  useEffect(() => {
    const handle = window.setTimeout(async () => {
      ...
      try {
        const url = `/api/search?q=${encodeURIComponent(queryTrimmed)}&type=${encodeURIComponent(type)}&limit=30`;
  ...
  return (
    <form action="/search" method="GET" onSubmit={(event) => event.preventDefault()} className="space-y-6">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {filterButtons.map((button) => (
              <FilterButton key={button.key} active={type === button.key} label={button.label} onClick={() => setType(button.key)} />
            ))}
          </div>
        </div>
      </div>
      ...
      {queryTrimmed.length > 1 ? (
        <div className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[rgba(31,28,25,0.08)] pt-5">
            <p className="text-[13px] text-[var(--text-secondary)]">
              <span className="font-semibold text-[var(--text-primary)]">{results.length}</span> 件を表示中（{typeLabel(type)}）
            </p>
            <p className="text-[11px] text-[var(--text-tertiary)]">
              症状名・部品名・使い方まで入れると絞り込みやすくなります。
            </p>
          </div>
