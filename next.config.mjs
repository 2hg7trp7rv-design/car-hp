...
const nextConfig = {
  ...
  // 逆に build cache / docs / public / scripts はサーバーレス関数へ同梱しない。
  outputFileTracingIncludes: {
    "/": ["./data/articles/**/*"],
    "/api/search": ["./data/articles/**/*"],
  },
  ...
  async redirects() {
    return [
      // Retired entry points: keep them as pure redirects so GSC does not keep noindex noise.
      { source: "/exhibition", destination: "/site-map", permanent: true },
      { source: "/start", destination: "/site-map", permanent: true },
