 ...
 *
 * 役割
 * - data/columns.json の生データを読み込み ColumnItem に正規化
...
// ----------------------------------------
// Markdown bodies (content/columns/*.md) [optional]
// ----------------------------------------
const COLUMN_BODY_BY_SLUG = loadMarkdownBodiesFromDir("content/columns");
...
function normalizeColumn(raw: RawColumnRecord, index: number): ColumnItem {
  ...
  // Prefer Markdown file body when exists: content/columns/<slug>.md
  const bodyFromFile = COLUMN_BODY_BY_SLUG.get(slug) ?? null;
