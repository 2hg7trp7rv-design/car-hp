...
// ボディタイプの特徴説明 (B04)
const BODY_TYPE_DESC: Record<string, { feature: string; suitedFor: string }> = {
  "セダン": {
    feature: "3ボックス構造で静粛性と走行安定性が高い。トランクが独立しているため荷物の盗難リスクが低い。",
    suitedFor: "ビジネス用途・長距離ドライブ・静粛性を重視する人",
  },
  "SUV/クロスオーバー": {
    feature: "高い着座位置で視界が広く、積載性も高い。悪路走破性を持つモデルも多い。",
    suitedFor: "ファミリー・アウトドア・見晴らしの良い運転を好む人",
  },
  "ハッチバック": {
    feature: "コンパクトで取り回しがしやすく、リアハッチを開ければ荷室が広がる実用性が高い形状。",
    suitedFor: "街乗り・駐車スペースが限られる環境・コスパ重視の人",
  },
  "ミニバン": {
    feature: "3列シートで多人数乗車が可能。スライドドアで乗降しやすく、荷室の汎用性も高い。",
    suitedFor: "大家族・送迎用途・荷物が多い人",
  },
  "クーペ": {
    feature: "2ドアまたは2+2ドアのスポーティなシルエット。走行性能と見た目を重視したモデルが多い。",
    suitedFor: "ドライビングを楽しみたい人・スタイルを重視する人",
  },
  "ワゴン": {
    feature: "セダンをベースにルーフラインを後方まで延ばした形状。荷室容量が大きくファミリー向け。",
    suitedFor: "荷物が多い人・アウトドア・長期旅行が多い人",
  },
};
const bodyTypeDesc = BODY_TYPE_DESC[label] ?? null;

  return (
    <main className="relative min-h-screen text-white">
      <DetailFixedBackground imageSrc="/images/exhibit/kv-bodytypes.webp" noUpscale />
      ...
      <div className="mx-auto max-w-7xl px-4 pb-32 pt-28 sm:px-6 lg:px-8">
        ...
        {/* パンくず */}
        <Breadcrumb items={[{ label: "HOME", href: "/" }, { label: "CARS", href: "/cars" }, { label: "BODY TYPES", href: "/cars/body-types" }, { label }]} className="mb-6" />
            ...
            {bodyTypeDesc && (
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div className="rounded-xl bg-white/60 p-3">
                  <p className="text-[10px] font-semibold tracking-[0.18em] text-[#222222]/55">特徴</p>
                  <p className="mt-1 text-[12px] leading-relaxed text-[#222222]/75">{bodyTypeDesc.feature}</p>
                </div>
                <div className="rounded-xl bg-white/60 p-3">
                  <p className="text-[10px] font-semibold tracking-[0.18em] text-[#0ABAB5]">向く人</p>
                  <p className="mt-1 text-[12px] leading-relaxed text-[#222222]/75">{bodyTypeDesc.suitedFor}</p>
                </div>
              </div>
            )}
        ...
        <section className="mb-8 grid gap-3 md:grid-cols-2" aria-label="絞り込みヒント">
          ...
          <GlassCard padding="md" className="bg-white/80">
            <p className="text-[10px] font-semibold tracking-[0.22em] text-[#222222]/55">SEGMENTS</p>
            <div className="mt-2 flex flex-wrap gap-2">
