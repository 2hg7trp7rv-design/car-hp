...
export async function generateMetadata({
  ...
  const title = "CARS | CAR DATABASE";
  const description =
    "主要な車種についてボディタイプセグメント年式や価格帯などの条件で絞り込んで確認できる車種データベース";
          ...
          <section className="mb-8 grid gap-3 text-[11px] text-slate-700 sm:grid-cols-3">
            <GlassCard padding="md" className="flex items-center justify-between bg-white/80">
              ...
              <div className="text-right text-[10px] text-slate-500">
                ...
                <p>
                  メーカー数:
                  <span className="ml-1 font-semibold text-slate-700">{makers.length}</span>
                </p>
                <p>
                  セグメント数:
            ...
            <GlassCard padding="md" className="bg-gradient-to-br from-tiffany-50/80 to-white">
              <p className="text-[10px] font-semibold tracking-[0.2em] text-slate-500">NOTES</p>
              <div className="mt-2 space-y-2 text-[11px] text-slate-700">
                <p className="leading-relaxed">
                  年式は「世代の目安」。価格はグレードや状態で大きく変わります。
                </p>
                <p className="leading-relaxed">
                  まずはメーカー/ボディタイプ/セグメントで絞って、年式・価格レンジで現実に落とすのが早い。
                </p>
                <p className="leading-relaxed">
                  詳細ページ側に「買う前の注意点」「よくあるトラブル」「相場の見方」を順次追記していきます。
                </p>
              </div>
            </GlassCard>
          </section>
                ...
                <div className="flex flex-col gap-1.5 text-[10px] text-slate-500">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {quickPresetsFinal.map((p) => (
                      <Link key={p.href} href={p.href} className={p.className}>
                        {p.label}
                      </Link>
                    ))}
                  </div>
