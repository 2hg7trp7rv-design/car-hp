...
export function normalizeInternalHref(raw: string): string | null {
  ...
  const ok = /^\/(guide|column|search|site-map|contact|legal)(\/|$)/.test(href);
  if (!ok) return null;
...
export function extractInternalLinksFromText(
  ...
  const re = /(^|[^A-Za-z0-9_])((?:\/)?(?:guide|column|search|site-map|contact|legal)(?:\/[a-z0-9][a-z0-9_\/-]*)?)(?=$|[^A-Za-z0-9_/-])/gi;
