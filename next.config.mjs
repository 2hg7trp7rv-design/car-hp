...
const nextConfig = {
  ...
  async headers() {
    ...
    return [
      ...
      // ---- Index safety ----
      // /search, /compare はユーザー導線として残すが、検索エンジンには載せない。
      // - meta robots だけでも良いが、念のためヘッダでも noindex を返す（意図の明確化）。
      // - robots.txt で Disallow しない（noindex を読ませるため）。
      {
        source: "/search",
      ...
      {
        // API はクロール対象ではないが、誤って露出しても index されないようにガード。
        source: "/api/search",
