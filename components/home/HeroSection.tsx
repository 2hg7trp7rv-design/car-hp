  ...
  return (
    ...
    >
      <div className="absolute inset-x-0 top-0 h-[74%] sm:h-[78%] lg:h-[82%]">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_16%_22%,rgba(27,63,229,0.18),transparent_34%),radial-gradient(circle_at_82%_18%,rgba(27,63,229,0.16),transparent_28%),radial-gradient(circle_at_52%_84%,rgba(246,242,235,0.08),transparent_36%)]" />
      </div>
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 pb-14 pt-16 sm:px-6 sm:pb-16 lg:pt-20">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1.15fr)_minmax(21rem,0.85fr)] lg:items-end">
          <div className="max-w-3xl">
            ...
            <Reveal delay={120}>
              <h1 className="cb-sans-heading mt-6 max-w-[11ch] text-[clamp(2.25rem,7.5vw,5.2rem)] font-semibold leading-[1.06] tracking-[-0.02em] text-[var(--text-primary)]">
                車のある空気を、
                <br />
                読む。
              </h1>
            ...
            <Reveal delay={220}>
              <div className="mt-7 flex flex-wrap items-center gap-3">
                <Link
                  href="/cars"
                  className="inline-flex min-h-11 items-center justify-center rounded-full border border-[var(--accent-strong)] bg-[var(--surface-1)] px-5 text-[13px] font-medium tracking-[0.12em] text-[var(--text-primary)] transition hover:-translate-y-px hover:bg-[var(--surface-glow)]"
                >
                  車種を見る
                </Link>
              </div>
              ...
              <aside className="rounded-[28px] border border-[var(--border-default)] bg-[rgba(251,248,243,0.82)] p-5 shadow-soft-card backdrop-blur-sm sm:p-6">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="cb-sans-heading mt-2 text-[24px] font-medium leading-[1.3] text-[var(--text-primary)]">
                      このサイトでできること
                    </p>
                  </div>
                </div>
                ...
                <nav className="mt-5 grid gap-2">
                  {overviewShortcuts.map((s) => (
                    <Link
                      key={s.href}
                      href={s.href}
                      className="group flex items-start gap-3 rounded-[20px] border border-[var(--border-default)] bg-[var(--surface-1)] px-4 py-4 transition hover:-translate-y-px hover:border-[rgba(27,63,229,0.38)] hover:bg-[var(--surface-glow)]"
                      aria-label={s.label}
                    >
