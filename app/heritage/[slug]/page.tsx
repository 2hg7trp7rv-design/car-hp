...
export async function generateMetadata({
  ...
  const image = resolveOgImageUrl(pickHeroImage(item), getSiteUrl());
  ...
  if (deck.length === 0) {
    return (
      <div className="cbj-heritage-hero-card is-active" data-hero-card>
        <FilmImage src={fallbackImage} alt="Heritage hero" />
      </div>
    );
  }
...
export default async function HeritageDetailPage({ params }: PageProps) {
  ...
  const cinematic = getCinematicHeritage(item);
  const relatedHeritage = await getNextReadHeritageV12(item, 3);
      ...
      <section className="cbj-heritage-hero" aria-labelledby="heritage-title">
        ...
        <div className="cbj-heritage-hero-inner">
          <div className="cbj-heritage-hero-copy">
            ...
            <h1 id="heritage-title">{cinematic.title || item.title}</h1>
      ...
      >
        <div className="cbj-heritage-container">
          <div className="sr-only">
            ...
            {cinematic.timelineLead ? <p>{cinematic.timelineLead}</p> : null}
          </div>
      ...
      {chapters.map((chapter, index) => {
        ...
        if (dna) {
          return (
            ...
            >
              <div className="cbj-heritage-container">
                <SectionLabel label={chapter.kicker || "DNA"} />
                <div className="cbj-heritage-dna-head">
                  <h2>{chapter.title}</h2>
                  {chapter.subtitle ? (
                    <p className="cbj-heritage-lead">{chapter.subtitle}</p>
                  ) : null}
                  {chapter.body[0] ? (
        ...
        return (
          ...
          >
            ...
            <div className="cbj-heritage-container">
              {chapter.image && !dark ? (
                <div
                  className="cbj-heritage-chapter-prelude"
                  data-heritage-reveal
                >
                  <FilmImage src={chapter.image} alt={chapter.title} />
                </div>
              ) : null}
