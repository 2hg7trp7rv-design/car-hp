        ...
        <Reveal>
          <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <h2 className="cb-sans-heading mt-2 text-lg font-semibold text-[var(--text-primary)] sm:text-xl">
                {title}
              </h2>
              <p className="mt-2 max-w-2xl text-[11px] leading-relaxed text-[var(--text-secondary)] sm:text-[12px]">
                {lead}
              </p>
            </div>
          </div>
          ...
          {cards.map((card, index) => (
            <Reveal key={card.href} delay={80 + index * 40}>
              <TrackedLink
                ...
                toId={card.toId}
              >
                <GlassCard className="group h-full border border-[rgba(14,12,10,0.12)] bg-[var(--surface-1)] p-5 text-xs shadow-soft transition hover:-translate-y-[1px] hover:border-[rgba(27,63,229,0.32)] hover:shadow-soft-card">
                  <h3 className="line-clamp-2 text-[13px] font-semibold leading-relaxed text-[var(--text-primary)] group-hover:text-[var(--accent-strong)]">
                    {card.label}
                  </h3>
                  <p className="mt-2 line-clamp-3 text-[11px] leading-relaxed text-text-sub">
                    {card.description}
                  </p>
                  <p className="mt-4 text-[12px] font-semibold text-[var(--accent-strong)]">
                    詳細へ →
                  </p>
                </GlassCard>
              </TrackedLink>
