...
export function GuideMonetizeBlock(props: GuideMonetizeBlockProps) {
  ...
  const resolvedMonetizeKey = isNonEmptyString(monetizeKey) ? monetizeKey : null;

  const mergedLinks = useMemo(() => {
    if (!resolvedMonetizeKey) return {} as AffiliateLinksMap;
    return (
      resolveAffiliateLinksForGuide({
        monetizeKey: resolvedMonetizeKey,
        affiliateLinks: (affiliateLinks ?? null) as any,
      }) ?? {}
    ) as AffiliateLinksMap;
  }, [resolvedMonetizeKey, affiliateLinks]);

  const config = useMemo(() => {
    if (!resolvedMonetizeKey) return null;
    return resolveMonetizeConfig(resolvedMonetizeKey, mergedLinks);
  }, [resolvedMonetizeKey, mergedLinks]);

  const hrefRaw = config?.primaryCta?.href;
  const href = isNonEmptyString(hrefRaw) ? ensureAmazonTag(hrefRaw) : "";
  ...
  const resolvedCtaId =
    isNonEmptyString(ctaId) ? ctaId : `${page_type}_${content_id}_${resolvedMonetizeKey ?? "cta"}_${position}`;
  ...
  useEffect(() => {
    if (!canTrack || !resolvedMonetizeKey) return;
    ...
    <Reveal delay={80}>
      ...
      >
        <GlassCard
          padding="lg"
          magnetic={false}
          className="border border-[rgba(31,28,25,0.12)] bg-[rgba(228,219,207,0.42)] shadow-soft-card"
        >
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <span
              aria-label="PR"
              className="inline-flex w-fit rounded-full border border-[var(--border-default)] bg-[rgba(228,219,207,0.42)] px-2 py-0.5 text-[10px] tracking-wide text-[var(--text-secondary)]"
            >
              PR
            </span>
            ...
            <div className="space-y-2.5 md:max-w-[70%]">
              ...
              <h2 className="font-serif text-[15px] font-semibold tracking-tight text-[var(--text-primary)] sm:text-[16px]">
                {config.heading}
              </h2>
              {config.body.map((paragraph, i) => (
                <p
                  key={i}
                  className="text-[11px] leading-relaxed text-[var(--text-secondary)] sm:text-[13px]"
                >
                  {paragraph}
                </p>
              ))}
              <p className="pt-1 text-[10px] leading-relaxed text-[var(--text-tertiary)]">
  ...
  switch (monetizeKey) {
    ...
    case "car_search_price":
      return {
        ...
        body: [
          "「安すぎる個体」には理由があります。まずは適正な相場を知ることで、失敗しない車選びが。",
        ],
        primaryCta: {
          label: "現在の中古車相場を見る",
          href: links.carSearchUrl ?? links.carSellIkkatsuUrl,
