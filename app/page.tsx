...
export default async function Home() {
  ...
  return (
    <main className="min-h-screen font-sans">
      ...
      {/* コンテンツエリア（ここから下は白背景＋グラデーション） */}
      <div className="bg-gradient-to-r from-[#D1F2EB] via-[#E8F8F5] to-white pt-16 pb-20">
        <div className="max-w-4xl mx-auto px-4">
          
          {/* セクションタイトル: Latest News */}
          <div className="flex items-center mb-8">
            <div className="h-px w-10 bg-[#0ABAB5] mr-4"></div>
            <h3 className="text-xl font-serif tracking-wider text-slate-600">LATEST NEWS</h3>
          </div>
            ...
            {latestNews.map((item, index) => (
              ...
              >
                ...
                <div className="flex items-center justify-between mb-4 text-sm text-slate-400">
                  ...
                  {item.publishedAt && (
                    <time className="font-light">{new Date(item.publishedAt).toLocaleDateString('ja-JP')}</time>
                  )}
                </div>
          ...
          <div className="mt-16 text-center">
            <Link 
              href="/news" 
              className="inline-block bg-white text-[#0ABAB5] px-10 py-4 rounded-full border-2 border-[#0ABAB5] hover:bg-[#0ABAB5] hover:text-white hover:shadow-lg transition-all duration-300 text-sm font-bold tracking-[0.2em]"
            >
              VIEW ALL NEWS
            </Link>
          </div>
      ...
      {/* フッター */}
      <footer className="bg-white text-center py-8 text-xs text-slate-400">
