...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const title = "COLUMN｜車のコラム";
  const description =
    "車選びや維持費、カスタムの考え方を、少し深く読み解く記事をまとめています。";
...
function mapCategoryLabel(category?: string | null): string {
  switch ((category ?? "").toUpperCase()) {
    ...
    case "SELL":
      return "売り方";
    default:
      return category ? String(category) : "コラム";
  }
}
...
export default async function ColumnArchivePage({ searchParams }: PageProps) {
  ...
  const legacyTagRaw = toSingle(resolvedSearchParams.tag);
  ...
  return (
    <main className="min-h-screen bg-[var(--paper)]">
      ...
      <section className="relative overflow-hidden bg-[linear-gradient(150deg,#E5604C,#C74B39)]">
        ...
        <div className="page-shell relative z-10 pb-12 pt-24 sm:pb-14">
          ...
          <p className="mt-5 max-w-[40rem] text-[14px] leading-[1.9] text-white/85 sm:text-[15px]">
            車選びや維持費、カスタムの考え方を、少し深く読み解く記事。背景にある仕組みまで、じっくりたどれます。
          </p>
            ...
            <section className="rounded-2xl border border-[rgba(31,28,25,0.10)] bg-[rgba(251,248,243,0.60)] px-4 py-3">
              ...
              <div className="grid gap-5 pt-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  >
                    <option value="">すべて</option>
                    {COLUMN_DISPLAY_TAGS.filter((t) => (displayTagCounts.get(t) ?? 0) > 0).map((t) => (
