...
const SUMMARY_ITEMS = [
  { label: "Ads", title: "広告掲載", body: "サイト運営のために広告配信を利用する場合があります" },
  { label: "Affiliate", title: "アフィリエイト", body: "リンク経由の成果報酬が発生する場合があります" },
  { label: "Editorial", title: "編集判断", body: "収益化と記事の判断は分けて扱います" },
] as const;
...
export default function AdsAffiliatePolicyPage() {
  const breadcrumbData = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "広告・アフィリエイト", item: `${getSiteUrl()}/legal/ads-affiliate-policy` },
    ],
  };
  ...
  return (
    <>
      ...
      <LegalDocHeader
        eyebrow="広告表示"
        title="収益化と編集判断を分ける"
        lead="CAR BOUTIQUE JOURNALは、サイト運営のために広告配信やアフィリエイトプログラムを利用することがあります"
        meta={[]}
      />
      ...
      <div className="mt-8 space-y-8">
        ...
        <div className="space-y-10">
          <LegalDocSection index="01" title="収益化の方法">
            <p>
              広告配信やリンク経由の成果報酬によって収益を得ることがあります
            </p>
            <p>
        ...
        <LegalRelatedLinks
          items={[
            { href: "/legal/editorial-policy", label: "編集方針", description: "記事制作と公開後の更新・修正の考え方" },
            { href: "/legal/sources-factcheck", label: "出典・ファクトチェック", description: "一次情報をどう優先し、どこまで確認して書くか" },
            { href: "/legal/disclaimer", label: "免責事項", description: "掲載情報の範囲と利用時の注意" },
            { href: "/contact", label: "お問い合わせ", description: "広告表記や掲載内容に関する連絡窓口" },
          ]}
        />
      </div>
    </>
