    ...
    <section className="py-12">
      <Reveal>
        <div className="mb-6 flex items-baseline justify-between">
          ...
          <Link
            href="/guide"
            className="text-[11px] font-medium text-[var(--text-tertiary)] transition-colors hover:text-[var(--accent-slate)]"
          >
            ガイド一覧へ
          </Link>
        </div>
          ...
          <Reveal key={guide.slug} delay={idx * 100}>
            <Link href={`/guide/${guide.slug}`} className="group block h-full">
              <GlassCard
                interactive
                magnetic={false}
                className="flex h-full flex-col justify-between border border-[var(--border-default)] bg-[var(--surface-2)] p-5 text-[var(--text-primary)] transition-all duration-300 hover:border-[rgba(122,135,108,0.3)]"
              >
                <div>
                  ...
                  <h3 className="serif-heading text-lg font-medium text-[var(--text-primary)] transition-colors group-hover:text-[var(--accent-slate)]">
                    {guide.title}
                  </h3>
                  {guide.summary && (
