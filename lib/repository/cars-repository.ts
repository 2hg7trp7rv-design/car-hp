...
import { readCarsJsonDir } from "./data-dir";
...
function coercePublicState(val: unknown, status: ContentStatus, noindex?: boolean): any {
  ...
  // - ここを noindex 既定にすると、新規記事JSONで publicState を入れ忘れた時に
  //   公開済み記事まで noindex になる。
...
function _toArray(data: unknown): RawCarRecord[] {
 ...
 */
const RAW_ALL: RawCarRecord[] = readCarsJsonDir();
