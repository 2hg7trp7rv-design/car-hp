 ...
 *
 * 役割:
 * - data/guides*.json から“生データ”を読み込む
...
import guidesRaw7 from "@/data/guides7.json"; // ★ 起爆剤追加分
...
import { loadMarkdownBodiesFromDir } from "@/lib/repository/content-files";



// ----------------------------------------
// Markdown bodies (content/guides/*.md) [optional]
// ----------------------------------------
const GUIDE_BODY_BY_SLUG = loadMarkdownBodiesFromDir("content/guides");
...
function normalizeGuide(raw: RawGuideRecord, index: number): GuideItem {
  ...
  // Prefer Markdown file body when exists: content/guides/<slug>.md
  const bodyFromFile = GUIDE_BODY_BY_SLUG.get(slug) ?? null;
  const body = (bodyFromFile ?? bodyFromJson).trim();
...
const RAW_ALL: RawGuideRecord[] = [
  ...
  ...toArray(guidesRaw7), // ★ ここで guides7.json を取り込む
];
