...
type VisualCard = { number?: string; title: string; body: string; avoid?: string; check?: string; category?: string };
type VisualChapter = { label: string; title: string; score?: string; body?: string; risks?: string[]; actionTitle?: string; action?: string; summary?: string; character?: string; dark?: boolean };
type VisualRisk = { title: string; body: string };
type VisualStep = { title: string; body: string };
type VisualLayout = {
  slug?: string;
  template?: string;
  hero?: { label?: string; category?: string; title?: string; subtitle?: string; lead?: string; score?: string; character?: string };
  intro?: { body?: string; important?: string; chips?: string[] };
  overview?: { label?: string; title?: string; body?: string; cards?: VisualCard[] };
  chapters?: VisualChapter[];
  check?: { label?: string; title?: string; body?: string; warning?: string; chips?: string[] };
  guide?: { label?: string; title?: string; items?: VisualCard[] };
  tips?: { label?: string; title?: string; body?: string; items?: VisualCard[] };
  darkRisk?: { label?: string; title?: string; score?: string; body?: string; risks?: VisualRisk[]; summary?: string; character?: string };
  steps?: { label?: string; title?: string; items?: VisualStep[] };
  mindset?: { label?: string; title?: string; body?: string; character?: string };
  summary?: { label?: string; title?: string; body?: string; pillars?: VisualCard[] };
};
...
function createAutoLayout(article: VisualArticle, labels: EditorialArticleLabels): VisualLayout {
  ...
  const overviewCards = (article.keyPoints?.length ? article.keyPoints : sections.map((section, index) => `${sectionTitle(section, index)}。${firstParagraph(section)}`)).slice(0, 3).map((body, index) => ({
...
function CharacterNote({ src, textValue, dark = false }: { src?: string; textValue?: string; dark?: boolean }) {
  ...
  return <aside className={cx(styles.characterNote, dark && styles.characterNoteDark)}>{src ? <Image className={styles.characterImg} src={src} alt="" width={120} height={180} /> : null}<div><span className={styles.characterKicker}>GUIDE</span><p className={styles.characterText}>{textValue}</p></div></aside>;
}
...
function Overview({ layout, linkIndex }: { layout: VisualLayout; linkIndex: Record<string, InternalLinkMeta> }) {
  ...
  return <section className={cx(styles.section, styles.sectionWhite)}><div className={styles.container}><SectionTitle label={overview.label} title={overview.title} body={overview.body} /><div className={styles.overviewGrid}>{overview.cards?.map((card, index) => <article className={styles.card} key={`${card.title}-${index}`}><span className={styles.cardNumber}>{card.number ?? String(index + 1).padStart(2, "0")}</span><h3 className={styles.cardTitle}>{card.title}</h3><Rich value={card.body} linkIndex={linkIndex} className={styles.cardBody} /></article>)}</div></div></section>;
}
...
export function VisualArticlePage({ article, labels, linkIndex }: VisualArticleProps) {
  ...
  return <main className={styles.articlePage} data-cbj-visual-article><Hero article={article} labels={labels} layout={layout} /><Intro layout={layout} linkIndex={linkIndex} /><Overview layout={layout} linkIndex={linkIndex} />{layout.chapters?.map((chapter, index) => <Chapter key={`${chapter.label}-${chapter.title}`} chapter={chapter} index={index} linkIndex={linkIndex} />)}<CheckSection layout={layout} linkIndex={linkIndex} /><GuideCompare layout={layout} /><TipsSection layout={layout} linkIndex={linkIndex} /><DarkRisk layout={layout} linkIndex={linkIndex} /><Steps layout={layout} /><Mindset layout={layout} /><Summary layout={layout} linkIndex={linkIndex} article={article} /><MetaSection article={article} labels={labels} related={related} /></main>;
}
