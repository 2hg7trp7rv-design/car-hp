...
import {
  DECISION_COLUMN_MIN,
  DECISION_GUIDE_MIN,
  countMarkdownHeadings as countDecisionMarkdownHeadings,
  getDecisionColumnAuditBody,
  getDecisionColumnStats,
  getDecisionGuideAuditBody,
  getDecisionGuideStats,
  getStructuredColumnAuditBody,
  getStructuredGuideAuditBody,
  isDecisionColumn,
  isDecisionGuide,
  isStructuredColumn,
  isStructuredGuide,
...
const quality = {
  ...
  GUIDE: (g) => {
    const body = isStructuredGuide(g) ? getStructuredGuideAuditBody(g) : asString(g.body);
    const bodyLen = body.length;
    const headings = countDecisionMarkdownHeadings(body);
    const meetsLength = bodyLen >= DECISION_GUIDE_MIN.bodyLen;
    const hasStructure = headings >= DECISION_GUIDE_MIN.headings;

    if (!isDecisionGuide(g)) {
      return {
    ...
    const stats = getDecisionGuideStats(g);
    const hasKeyPoints = stats.keyPointsCount >= DECISION_GUIDE_MIN.keyPoints;
    const hasCheckpoints = stats.checkpointsCount >= DECISION_GUIDE_MIN.checkpoints;
    const hasFaq = stats.faqCount >= DECISION_GUIDE_MIN.faq;
    const hasDetailSections = stats.detailSectionsCount >= DECISION_GUIDE_MIN.detailSections;
    const hasDetailBlocks = stats.detailBlocksCount >= DECISION_GUIDE_MIN.detailBlocks;
    const hasActionBox = Boolean(stats.actionTitle) && stats.actionLinksCount >= DECISION_GUIDE_MIN.actionLinks;
    ...
    return {
      ...
      metrics: {
        bodyLen,
        headings,
        decisionKeyPoints: stats.keyPointsCount,
        decisionCheckpoints: stats.checkpointsCount,
        decisionFaq: stats.faqCount,
        decisionSections: stats.detailSectionsCount,
        decisionBlocks: stats.detailBlocksCount,
        decisionActionLinks: stats.actionLinksCount,
      },
      checks: {
        meetsLength,
        hasStructure,
  ...
  COLUMN: (c) => {
    const body = isStructuredColumn(c) ? getStructuredColumnAuditBody(c) : asString(c.body);
    const bodyLen = body.length;
    const headings = countDecisionMarkdownHeadings(body);
    const meetsLength = bodyLen >= DECISION_COLUMN_MIN.bodyLen;
    const hasStructure = headings >= DECISION_COLUMN_MIN.headings;

    if (!isDecisionColumn(c)) {
      return {
        ok: meetsLength && hasStructure,
        metrics: { bodyLen, headings },
        checks: { meetsLength, hasStructure },
    ...
    const stats = getDecisionColumnStats(c);
    const hasKeyPoints = stats.keyPointsCount >= DECISION_COLUMN_MIN.keyPoints;
    const hasCheckpoints = stats.checkpointsCount >= DECISION_COLUMN_MIN.checkpoints;
    const hasFaq = stats.faqCount >= DECISION_COLUMN_MIN.faq;
    const hasDetailSections = stats.detailSectionsCount >= DECISION_COLUMN_MIN.detailSections;
    const hasDetailBlocks = stats.detailBlocksCount >= DECISION_COLUMN_MIN.detailBlocks;
    const hasActionBox = Boolean(stats.actionTitle) && stats.actionLinksCount >= DECISION_COLUMN_MIN.actionLinks;
    const hasSources = stats.sourcesCount >= DECISION_COLUMN_MIN.sources;
    ...
    return {
      ...
      metrics: {
        ...
        decisionBlocks: stats.detailBlocksCount,
        decisionActionLinks: stats.actionLinksCount,
        decisionSources: stats.sourcesCount,
      },
      checks: {
        meetsLength,
        hasStructure,
  ...
  HERITAGE: (h) => {
    ...
    const heritageType = asLower(h.heritageType);
    const pillarInsight = asString(h.pillarInsight);
    const hasStructure = chapters >= 3 || headings >= 4;
    const sourceCount = Array.isArray(h.sources) ? h.sources.filter(Boolean).length : 0;
    const relatedCars = Array.isArray(h.relatedCarSlugs) ? h.relatedCarSlugs.filter(Boolean).length : 0;
    const relatedHeritage = Array.isArray(h.relatedHeritageSlugs) ? h.relatedHeritageSlugs.filter(Boolean).length : 0;
    const relatedColumns = Array.isArray(h.relatedColumnSlugs) ? h.relatedColumnSlugs.filter(Boolean).length : 0;
    const meetsLength = bodyLen >= 4000;
    const hasSources = sourceCount >= 2;
    const hasInsight = pillarInsight.length >= 40;
    const hasType = ["hub", "thought", "era"].includes(heritageType);
    const hasRelated = heritageType === "hub"
      ? relatedCars >= 3 && relatedHeritage >= 1
      : heritageType === "thought"
        ? relatedCars >= 1
        : heritageType === "era"
          ? relatedCars >= 2 || relatedColumns >= 1
          : false;
    return {
      ok: meetsLength && hasStructure && hasSources && hasInsight && hasType && hasRelated,
      metrics: { bodyLen, chapters, headings, sourceCount, relatedCars, relatedHeritage, relatedColumns },
      checks: { meetsLength, hasStructure, hasSources, hasInsight, hasType, hasRelated },
    };
  },
};
...
const main = async () => {
  ...
  const items = all.map((item) => {
    ...
    // Banned chars
    const body =
      type === "GUIDE" && isStructuredGuide(item)
        ? getStructuredGuideAuditBody(item)
        : type === "COLUMN" && isStructuredColumn(item)
          ? getStructuredColumnAuditBody(item)
          : item.body;
    if (hasBannedChars(body)) {
      row.issues.push({ level: "error", code: "body:banned_chars", message: "body contains * or \"" });
    }

    if (type === "GUIDE" && isDecisionGuide(item)) {
      const stats = getDecisionGuideStats(item);
      const bodyText = getDecisionGuideAuditBody(item);
      const headingCount = countDecisionMarkdownHeadings(bodyText);

      if (bodyText.length < DECISION_GUIDE_MIN.bodyLen) {
        row.issues.push({
          level: "error",
          code: "decision:body_too_short",
          message: `decision-v1 content should be at least ${DECISION_GUIDE_MIN.bodyLen} chars after structured-body generation`,
        });
      }
      if (headingCount < DECISION_GUIDE_MIN.headings) {
        row.issues.push({
          level: "error",
          code: "decision:body_structure",
          message: `decision-v1 content should contain at least ${DECISION_GUIDE_MIN.headings} markdown headings after structured-body generation`,
        });
      }
      if (stats.keyPointsCount < DECISION_GUIDE_MIN.keyPoints) {
        row.issues.push({
          level: "error",
          code: "decision:key_points",
          message: `decision-v1 requires at least ${DECISION_GUIDE_MIN.keyPoints} keyPoints`,
        });
      }
      if (stats.checkpointsCount < DECISION_GUIDE_MIN.checkpoints) {
        row.issues.push({
          level: "error",
          code: "decision:checkpoints",
          message: `decision-v1 requires at least ${DECISION_GUIDE_MIN.checkpoints} checkpoints`,
        });
      }
      if (stats.faqCount < DECISION_GUIDE_MIN.faq) {
        row.issues.push({
          level: "error",
          code: "decision:faq",
          message: `decision-v1 requires at least ${DECISION_GUIDE_MIN.faq} faq item`,
        });
      }
      if (stats.detailSectionsCount < DECISION_GUIDE_MIN.detailSections) {
        row.issues.push({
          level: "error",
          code: "decision:detail_sections",
          message: `decision-v1 requires at least ${DECISION_GUIDE_MIN.detailSections} detailSections`,
        });
      }
      if (stats.detailBlocksCount < DECISION_GUIDE_MIN.detailBlocks) {
        row.issues.push({
          level: "error",
          code: "decision:detail_blocks",
          message: `decision-v1 requires at least ${DECISION_GUIDE_MIN.detailBlocks} detail blocks`,
        });
      }
      if (!stats.actionTitle || stats.actionLinksCount < DECISION_GUIDE_MIN.actionLinks) {
        row.issues.push({
          level: "error",
          code: "decision:action_box",
          message: "decision-v1 requires actionBox.title and at least one action link",
        });
      }
    }

    if (type === "COLUMN" && isDecisionColumn(item)) {
      const stats = getDecisionColumnStats(item);
      const bodyText = getDecisionColumnAuditBody(item);
      const headingCount = countDecisionMarkdownHeadings(bodyText);

      if (bodyText.length < DECISION_COLUMN_MIN.bodyLen) {
        row.issues.push({
          level: "error",
          code: "decision-column:body_too_short",
          message: `decision column content should be at least ${DECISION_COLUMN_MIN.bodyLen} chars after structured-body generation`,
        });
      }
      if (headingCount < DECISION_COLUMN_MIN.headings) {
        row.issues.push({
          level: "error",
          code: "decision-column:body_structure",
          message: `decision column content should contain at least ${DECISION_COLUMN_MIN.headings} markdown headings after structured-body generation`,
        });
      }
      if (stats.keyPointsCount < DECISION_COLUMN_MIN.keyPoints) {
        row.issues.push({
          level: "error",
          code: "decision-column:key_points",
          message: `decision column requires at least ${DECISION_COLUMN_MIN.keyPoints} keyPoints`,
        });
      }
      if (stats.checkpointsCount < DECISION_COLUMN_MIN.checkpoints) {
        row.issues.push({
          level: "error",
          code: "decision-column:checkpoints",
          message: `decision column requires at least ${DECISION_COLUMN_MIN.checkpoints} checkpoints`,
        });
      }
      if (stats.faqCount < DECISION_COLUMN_MIN.faq) {
        row.issues.push({
          level: "error",
          code: "decision-column:faq",
          message: `decision column requires at least ${DECISION_COLUMN_MIN.faq} faq item`,
        });
      }
      if (stats.detailSectionsCount < DECISION_COLUMN_MIN.detailSections) {
        row.issues.push({
          level: "error",
          code: "decision-column:detail_sections",
          message: `decision column requires at least ${DECISION_COLUMN_MIN.detailSections} detailSections`,
        });
      }
      if (stats.detailBlocksCount < DECISION_COLUMN_MIN.detailBlocks) {
        row.issues.push({
          level: "error",
          code: "decision-column:detail_blocks",
          message: `decision column requires at least ${DECISION_COLUMN_MIN.detailBlocks} detail blocks`,
        });
      }
      if (!stats.actionTitle || stats.actionLinksCount < DECISION_COLUMN_MIN.actionLinks) {
        row.issues.push({
          level: "error",
          code: "decision-column:action_box",
          message: "decision column requires actionBox.title and at least one action link",
        });
      }
      if (stats.sourcesCount < DECISION_COLUMN_MIN.sources) {
        row.issues.push({
          level: "error",
          code: "decision-column:sources",
          message: `decision column requires at least ${DECISION_COLUMN_MIN.sources} sources`,
        });
      }
    }
    ...
    if (state === "index") {
      ...
      if (!q.ok) {
        row.issues.push({
          level: STRICT_QUALITY ? "error" : "warning",
          code: STRICT_QUALITY ? "quality:strict_fail" : "quality:fail",
          message: STRICT_QUALITY
            ? "publicState=index but quality gate failed under CBJ_STRICT_QUALITY=1"
            : "publicState=index but quality gate failed; index is still allowed",
        });
      }
    ...
    // Collect report issues. Quality warnings do not block build/indexing.
    for (const issue of row.issues) {
      if (issue.level === "error") errors.push({ ...row, code: issue.code, message: issue.message });
      if (issue.level === "warning") warnings.push({ ...row, code: issue.code, message: issue.message });
    }
  ...
  // Write to generated module. Public audit output is disabled in production by default.
  const shouldWritePublicAudit =
    process.env.CBJ_WRITE_PUBLIC_AUDIT === "1" &&
    String(process.env.VERCEL_ENV || "").toLowerCase() !== "production";

  if (shouldWritePublicAudit) {
    ...
    await fs.promises.rm(path.join(ROOT, "public", "_internal", "audit.json"), { force: true });
  }
