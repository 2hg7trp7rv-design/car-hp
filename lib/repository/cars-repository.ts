 ...
 *
 * 役割:
 * - data/cars*.json をメタデータの正として読み込む
 * - JSONの揺れ・欠損を吸収し、統一された CarItem 型として提供する
 * - 長文本文は content/cars/<slug>.md（任意）で補完できる
 */
...
const CAR_BODY_BY_SLUG = loadMarkdownBodiesFromDir("content/cars");
...
function normalizeCar(raw: RawCarRecord, index: number): CarItem {
  ...
  const bodyFromFile = CAR_BODY_BY_SLUG.get(slug) ?? null;
