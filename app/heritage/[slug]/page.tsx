...
export default async function HeritageDetailPage({ params }: PageProps) {
  ...
  if (formattedBodyText) {
    const originalLines = formattedBodyText.split(/\r?\n/);
    const trimmedLines = originalLines.map((line) => line.trim());

    // まず Markdown の `##` 見出しで分割（本文が Markdown 構造のときにカード化が安定する）
    const hasMarkdownH2 = trimmedLines.some((line) => /^##\s+/.test(line));

    const lines = hasMarkdownH2 ? originalLines : trimmedLines;
    ...
    for (const rawLine of lines) {
      ...
      if (hasMarkdownH2) {
        ...
        if (mdH2) {
          const headingTitle = mdH2[1]?.trim() ?? "";
          if (headingTitle) {
            pushCurrent();
            current = {
              title: headingTitle,
  ...
  const structuredSections: BodySection[] =
    Array.isArray((heritage as any).sections) && (heritage as any).sections.length > 0
      ? (heritage as any).sections.map((sec: any) => {
          // NOTE:
          // - 章カード内の読みやすさ重視。
          // - 文章を「1文=1段落」にしない（細切れになる）
          // - 2連改行（空行）を段落区切りとして扱い、1段落をある程度の文量で保つ。
  ...
  // v1.2+:
  // - 本文(body)に「【見出し】」「■見出し」が入っていない長文（Markdown見出し中心）の場合、
  //   rawSections が1つに潰れがちで「章ごとカード化」ができない。
  // - その場合は JSON の sections を本文として採用し、章カード+章末CTAを安定表示する。
  const hasParsableBodySections =
    mergedBodySections.length > 1 ||
    (mergedBodySections.length === 1 &&
      (typeof mergedBodySections[0]?.title === "string" && mergedBodySections[0].title.length > 0));

  const contentSections: BodySection[] =
    structuredSections.length > 0 && !hasParsableBodySections
      ? structuredSections
