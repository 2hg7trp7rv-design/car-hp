...
export const metadata: Metadata = {
  ...
  alternates: { canonical: `${getSiteUrl()}/guide/insurance` },
};
...
export default async function InsuranceHubPage() {
  ...
  return (
    <main className="relative min-h-screen bg-[var(--bg-stage)] text-[var(--text-primary)] pb-20 pt-24">
      <DetailFixedBackground />
      <ScrollDepthTracker />
      <div className="porcelain porcelain-panel mx-auto max-w-6xl px-4 sm:px-6 rounded-[20px] border border-[var(--border-default)] bg-[rgba(251,248,243,0.92)] text-[var(--text-primary)] shadow-soft-card backdrop-blur">
        <div className="lg:flex lg:items-start lg:gap-10">
          <div className="lg:flex-1">
            <Reveal delay={80}>
              <header className="mb-8">
                <h1 className="serif-heading mt-3 text-2xl text-[var(--text-primary)] sm:text-3xl">
                  自動車保険の選び方・見直しガイド
                </h1>
                <p className="mt-3 max-w-2xl text-[12px] leading-relaxed text-[rgba(76,69,61,0.82)] sm:text-[14px]">
                  補償条件を整理してから比較すると、より適切なプランを選びやすくなります。
                </p>
              </header>
            </Reveal>
            ...
            <section id="entry" className="mt-12 scroll-mt-24">
              <Reveal delay={160}>
                <h2 className="serif-heading text-xl text-[var(--text-primary)]">見積もりを始める</h2>
            ...
            <section className="mt-12">
              <Reveal delay={180}>
                <FaqList
                  title="自動車保険のよくある質問"
                  description="見積もりに進む前に、迷いがちな論点を見ます。"
                  items={[
                    {
                      q: "車両保険は付けるべき？",
              ...
              <section className="mt-12">
                <div className="mb-4 flex flex-wrap items-baseline justify-between gap-3">
                  <div>
                    ...
                    <h2 className="serif-heading mt-2 text-lg text-[var(--text-primary)] sm:text-xl">
                      さらに読む（必要なときだけ）
                    </h2>
                  </div>
                  <Link
                    href="/guide"
                    className="text-[11px] font-semibold tracking-[0.18em] text-[rgba(76,69,61,0.82)] hover:text-[var(--text-primary)]"
                  >
                    ガイド一覧へ →
                  </Link>
                </div>
                  ...
                  {picks.map((guide, index) => (
                    <Reveal key={guide.id} delay={260 + index * 40}>
                      <Link href={`/guide/${encodeURIComponent(guide.slug)}`}>
                        <GlassCard className="group h-full border border-[var(--border-default)] bg-[var(--surface-2)] p-4 text-xs shadow-soft transition hover:-translate-y-[1px] hover:border-accent-100 hover:shadow-soft-card sm:p-5">
                          <h3 className="line-clamp-2 text-[13px] font-semibold leading-relaxed text-[var(--text-primary)] group-hover:text-[var(--accent-slate)]">
                            {guide.title}
                          </h3>
                          {guide.summary && (
                            <p className="mt-2 line-clamp-3 text-[11px] leading-relaxed text-[var(--text-secondary)]">
                              {guide.summary}
                            </p>
                          )}
                        </GlassCard>
