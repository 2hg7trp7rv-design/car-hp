...
export default async function LessonPage({params}:Props){const p=await params,c=getLearningCourse(p.course),lessonIndex=c?.lessons.findIndex(l=>l.slug===p.lesson)??-1;if(!c||lessonIndex<0)notFound();const l=c.lessons[lessonIndex],previous=c.lessons[lessonIndex-1],next=c.lessons[lessonIndex+1];const review=c.lessons.find(r=>r.slug===l.checkpoint.review);const sources=c.sources.filter(s=>l.sources.includes(s.id));
 ...
 <header className={styles.lessonHead}><div className={styles.lessonMeta}><span className={styles.badge} data-stage={l.stage}>{LEARNING_STAGES[l.stage].label}</span><span>{LEARNING_TOPICS[c.topic].title}</span><span>全{c.lessons.length}回のうち第{lessonIndex+1}回</span></div><h1>{l.title}</h1><p className={styles.goal}><strong>今日のゴール</strong>{l.goal}</p>
 ...
 <div className={styles.lessonLayout}><article className={styles.lessonBody} aria-label="会話と図解のレッスン">
 {teachingGroups(l.blocks).map((group,i)=><section key={i} className={`${styles.teachingGroup} ${group.visual&&group.talk.length?styles.paired:''}`} aria-label={`会話と図解 ${i+1}`}>{group.talk.length?<div className={styles.conversation}>{group.talk.map((block,j)=><LessonBlock key={j} block={block}/>)}</div>:null}{group.visual?<LessonBlock block={group.visual}/>:null}</section>)}
 ...
 <section className={styles.sources}><h2>この回の出典・参考資料</h2><ul>{sources.map(s=><li key={s.id}><a href={s.url} rel="noopener noreferrer" target="_blank">{s.title} <span aria-label="新しいタブで開く">↗</span></a><p>{s.note}</p></li>)}</ul></section>
