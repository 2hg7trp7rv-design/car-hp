 ...
 *
 * 役割:
 * - data/articles/heritage/*.json の生データを読み込み HeritageItem に正規化
 * - JSONの揺れ（bodyの欠損をlead/summaryで補完、carSlugsの継承など）をこの層で吸収
 */

import { readHeritageJsonDir } from "./data-dir";
...
// ----------------------------------------
// Markdown bodies (content/heritage/*.md) [optional]
// ----------------------------------------
...
function _toArray(data: unknown): RawHeritageRecord[] {
...
function normalizePublicState(v: unknown, status: ContentStatus): PublicState {
  const s = safeString(v)?.toLowerCase();
  if (s === "index" || s === "noindex" || s === "draft" || s === "redirect") {
    return s as PublicState;
  }
...
function normalizeHeritage(raw: RawHeritageRecord, index: number): HeritageItem {
  ...
  const bodyFromJson = safeString(raw.body) ?? safeString(raw.lead) ?? safeString(raw.summary) ?? "";
  ...
  return {
    ...
    type: "HERITAGE",
    status,
    publicState: normalizePublicState(raw.publicState, status),
    parentPillarId: safeString(raw.parentPillarId) ?? "/heritage",
    relatedClusterIds: toStringArray(raw.relatedClusterIds),
    primaryQuery: safeString(raw.primaryQuery) ?? title,
    updateReason: safeString(raw.updateReason) ?? "initial-import",
    ...
    displayTag: safeString(raw.displayTag),
    ...
    // repository層で落とすと detail 側が body fallback に戻り、
    // Hero / Timeline など制作上の見出しが本文として露出する。
    cinematicHeritage: raw.cinematicHeritage,
 ...
 */
const RAW_ALL: RawHeritageRecord[] = readHeritageJsonDir();
