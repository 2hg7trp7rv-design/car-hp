...
export default function ColumnError({ error, reset }: ErrorPageProps) {
  useEffect(() => {
  ...
  return (
    <main className="relative min-h-screen text-white">
      ...
      <div className="page-shell pb-24 pt-24">
        <StatusPanel
          kicker="ERROR / COLUMN"
          title="COLUMNでエラーが発生しました"
          lead="一時的な不具合の可能性があります。もう一度試しても解消しない場合は、時間をおいて再度アクセスしてください。"
          variant="column"
          seedKey={error?.digest ? `error:column:${error.digest}` : "error:column"}
          note={note}
        >
          <button
            type="button"
