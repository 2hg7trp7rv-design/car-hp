...
function coerceCtaVariants(value: unknown): CtaVariant[] | null {
  ...
  for (const item of value) {
    ...
    const monetizeKey = coerceString(raw.monetizeKey) as MonetizeKey | null;
    const title = coerceString(raw.title);
    const lead = coerceString(raw.lead);
    const ctaLabel = coerceString(raw.ctaLabel);

    const whenIntentTagsAny = Array.isArray(raw.whenIntentTagsAny)
      ? (raw.whenIntentTagsAny
          .map((v) => (typeof v === "string" ? v.trim() : ""))
          .filter((v) => v.length > 0) as string[])
      : null;

    const priority =
      typeof raw.priority === "number" ? raw.priority : null;
