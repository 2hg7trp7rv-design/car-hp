  ...
  return (
    <main className="relative min-h-screen text-white">
      ...
      <div className="page-shell pb-24 pt-24">
        <div className="porcelain porcelain-panel overflow-hidden rounded-3xl border border-[#222222]/10 bg-white/92 text-[#222222] shadow-soft-card backdrop-blur">
      {/* 上部ヒーロー＋パンくず */}
      <section className="border-b border-[#222222]/10 bg-gradient-to-b from-slate-50 to-slate-100">
        <div className="mx-auto flex max-w-5xl flex-col gap-6 px-4 py-8 md:py-10">
          <Reveal>
            <Breadcrumb items={[{ label: "HOME", href: "/" }, { label: "NEWS", href: "/news" }, { label: "DETAIL" }]} />
          </Reveal>

          <Reveal>
            <div className="flex flex-col gap-4 md:flex-row md:items-end">
              <div className="flex-1 space-y-3">
                <p className="text-[11px] uppercase tracking-[0.28em] text-[#222222]/55">
                  EDITOR&apos;S PICK NEWS
                </p>
                <h1 className="text-xl font-semibold tracking-wide text-[#222222] md:text-2xl">
                  {titleJa}
                </h1>
                {leadExcerpt && (
                  <p className="text-sm leading-relaxed text-[#222222]/70">
                    {leadExcerpt}
                  </p>
                )}
              </div>
              <div className="flex flex-col items-start gap-2 text-xs text-[#222222]/55 md:items-end">
                {dateLabel && (
                  <p className="font-medium text-[#222222]/70">{dateLabel}</p>
                )}
                <div className="flex flex-wrap gap-1">
                  {news.maker && (
                    <span className="rounded-full border border-[#222222]/16 px-2.5 py-0.5 text-[11px] uppercase tracking-[0.16em] text-[#222222]/80">
                      {news.maker}
                    </span>
                  )}
                  {news.category && (
                    <span className="rounded-full border border-[#222222]/16 px-2.5 py-0.5 text-[11px] tracking-[0.08em] text-[#222222]/80">
                      {news.category}
                    </span>
                  )}
                  {sourceLabel && (
                    <span className="rounded-full border border-[#222222]/16 px-2.5 py-0.5 text-[11px] tracking-[0.08em] text-[#222222]/70">
                      SOURCE {sourceLabel}
                    </span>
                  )}
                </div>
      ...
      {/* 本文＋サイド */}
      <section className="border-b border-[#222222]/10 bg-[#222222]/05/80">
          ...
          <div className="space-y-6">
            <GlassCard className="bg-white/90">
              <div className="space-y-6 p-4 md:p-6">
                {news.imageUrl && (
                  <div className="aspect-[16/9] w-full overflow-hidden rounded-xl bg-[#222222]/05">
                ...
                {news.excerpt && (
                  <section className="space-y-2">
                    <h2 className="text-sm font-semibold tracking-[0.12em] text-[#222222]/55">
                      ニュース概要
                    </h2>
                    <p className="whitespace-pre-line text-sm leading-relaxed text-[#222222]/80">
                      {news.excerpt}
                    </p>
                  </section>
                )}
                ...
                {/* 編集部コメント（commentJa） */}
                {news.commentJa && (
                  <section className="space-y-2 border-t border-[#222222]/08 pt-4">
                    <h2 className="text-sm font-semibold tracking-[0.12em] text-[#222222]/55">
                      CAR BOUTIQUE JOURNAL編集部コメント
                    </h2>
                    <div className="space-y-3 whitespace-pre-line text-[13px] leading-relaxed text-[#222222]/80 md:text-sm">
                      {news.commentJa}
                    </div>
                  </section>
                )}
                ...
                {longformSections.length > 0 && (
                  <section className="space-y-6 border-t border-[#222222]/08 pt-4">
                    {longformSections.map((sec, idx) => (
                      <div
                        ...
                        className={
                          idx === 0
                            ? "space-y-2"
                            : "space-y-2 border-t border-[#222222]/08 pt-4"
                        }
                      >
                        <h2 className="text-sm font-semibold tracking-[0.12em] text-[#222222]/55">
                          {sec.title}
                        ...
                        {Array.isArray(sec.paragraphs) && sec.paragraphs.length > 0 && (
                          <div className="space-y-3">
                            {sec.paragraphs.map((p, i) => (
                              <p
                                key={`${sec.id}-p-${i}`}
                                className="whitespace-pre-line text-[13px] leading-relaxed text-[#222222]/80 md:text-sm"
                              >
                                {p}
                              </p>
                            ))}
                        ...
                        {Array.isArray(sec.bullets) && sec.bullets.length > 0 && (
                          <ul className="list-disc space-y-1.5 pl-5 text-[13px] leading-relaxed text-[#222222]/80 md:text-sm">
                            {sec.bullets.map((b, i) => (
                              <li
                                key={`${sec.id}-b-${i}`}
                ...
                {/* 元記事リンク＋出典 */}
                {news.url && (
                  <section className="border-t border-[#222222]/08 pt-4 text-xs">
                    <p className="mb-1 text-[11px] font-medium tracking-[0.12em] text-[#222222]/55">
                      元記事・出典
                    </p>
                    <a
                      href={news.url}
                      ...
                      className="cb-tap inline-flex items-center gap-2 rounded-full bg-black/80 px-4 py-2 text-[11px] font-medium text-white/85 underline-offset-4 hover:bg-black/70"
                    >
                      元記事を開く
                      <span className="text-[10px]">↗</span>
                    </a>
                    {sourceLabel && (
                      <p className="mt-1 text-[11px] text-[#222222]/55">
                        出典:{sourceLabel}
                      </p>
                    )}
                  </section>
                ...
                {/* タグ */}
                {tags.length > 0 && (
                  <section className="border-t border-[#222222]/08 pt-4">
                    <p className="mb-1 text-[11px] font-medium tracking-[0.16em] text-[#222222]/55">
                      TAGS
                    </p>
                    <div className="flex flex-wrap gap-1.5">
                      {tags.map((tag) => (
                        <Link
                          ...
                          rel="nofollow"
                          className="cb-tap inline-flex items-center rounded-full border border-[#222222]/16 bg-[#222222]/03 px-3 py-2 text-[11px] text-[#222222]/80 hover:border-tiffany-400 hover:bg-white"
                        >
                          #{tag}
                        </Link>
                      ))}
            ...
            <div className="flex justify-start pt-2">
              <Link
                href="/news"
                className="cb-tap inline-flex items-center gap-2 px-2 text-xs font-medium text-[#222222]/70 underline-offset-4 hover:text-[#222222] hover:underline"
              >
                <span className="flex h-6 w-6 items-center justify-center rounded-full border border-[#222222]/16 text-[11px]">
                  ←
                </span>
                NEWS一覧に戻る
              </Link>
              ...
              >
                <GlassCard className="bg-white/90">
                  <div className="space-y-3 p-4 md:p-5">
                    <h2 className="text-xs font-semibold tracking-[0.16em] text-[#222222]/55">
                      CAR BOUTIQUE JOURNAL LINKS
                    </h2>
                    ...
                    {recommended.cars.length > 0 && (
                      <div className="space-y-2">
                        <p className="text-[11px] font-medium tracking-[0.14em] text-[#222222]/55">
                          CARS
                        </p>
                        <div className="space-y-2">
                          {recommended.cars.map((car) => (
                            <TrackedLink
                              ...
                              toId={car.slug}
                              shelfId="news_recommendations"
                              className="cb-tap block rounded-lg border border-[#222222]/10 bg-white/70 p-3 text-xs text-[#222222]/85 transition hover:border-tiffany-300 hover:bg-white"
                            >
                              ...
                              <div className="flex flex-wrap items-center justify-between gap-1">
                                ...
                                {car.publishedAt && (
                                  <p className="text-[11px] text-[#222222]/45">
