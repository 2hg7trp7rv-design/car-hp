...
function toggle(setter: (_nextValue: string[]) => void, values: string[], value: string): void {
  const v = value.trim();
  if (!v) return;
...
function BudgetPresets({ onPick }: { onPick: (_value: number) => void }) {
  ...
  return (
    <div className="grid gap-6 p-6 lg:grid-cols-[360px_1fr]">
      ...
      <aside className="space-y-4">
        <GlassCard padding="md" className="bg-[var(--surface-1)]" magnetic={false}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-semibold tracking-[0.22em] text-[rgba(107,101,93,0.88)]">
                INPUT
              </p>
              <h2 className="mt-2 font-serif text-[18px] font-semibold tracking-tight text-[var(--text-primary)]">
                条件
              </h2>
              <p className="mt-2 text-[12px] leading-relaxed text-[rgba(76,69,61,0.88)]">
          ...
          <div className="mt-6 space-y-6">
            {/* Budget */}
            <div>
              <label className="block text-[12px] font-semibold tracking-[0.12em] text-[var(--text-primary)]">
                予算（中古の上限）
              </label>
              <p className="mt-2 text-[12px] text-[rgba(76,69,61,0.88)]">
                上限: <span className="font-semibold text-[var(--text-primary)]">{budgetMaxMan}万円</span>
              </p>
              <input
                type="range"
                min={100}
            ...
            <div>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                <Button
                  ...
                  className="h-11 rounded-[20px] text-[12px]"
                >
                  Balanced
                </Button>
                <Button
              ...
              <p className="mt-3 text-[11px] leading-relaxed text-[rgba(76,69,61,0.88)]">
      ...
      <section className="space-y-4">
        <GlassCard padding="md" className="bg-[var(--surface-1)]" magnetic={false}>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-[12px] leading-relaxed text-[rgba(76,69,61,0.88)]">
                今の条件で <span className="font-semibold text-[var(--text-primary)]">{scored.length}</span> 件。
        ...
        {scored.length === 0 && (
          <GlassCard padding="md" className="bg-[var(--surface-1)]" magnetic={false}>
            <p className="text-[12px] leading-relaxed text-[rgba(76,69,61,0.88)]">
        ...
        <GlassCard padding="md" className="bg-[var(--surface-1)]" magnetic={false}>
          <h3 className="text-[12px] font-semibold tracking-[0.12em] text-[var(--text-primary)]">次にやること</h3>
          <ul className="mt-3 space-y-2 text-[12px] leading-relaxed text-[rgba(76,69,61,0.88)]">
            <li>1) 候補を3〜10台まで落とす（このページ）</li>
            <li>2) 2〜3台まで絞って車種比較で並べる</li>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <Button asChild variant="outline" size="sm" magnetic className="h-11 rounded-full px-4">
              <a href="/guide/hub-usedcar">中古車の探し方</a>
            </Button>
            <Button asChild variant="outline" size="sm" magnetic className="h-11 rounded-full px-4">
              <a href="/cars">車種を見る</a>
            </Button>
    ...
    <div className="relative">
      ...
      >
        <GlassCard
          ...
          interactive
          magnetic={false}
          className="h-full border border-[var(--border-default)] bg-[var(--surface-2)] text-[var(--text-primary)]"
        >
          <div className="flex flex-col gap-3">
            {image && (
              <div className="overflow-hidden rounded-[20px] border border-[var(--border-default)] bg-[var(--surface-2)]">
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                {car.maker && (
                  <p className="text-[10px] font-semibold tracking-[0.18em] text-[rgba(107,101,93,0.88)]">
                    {car.maker}
                  </p>
                )}
                <h3 className="mt-1 line-clamp-2 font-serif text-[18px] font-semibold tracking-tight text-[var(--text-primary)]">
                  {car.name}
                </h3>
              </div>

              <div className="flex flex-col items-end gap-1">
                <p className="text-[10px] font-semibold tracking-[0.18em] text-[rgba(107,101,93,0.88)]">評価</p>
                <p className="text-[20px] font-semibold text-[var(--accent-base)]">{item.score}</p>
              </div>
            </div>
            ...
            {item.badges.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {item.badges.map((b) => (
                  <span
                    key={b}
                    className="rounded-full border border-[var(--border-default)] bg-[rgba(122,135,108,0.1)] px-3 py-1 text-[10px] font-semibold tracking-[0.14em] text-[var(--accent-strong)]"
                  >
                    {b}
                  </span>
                ))}
            ...
            {item.reasons.length > 0 && (
              <ul className="space-y-1 text-[12px] leading-relaxed text-[rgba(76,69,61,0.88)]">
                {item.reasons.map((r) => (
                  <li key={r}>• {r}</li>
                ))}
              </ul>
            ...
            {item.flags.length > 0 && (
              <div className="rounded-[20px] border border-amber-200/60 bg-amber-50/70 p-3">
            ...
            <p className="text-[11px] leading-relaxed text-[rgba(76,69,61,0.88)]">
