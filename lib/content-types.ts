...
export type ArticleBlockPresentation = {
  /**
   * 同じ意味ブロックから選ぶ見た目。
   * COLUMN/GUIDE共通の部品箱では、soft/outline/emphasis/compact/lead/sectionに加えて
   * summaryCard / infoBox / systemCards / dataTable / checklistCard / numberedSteps / stepCards
   * などの読みやすいaliasも指定できる。未知値は標準表示へフォールバックする。
   */
...
export type ArticleDesignSpec = {
  ...
  /** cbj-shared-editorial-v1 を指定すると、COLUMN/GUIDE共通の記事部品箱を使う。column-renewal-v1 は互換alias。 */
  layoutPreset?: "cbj-shared-editorial-v1" | "column-renewal-v1" | string | null;
