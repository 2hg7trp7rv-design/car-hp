...
## Sitemap / Robots（現状）
### sitemap の入口
- `https://carboutiquejournal.com/sitemap`（200）
- `https://carboutiquejournal.com/sitemap.xml`（301 → /sitemap）
  - sitemap は **/sitemap を正**にしています（重複URLのノイズを減らす目的）。
...
- `/sitemaps/sitemap-static`
- `/sitemaps/sitemap-cars`
- `/sitemaps/sitemap-makers`
- `/sitemaps/sitemap-body-types`
- `/sitemaps/sitemap-segments`
- `/sitemaps/sitemap-guides`
- `/sitemaps/sitemap-columns`
- `/sitemaps/sitemap-heritage`

補足:
- `/sitemaps/sitemap-news` は現状 **空**です。
...
- `/sitemap` と `/sitemap.xml` が **200** で返る
...
- sitemap / robots を Route Handler で返す（環境差で 4xx になるのを回避）
