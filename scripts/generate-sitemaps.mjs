...
const main = async () => {
  ...
  // Exhibition curated route pages are few and stable (data-driven).
  // Treat them as static for sitemap purposes.
  const exhibitRoutes = await readJson("data/exhibit/routes.json");
  if (Array.isArray(exhibitRoutes)) {
    for (const r of exhibitRoutes) {
      const id = safeString(r?.id);
      if (!id) continue;
      staticPaths.push(`/exhibition/route/${encodeURIComponent(id)}`);
    }
  }
