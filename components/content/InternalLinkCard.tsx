...
function worldAccent(kind: InternalLinkKind): string {
  switch (kind) {
    case "GUIDE":
      return "border-l-[var(--teal)] bg-white hover:border-[var(--teal)]";
    case "COLUMN":
      return "border-l-[var(--coral)] bg-white hover:border-[var(--coral)]";
    default:
      return "border-l-[var(--coral)] bg-white hover:border-[var(--coral)]";
  }
}
...
function worldBadge(kind: InternalLinkKind): string {
  switch (kind) {
    case "GUIDE":
      return "bg-[var(--surface-moss)] text-[var(--teal-deep)]";
    case "COLUMN":
      return "bg-[var(--surface-clay)] text-[var(--coral-deep)]";
    default:
      return "bg-[var(--surface-clay)] text-[var(--coral-deep)]";
  }
}
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span
            className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[var(--surface-clay)] text-[16px] text-[var(--coral)] transition group-hover:translate-x-[2px] group-hover:bg-[var(--coral)] group-hover:text-white"
            aria-hidden
          >
            →
          </span>
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        "hover:-translate-y-[1px] hover:border-[rgba(14,124,123,0.32)] hover:bg-[rgba(251,248,243,0.96)]",
        className,
      )}
    >
