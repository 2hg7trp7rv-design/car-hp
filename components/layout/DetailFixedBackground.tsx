...
export function DetailFixedBackground(
  ...
  const safeDesktop = desktopRaw.startsWith("/") ? desktopRaw : "";
  const safeMobile = mobileRaw.startsWith("/") ? mobileRaw : "";

  const seed = String(props.seed ?? "").trim();
  const kv = seed ? pickExhibitKvPaths(seed) : null;

  const desktopSrc = safeDesktop || safeMobile || kv?.desktop || "/images/hero-top-desktop.jpeg";
  const mobileSrc = safeMobile || safeDesktop || kv?.mobile || "/images/hero-top-mobile.jpeg";
