...
export default async function CarDetailPage({ params }: PageProps) {
  ...
  const difficultyText: string | null = difficultyLabel
  ...
  const externalSecondaryKey: MonetizeKey = "ins_compare";

  const ctaPrimary = getMonetizeConfig(externalPrimaryKey);
  const ctaSecondary = getMonetizeConfig(externalSecondaryKey);

  const externalQuickKeys: MonetizeKey[] = (["loan_precheck", "sell_price_check"] as MonetizeKey[]).filter(
                ...
                <div className="mt-4 flex flex-col gap-1 text-[11px] text-white/75">
                  {car.priceNew && (
                    <p className="inline-flex w-fit rounded-full bg-white/10 px-3 py-1 ring-1 ring-white/10">
                      新車：参考価格:{car.priceNew}
                    </p>
                  )}
                  {car.priceUsed && (
                    <p className="inline-flex w-fit rounded-full bg-white/10 px-3 py-1 ring-1 ring-white/10">
                      中古：参考相場:{car.priceUsed}
                    </p>
                  )}
                </div>
              )}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {externalQuickKeys.slice(0, 3).map((k) => {
                      ...
                      return (
                        ...
                        >
                          {k === "loan_precheck" ? "ローン比較" : "売却する"} <span aria-hidden>→</span>
                        </a>
                      );
                    })}
                  </div>
