...
function hasTableTitle(article, title) {
  return asArray(article.detailSections)
    .flatMap((section) => asArray(section?.blocks))
    .some((block) => block?.type === "comparisonTable" && String(block?.title ?? "").trim() === title);
}
...
for (const relativeFile of TARGETS) {
  ...
  if (isGuideArticle) {
    ...
    const expectedMainUi = CHILD_GUIDE_REQUIRED_MAIN_UI.get(String(article.slug ?? ""));
    if (expectedMainUi && !hasTableTitle(article, expectedMainUi)) {
      fail(relativeFile, `記事別の主役UI「${expectedMainUi}」が必要です`);
    }
