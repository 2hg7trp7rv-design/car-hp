    ...
    >
      <div className="flex h-full flex-col gap-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="shrink-0 rounded-[20px] border border-[rgba(122,135,108,0.18)] bg-[var(--accent-subtle)] px-4 py-3 text-right">
          ...
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {samples.slice(0, isFeature ? 5 : 4).map((sample) => (
                <span
                  key={sample}
                  className="rounded-full border border-[rgba(31,28,25,0.08)] bg-[var(--surface-2)] px-3 py-1 text-[10px] tracking-[0.14em] text-[var(--text-secondary)]"
                >
                  {sample}
                </span>
              ))}
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {meta.slice(0, 4).map((item) => (
              <div
                ...
                className="rounded-[18px] border border-[rgba(31,28,25,0.08)] bg-[rgba(251,248,243,0.6)] px-4 py-3"
              >
                <div className="text-[10px] font-semibold tracking-[0.18em] text-[var(--text-tertiary)]">
                  {item.label}
                </div>
