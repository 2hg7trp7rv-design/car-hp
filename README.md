...
- COLUMN / GUIDE 記事デザインシステム：`docs/CBJ_ARTICLE_DESIGN_SYSTEM.md`
- 最新実装記録：`docs/CBJ_ARTICLE_DESIGN_SYSTEM_IMPLEMENTATION_6.md`
- 記事ブロック仕様：`docs/article-redesign/json-blocks.md`
- 開発用表示カタログ：`/article-system-preview`（developmentのみ。productionでは404）
...
- スマホ（GitHub UI）運用前提のため、本文は **Markdownファイルを増やさず** `data/articles/**.json` で完結させます。
- 新しいCOLUMN / GUIDE記事は `detailSections` を本文と表示構成の正本とし、旧 `body` との二重管理は行いません。
