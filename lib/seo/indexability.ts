...
export function evaluateCarIndexability(car: CarItem): IndexabilityResult {
  ...
  const hasMaintenanceCost = Boolean(car.maintenanceSimulation?.yearlyRoughTotal) || Boolean(car.costImpression) || (typeof car.maintenanceCostYenPerYear === "number" && Number.isFinite(car.maintenanceCostYenPerYear));
