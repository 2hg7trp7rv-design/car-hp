...
export type EditorialVariant = "guide" | "column" | "generic";
...
const PLACEHOLDER_ASSET_FILENAMES = new Set([
  "ogp-default.jpg",
  "ogp-default.png",
  "hero_default.jpg",
  "hero_default.png",
...
export function isEditorialPlaceholderAsset(value: string): boolean {
  ...
  const cleanPath = v.split(/[?#]/)[0] ?? v;
  const filename = cleanPath.split("/").filter(Boolean).pop() ?? cleanPath;
  return PLACEHOLDER_ASSET_FILENAMES.has(filename);
}
