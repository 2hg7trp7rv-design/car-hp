...
export default async function HeritageDetailPage({ params }: PageProps) {
  ...
  // 表示は「本文」を優先（sections は章リンク棚のために保持）
  const shouldPreferBody =
    hasBody &&
    bodyText.trim().length > 0 &&
    bodyText.trim() !== (heritage.summary ?? "").trim() &&
    bodyText.trim().length >= 800;

  const baseSections =
    shouldPreferBody ? bodySections : structuredSections.length > 0 ? structuredSections : bodySections;

  // v1.2: sections に入っている関連Slugs/stockQuery を本文側セクションへ付与（title一致→index一致）
  const contentSections: BodySection[] = baseSections.map((sec, idx) => {
    if (structuredSections.length === 0) return sec;
    const match =
      (sec.title && structuredSections.find((s) => s.title === sec.title)) || structuredSections[idx];
    if (!match) return sec;
    return {
      ...sec,
      carSlugs: sec.carSlugs && sec.carSlugs.length > 0 ? sec.carSlugs : match.carSlugs ?? [],
      guideSlugs: sec.guideSlugs && sec.guideSlugs.length > 0 ? sec.guideSlugs : match.guideSlugs ?? [],
      columnSlugs:
        sec.columnSlugs && sec.columnSlugs.length > 0 ? sec.columnSlugs : match.columnSlugs ?? [],
      stockCarQuery: sec.stockCarQuery ?? match.stockCarQuery,
    };
  });
