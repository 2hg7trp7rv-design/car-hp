...
function extractInternalPaths(text) {
  ...
  for (const m of s.matchAll(relRe)) {
    ...
    // Avoid false positives inside external URLs.
    if (idx > 0) {
      const prev = s[idx - 1];
...
function collectDecisionStructuredStrings(obj) {
  const out = [];
  const push = (value) => {
    if (typeof value !== "string") return;
  ...
  visit(obj?.title);
  visit(obj?.summary);
  visit(obj?.seoDescription);
  visit(obj?.lead);
  visit(obj?.breadcrumbTrail);
  visit(obj?.keyPoints);
  visit(obj?.checkpoints);
  visit(obj?.faq);
  visit(obj?.actionBox);
  visit(obj?.detailSections);
  visit(obj?.sources);
  visit(obj?.updateReason);

  return out;
}

function collectInternalPathsFromContent(kind, obj) {
  const hits = new Set();
  const body = obj?.body;
  if (typeof body === "string" && body.trim()) {
    for (const p of extractInternalPaths(body)) hits.add(p);
  }

  const variant = asLower(obj?.layoutVariant);
  const isStructuredGuide = kind === "guides" && ["decision-v1", "guide-board-v2", "cbj-world-v1"].includes(variant);
  const isStructuredColumn = kind === "columns" && ["decision-v1", "column-decision-v2", "cbj-world-v1"].includes(variant);

  if (isStructuredGuide || isStructuredColumn) {
    for (const fragment of collectDecisionStructuredStrings(obj)) {
      for (const p of extractInternalPaths(fragment)) hits.add(p);
    }
  }
...
async function main() {
  ...
  for (const group of contentGroups) {
    for (const fp of group.files) {
      const obj = await readJson(fp);
      const paths = collectInternalPathsFromContent(group.kind, obj);
      if (paths.length === 0) continue;

      for (const p of paths) {
        if (!p) continue;

        // Internal links should point directly to canonical destinations.
        // Allowing redirect sources creates Search Console noise: "page with redirect".
        if (redirectSources.has(p)) {
          errors.push({ file: fp, path: p, reason: "internal link points to redirect source" });
          continue;
        }
