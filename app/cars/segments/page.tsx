    ...
    <main className="min-h-screen bg-site text-text-main">
      ...
      <div className="mx-auto max-w-7xl px-4 pb-32 pt-28 sm:px-6 lg:px-8">
        ...
        <nav className="mb-6 text-xs text-slate-500" aria-label="パンくずリスト">
          ...
          <span className="mx-2 text-slate-400">/</span>
          <span className="font-medium text-slate-700">SEGMENTS</span>
        </nav>

        {/* ヘッダー */}
        <header className="mb-8 rounded-3xl bg-white/80 p-5 shadow-soft-card backdrop-blur-sm sm:p-7 lg:p-8">
          <Reveal>
            <p className="text-[10px] font-bold tracking-[0.32em] text-tiffany-600">CAR DATABASE</p>
          </Reveal>
          <Reveal delay={80}>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              ...
              <div className="text-[10px] text-slate-500">
                ...
                <p className="mt-2 text-right text-2xl font-semibold text-slate-900">{segments.length}</p>
              </div>
            </div>
          </Reveal>
        </header>
        ...
        <section aria-label="セグメント一覧">
          <div className="mb-4 flex items-baseline justify-between gap-2">
            <h2 className="text-xs font-semibold tracking-[0.22em] text-slate-600">SEGMENT LIST</h2>
              ...
              return (
                <GlassCard key={s.key} padding="md" className="bg-white/80 hover:bg-white">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-[10px] font-semibold tracking-[0.22em] text-slate-500">SEGMENT</p>
                      <Link
                      ...
                      <p className="mt-1 text-[10px] text-slate-500">
                        モデル: <span className="font-semibold text-slate-700">{s.count}</span>
                      </p>
                    </div>
